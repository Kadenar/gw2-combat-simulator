...
function run(rotation, options = {}, profession = native()) {
  return runGw2Runtime({
    profession: profession.liveRuntimeFor({ specialization: 'Core' }),
    config,
    rotation,
    ...options
  });
...
test('live modifier hooks read executed history independently of report collection', () => {
  for (const output of ['detailed', 'score']) {
    ...
    }).liveRuntimeFor({ specialization: 'Core' });
    runGw2Runtime({
      profession: {
        ...source,
...
test('native duration selection composes before start mutations and rejects invalid reservations', () => {
  ...
  const elite = defineNativeModule({
    ...
    data: {},
    state: { create: () => ({}) },
    mechanics: { live: { castDurationMs: (_runtime, _skill, duration) => duration + 500 } }
  });
  const profession = defineNativeProfession({ id: 'native', name: 'Native', modules: [core, elite] });
  const result = runGw2Runtime({
    profession: profession.liveRuntimeFor({ specialization: 'Elite' }),
...
test('native live composition creates each selected state once and supports data-only modules', () => {
  const creates = [];
  const core = defineNativeModule({
    id: 'Core',
    data: {},
  ...
  assert.doesNotThrow(() =>
    defineNativeProfession({ id: 'data-only', name: 'Data only', modules: [unconverted] }).liveRuntimeFor({})
  );
  assert.throws(
...
test('live recharge anchors compose once per cast and reject nonfinite results', () => {
  ...
  const runtime = profession.liveRuntimeFor({ specialization: 'Elite' });
