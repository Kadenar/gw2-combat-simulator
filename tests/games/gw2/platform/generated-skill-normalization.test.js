...
test('generated normalization preserves extension fields and uses the caller-selected flip parent', () => {
  ...
  const source = Object.freeze({ id: 1, name: 'Fixture', recharge: 12, flipParentId: 2, effects, extension });
  ...
  assert.equal(Object.hasOwn(source, 'cooldown'), false);
...
test('generated recharge defaults distinguish missing inputs and explicit zero without inventing legacy fields', () => {
  for (const [fields, cooldown] of [
    [{}, 0],
    [{ recharge: 0 }, 0],
    [{ cooldown: 0, recharge: 12 }, 0],
    [{ ammo: 2, ammoRecharge: 8, recharge: 1 }, 8]
  ]) {
    ...
    assert.equal(Object.hasOwn(normalized, 'recharge'), Object.hasOwn(source, 'recharge'));
    assert.equal(normalized.recharge, source.recharge);
