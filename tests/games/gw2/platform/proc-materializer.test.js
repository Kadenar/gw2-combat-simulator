...
test('Energy uses the declared endurance owner and skips unsupported resources', () => {
  // Identical field names alone do not opt a profession into shared resource mutations.
  for (const supported of [false, true]) {
    const config = { sigilSets: [{ names: ['Energy'] }] };
    const { state } = createGw2TriggerMaterializer(config);
    const resource = { endurance: 30, enduranceUpdatedAt: 5 };
    const events = [];
    const context = {
      config,
      events,
      state: { time: 6, profession: { specialization: resource } },
      profession: {
        resources: {
          endurance: supported
            ? {
                state: (ctx) => ctx.state.profession.specialization,
                maximum: () => 100,
                regenerationRate: () => 5
              }
            : null
    ...
    createSigilProcEngine(config, state).materialize('swap', context, { type: 'sigil_swap', at: 6, weaponSet: 1 });
    assert.equal(resource.endurance, supported ? 85 : 30);
    assert.equal(resource.enduranceUpdatedAt, supported ? 6 : 5);
    assert.equal(events.filter((event) => event.type === 'resource').length, supported ? 1 : 0);
  }
});
