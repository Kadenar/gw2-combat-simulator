...
const icon = (document, name) =>
  [...document.querySelectorAll(".pal-skill")].find(
  ...
  try {
    ...
    assert(
      professionResourceStack &&
        professionResourceStack.firstElementChild?.classList.contains(
          "pal-group",
        ) &&
    ...
    assert(
      document.getElementById("target-hp")?.value === "3970000",
      "target HP control missing",
    );
    assert(
    ...
    assert(
      alternateMain?.value === "Spear",
      "alternate weapon-set control missing",
    );
    assert(
    ...
    attributeWeaponSet.dispatchEvent(
      new window.Event("change", { bubbles: true }),
    );
    assert(
      app.attributeWeaponSet === 2,
      "attribute weapon-set selector did not update the view",
    );
    assert(
    ...
    assert(
      app.results.endState.activeWeaponSet === 2,
      "weapon swap did not activate set 2",
    );
    const alternateSkill = app.skills.find(
      (skill) =>
        skill.type === "Weapon" &&
        skill.weapon === app.build.alternateWeapons[0],
    );
    assert(
      icon(document, alternateSkill.name),
      "palette did not switch to alternate weapon skills",
    );
    app.build.rotation = ["Bladecall", "Swap Weapons", "Psycut"];
    app.changed(false);
    const weaponRows = [
      ...document.querySelectorAll(
        "#rotation-timeline .rot-row:not(.rot-procs-row)",
      ),
    ];
    assert(
      weaponRows.length === 2,
      "weapon swap did not create a new timeline row",
    );
    assert(
      weaponRows[0]
        .querySelector(".rot-row-label")
        .textContent.includes("W1") &&
        weaponRows[1]
          .querySelector(".rot-row-label")
          .textContent.includes("W2"),
    ...
    app.changed(false);
    let timelineSkills = [
      ...document.querySelectorAll("#rotation-timeline .rot-skill"),
    ];
    ...
    assert(
      movedSkill.classList.contains("dragging"),
      "dragged timeline skill is not styled",
    );
    ...
    assert(
      emptyTimeline.classList.contains("is-empty"),
      "empty timeline is not using the centered empty state",
    );
    ...
    assert(
      app.build.rotation.length === 1,
      "normal palette click did not queue",
    );
    assert(
      icon(document, "Bladecall").querySelector(".pal-cd"),
      "cooldown badge missing after cast",
    );
    assert(
      icon(document, "Bladecall").title.includes("available at"),
      "ready time missing from cooldown tooltip",
    );
    const procPanel = document.querySelector(".rotation-procs-wrap");
    assert(
      procPanel,
      "relic and trait proc panel is missing below the rotation",
    );
    ...
    assert(
      procFilter && procMenu && procCheckbox,
      "proc visibility controls are missing",
    );
    ...
    assert(
      [...document.querySelectorAll(".proc-icon[data-proc-key]")]
        .filter(
          (procIcon) =>
            procIcon.dataset.procKey === procCheckbox.dataset.procKey,
        )
    ...
    assert(
      document
        .querySelector(".mesmer-skill-breakdown .res-skill img")
        ?.getAttribute("src"),
    ...
    assert(
      document
        .querySelector('[data-role="dps-tooltip"]')
        ?.textContent.includes("DPS:"),
    ...
    assert(
      document
        .querySelector('[data-role="effects-tooltip"]')
        ?.textContent.includes("s"),
      "effects chart hover tooltip is missing",
    );
    icon(document, "Bladecall").click();
    assert(
      app.results.steps[1].start === 4440,
      "second click did not wait for cooldown",
    );
    ...
    assert(
      app.build.rotation[1].offset === 100,
      "Shift+click did not create concurrent command",
    );
    assert(
      app.results.steps[1].start === 100,
      "concurrent command did not start at 100ms",
    );
    ...
    const editActivation = document.querySelector(
      '#rotation-timeline .rot-skill[data-idx="0"] .rot-edit-activation',
    );
    assert(editActivation, "normal cast does not expose the activation editor");
    editActivation.click();
    let activationEditor = document.querySelector(
      ".rotation-activation-editor",
    );
    assert(
      activationEditor &&
    ...
    assert(
      app.build.rotation[0].interruptMs === 120,
      "activation editor did not add a manual interruption",
    );
    let interruptBadge = document.querySelector(
    ...
    assert(
      activationEditor?.querySelector(".activation-editor-input")?.value ===
        "120",
    ...
    activationEditor.querySelector(".activation-editor-apply").click();
    assert(
      typeof app.build.rotation[0] === "string" &&
        app.build.rotation[0] === "Bladecall",
    ...
    assert(
      app.attributeData.attributes.Power.final !== originalPower,
      "gear UI did not recalculate attributes",
    );
    assert(
    ...
    assert(
      !icon(document, "Wait 1 second"),
      "obsolete one-second action remains in the palette",
    );
    ...
    app.changed();
    assert(
      icon(document, "Counterspell"),
      "Counterspell flip skill missing from Scepter palette",
    );
    assert(
    ...
    assert(
      !icon(document, "Counterspell").classList.contains(
        "pal-context-disabled",
      ),
    ...
    app.changed();
    assert(
      icon(document, "Split Second").querySelector(".pal-charges")
        ?.textContent === "2/2",
      "Shatter Storm did not expose two Split Second ammo charges",
    );
    assert(
      icon(document, "Split Second").querySelectorAll(".pal-ammo-pip.filled")
        .length === 2,
    ...
    assert(
      split?.nextElementSibling === shift,
      "Continuum Shift is not directly beside Continuum Split",
    );
    assert(
      shift.classList.contains("pal-context-disabled"),
      "Continuum Shift is enabled before the split",
    );
    assert(
    ...
    assert(
      !icon(document, "Continuum Shift").classList.contains(
        "pal-context-disabled",
      ),
