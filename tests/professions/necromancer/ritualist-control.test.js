...
test("Wanderlust omits minion knockdown while its player controls still apply", () => {
  const summoned = simulate(
    ["Ritualist's Shroud", "Wanderlust", { type: "wait", durationMs: 6000 }],
    { relic: "Claw" },
  );
  ...
  const innervated = simulate(
    ["Ritualist's Shroud", "Wanderlust", "Innervate Wanderlust"],
    { relic: "Claw" },
  );
  const controls = (result) =>
    result.events.filter((event) => event.type === "control");
