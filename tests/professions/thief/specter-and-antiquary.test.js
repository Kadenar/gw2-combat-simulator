...
test('Stealth attacks gain positional damage and consume malice for bonus damage', () => {
...
test('Specter conditions land with their associated strikes', () => {
  // Conditions follow the appropriate hit even when packet timings or coefficients change.
  for (const [rotation, skillId, expectedConditions] of [
    [[ID.TWILIGHT_COMBO], ID.TWILIGHT_COMBO, [['Chilled', 'Poisoned'], ['Torment']]],
    [
      [ID.ENTER_SHADOW_SHROUD, ID.ETERNAL_NIGHT],
      ID.ETERNAL_NIGHT,
      [
        ['Chilled', 'Poisoned'],
        ['Weakness', 'Poisoned']
    ...
    assert.deepEqual(result.warnings, []);
    assert.equal(hits.length, expectedConditions.length);
    assert.deepEqual(
      hits.map((hit) => conditions.filter((event) => event.at === hit.at).map((event) => event.condition)),
      expectedConditions
    );
  }
});
...
test('Twilight Combo activates Deadly Ambition once for the dual-wield cast', () => {
  ...
  assert.equal(poisons[0].condition, 'Poisoned');
});

test('Mind Shock grants stability before its delayed strike and stun', () => {
  // The defensive effect is immediate; the delayed damage and control must resolve together.
  const result = simulate(
    'Specter',
    [ID.ENTER_SHADOW_SHROUD, ID.MIND_SHOCK],
    {
      initialShadowForce: 100
    },
    observationTail(4000)
  );
  const stability = result.events.find((event) => event.type === 'buff' && event.kind === 'stability');
  const hit = result.events.find((event) => event.type === 'damage' && event.skillId === ID.MIND_SHOCK);
  const stun = result.events.find((event) => event.type === 'control' && event.skillId === ID.MIND_SHOCK);

  assert.deepEqual(result.warnings, []);
  assert.ok(hit.at > stability.at);
  assert.equal(stun.at, hit.at);
...
test('Specter wells preserve pulse spacing and effect order', () => {
  ...
  for (const [skillId, type, property, sequence] of [
    ...
    [ID.WELL_OF_BOUNTY, 'buff', 'kind', ['stability', 'might', 'fury', 'vigor', 'regeneration']]
  ]) {
...
test('Specter traits amplify force gains and add their Siphon recharge reductions', () => {
  ...
  const lead = simulate('Specter', ['Siphon'], { selectedTraitIds: [TRAIT.LEAD_ATTACKS] });
...
test('Specter attribute, ally, and shadowstep traits resolve explicitly', () => {
  ...
  assert.deepEqual(
    venomBuffs.map((event) => event.at),
    [barrier.at, dawnBarrier.at]
  );
  assert.equal(
...
test('Thieves Guild summons specialization-specific thieves and expires', () => {
  ...
  const lifetime = simulate('Specter', [
    'Thieves Guild',
    { type: 'combat-start' },
    { type: 'wait', durationMs: thiefCatalog.skillsById.get(ID.THIEVES_GUILD).summonAttack.duration * 1000 + 2000 }
  ]);
...
test('Thieves Guild uses independent summon weapons and attack profiles', () => {
  ...
  // Each summon uses its own weapon profile regardless of the player's equipped weapons.
  for (const [name, weapon] of [
    ['Male Dual-Pistol Thief', 'weapon.pistol'],
    ['Female Dual-Dagger Thief', 'weapon.dagger'],
    ['Staff Daredevil', 'weapon.staff']
  ]) {
    const hits = profile(name);
    assert.ok(hits.length > 0);
    assert.ok(hits.every((event) => event.weaponStrengthProfileId === weapon && event.independentSummonStrike));
  }
