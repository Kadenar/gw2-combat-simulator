...
test("native module contributions assemble disjoint application and runtime catalogs", () => {
  ...
  for (const { name, family, catalog, modules } of fixtures) {
    ...
    for (const module of modules) {
      assert.equal(
        module.kind,
        "native-profession-module",
        `${name}:${module.id}`,
      );
      assert.equal(
        typeof module.state.scheduler,
        "function",
        `${name}:${module.id}`,
      );
      assert.equal(
        Object.hasOwn(module, "catalog"),
        false,
        `${name}:${module.id}`,
      );
      ...
      for (const handlerId of handlers.keys()) {
        assert.equal(
          contributed.handlers.has(handlerId),
          false,
    ...
    for (const active of ["Core", ...family.specializationIds]) {
      ...
      const runtimeIds = new Set(
        runtime.catalog.skills.map((skill) => skill.id),
      );
...
function testFamily(
  core = testModule("Core"),
  elite = testModule("Elite"),
  ui = undefined,
) {
...
test("family UI uses active slices, Core-first event precedence, and family vetoes", () => {
  ...
  const elite = testModule("Elite", {
    ui: {
      eventLogRow: (_context, event) =>
        event.type === "shared" || event.type === "elite-only"
          ? descriptor("elite")
          : undefined,
    },
  });
  ...
  assert.equal(
    family
      ...
      .ui.eventLogRow({ config: { specialization: "Elite" } }, shared)
      .description,
    "core",
  );
  assert.equal(
    family.ui.eventLogRow({ specialization: "Elite" }, shared).description,
    "core",
  );
  assert.equal(
    family.ui.eventLogRow({ specialization: "Core" }, eliteOnly),
    undefined,
  );
  assert.equal(
    family.ui.eventLogRow({ build: { specialization: "Elite" } }, eliteOnly)
      .description,
    "elite",
  );
  assert.equal(
    family.ui.eventLogRow(
      { config: { specialization: "Core Line" } },
      { type: "context", at: 0 },
    ).description,
    "Core",
  );
  assert.deepEqual(
    family.ui.paletteSkillAvailability({ specialization: "Elite" }, eliteSkill),
    { available: false, message: "family veto" },
  );
});
...
test("family attribute declarations compile after active module composition", () => {
  ...
  assert.equal(
    family
      .resolveRuntime({ specialization: "Elite" })
      .modifyStrikeDamage({}, 10),
    12,
  );
  assert.deepEqual(compiledRuleIds, [
    ["core.rule"],
    ["core.rule", "elite.rule"],
  ]);
});
...
test("Necromancer modules contain complete vertical slices", () => {
  ...
  for (const relative of [
    ...
    assert.equal(
      existsSync(
        new URL(
          `../../js/professions/necromancer/${relative}`,
          import.meta.url,
        ),
      ),
      true,
  ...
  for (const [directory, module] of slices) {
    for (const filename of [
      ...
      if (filename === "skills.ts") {
        ...
        assert.match(
          source,
          /_BASE_SKILL_MECHANICS\b/,
          `${directory}/${filename} owns no skill mechanics`,
        );
        assert.doesNotMatch(
          source,
          /from\s+["'][^"']*catalog\.js["']/,
    ...
    assert.ok(
      (module.data?.generatedSkills?.length || 0) +
        (module.data?.extraSkills?.length || 0) >
        0,
    );
  ...
  assert.equal(
    modifierRuleOwners.get("necromancer.wicked-corruption-blight"),
    "Harbinger",
  );
...
test("Necromancer runtimes exclude sibling catalogs, handlers, and state", () => {
  ...
  for (const active of ["Core", ...NECROMANCER_ELITE_SPECIALIZATIONS]) {
    ...
    assert.equal(
      Object.hasOwn(runtime.eventHandlers, "necromancer.painful-bond"),
      active === "Ritualist",
      active,
    );
    assert.equal(
      Object.hasOwn(runtime.eventHandlers, "necromancer.weapon-spell"),
      active === "Ritualist",
      active,
    );
  }
...
test("Guardian modules own disjoint vertical slices", () => {
  ...
  assert.equal(
    modifierRuleOwners.get("guardian.radiant-power-critical-chance"),
    "Core",
  );
});
...
test("Mesmer modules are vertical slices with disjoint catalog ownership", () => {
  ...
  for (const [directory, module] of mesmerSlices) {
    for (const filename of [
      ...
      if (filename === "skills.ts") {
        assert.match(
          source,
          /MESMER_SKILL_IDS\s+as\s+ID/,
          `${directory}/${filename} must use the shared skill ID map`,
        );
        assert.match(
          source,
          /_SKILL_MECHANICS\b/,
  ...
  const coreSources = [
    ...
    .map((filename) =>
      readFileSync(
        new URL(
          `../../js/professions/mesmer/core/${filename}`,
          import.meta.url,
        ),
        "utf8",
      ),
...
test("Mesmer runtimes exclude inactive elite catalogs, registries, and state", () => {
  ...
  for (const active of ["Core", ...MESMER_ELITE_SPECIALIZATIONS]) {
    ...
    assert.equal(
      runtime.catalog.skills.some(
        (skill) =>
          skillOwner.get(skill.id) !== "Core" &&
          skillOwner.get(skill.id) !== activeElite,
      ),
      false,
      `${active}:skills`,
    );
    for (const [owner, keys] of Object.entries(mesmerSpecializationStateKeys)) {
      for (const key of keys) {
        const expected =
          key === "numericResource"
            ? active === "Virtuoso" || active === "Troubadour"
            : owner === active;
        assert.equal(
          Object.hasOwn(state.specialization.state, key),
          expected,
          `${active}:slice:${owner}:${key}`,
        );
      }
    }
...
test("Revenant modules are vertical slices with disjoint ownership", () => {
  ...
  assert.equal(
    modifierRuleOwners.get("revenant.heartpiercer-strike"),
    "Renegade",
  );
  assert.equal(
    modifierRuleOwners.get("revenant.leviathan-strength"),
    "Vindicator",
  );
  assert.equal(
    modifierRuleOwners.get("revenant.release-warrior-affinity"),
    "Conduit",
  );
  ...
  const coreSources = [
    ...
    .map((filename) =>
      readFileSync(
        new URL(
          `../../js/professions/revenant/core/${filename}`,
          import.meta.url,
        ),
        "utf8",
      ),
...
test("Revenant runtimes exclude inactive elite catalogs, hooks, and state", () => {
  ...
  for (const active of ["Core", ...REVENANT_ELITE_SPECIALIZATIONS]) {
    ...
    for (const [owner, keys] of Object.entries(
      revenantSpecializationStateKeys,
    )) {
      for (const key of keys) {
        assert.equal(
          Object.hasOwn(state.specialization.state, key),
...
test("Revenant runtime UI and public projection preserve their contracts", () => {
  for (const active of ["Core", ...REVENANT_ELITE_SPECIALIZATIONS]) {
    ...
    const resourceIds = runtime.ui
      ...
      .map((resource) => resource.id);
    assert.equal(
      resourceIds.includes("endurance"),
      active === "Vindicator",
      active,
    );
    assert.equal(
      resourceIds.includes("affinity"),
      active === "Conduit",
      active,
    );
  }
...
test("Engineer modules are vertical slices with disjoint ownership", () => {
  ...
  const coreSources = [
    ...
    .map((filename) =>
      readFileSync(
        new URL(
          `../../js/professions/engineer/core/${filename}`,
          import.meta.url,
        ),
        "utf8",
      ),
...
test("Engineer raw skill mechanics retain a disjoint no-loss union", () => {
  ...
  for (const [fragmentIndex, fragment] of fragments.entries()) {
    for (const skillId of Object.keys(fragment)) {
      assert.equal(
        owners.has(skillId),
        false,
        `skill mechanics ${skillId} owned by fragments ` +
          `${owners.get(skillId)} and ${fragmentIndex}`,
      );
  ...
  assert.deepEqual(
    ...
    Object.keys(ENGINEER_SKILL_MECHANICS).sort(
      (left, right) => Number(left) - Number(right),
    ),
  );
});
...
test("Engineer runtimes exclude inactive elite catalogs, hooks, and state", () => {
  ...
  for (const active of ["Core", ...ENGINEER_ELITE_SPECIALIZATIONS]) {
    ...
    for (const [owner, keys] of Object.entries(
      engineerSpecializationStateKeys,
    )) {
      for (const key of keys) {
        assert.equal(
          Object.hasOwn(state.specialization.state, key),
...
test("Engineer runtime UI and public projection preserve their contracts", () => {
  for (const active of ["Core", ...ENGINEER_ELITE_SPECIALIZATIONS]) {
    ...
    assert.equal(
      runtime.ui
        .skillBarGroups(uiContext)
        .some((group) => group.label === "Photon Forge"),
      active === "Holosmith",
      active,
    );
    assert.equal(
      runtime.ui
        .paletteGroups(uiContext)
        .some((group) => group.id === "engineer-forge"),
      active === "Holosmith",
      active,
    );
    assert.equal(
      runtime.ui.assumptionControls.some(
