...
test("Catalyst exposes every Jade Sphere beside its energy in the rotation palette", () => {
  ...
  assert.deepEqual(
    group.skillIds.map(
      (id) => elementalistProfession.catalog.skillsById.get(id)?.name,
    ),
    sphereNames,
  );
  assert.equal(
    new Set(
      group.skillIds.map(
        (id) => elementalistProfession.catalog.skillsById.get(id)?.icon,
      ),
    ).size,
    sphereIds.length,
  );
...
test("Elementalist utilities render beside profession controls before weapons", () => {
  ...
  const attunements = palette.innerHTML.indexOf(
    "elementalist-attunement-palette",
  );
...
test("Catalyst sphere palette availability reflects attunement and energy", () => {
  const fireSphere = elementalistProfession.catalog.skillsById.get(
    ELEMENTALIST_JADE_SPHERE_SKILL_IDS.Fire,
  );
  const waterSphere = elementalistProfession.catalog.skillsById.get(
    ELEMENTALIST_JADE_SPHERE_SKILL_IDS.Water,
  );
  const context = {
    specialization: "Catalyst",
    professionState: {
  ...
  assert.deepEqual(
    elementalistProfession.ui.paletteSkillAvailability(context, fireSphere),
    { available: true, message: "" },
  );
  assert.match(
    elementalistProfession.ui.paletteSkillAvailability(context, waterSphere)
      .message,
