...
test("Mesmer mechanics are the sole simulation source and use stable skill ids", () => {
  ...
  assert.equal(
    MESMER_SKILL_MECHANICS[ID.WINDS_OF_CHAOS].quicknessCastTimeMs,
    760,
  );
  assert.equal(
    mesmerCatalog.skillsById.get(ID.WINDS_OF_CHAOS).castTimeMs,
    1140,
  );
...
test("every cataloged phantasm has an attack timing before clone conversion", () => {
  ...
  for (const skill of phantasms) {
    ...
    assert.ok(
      timing.damageAtMs > 0,
      `${skill.name} has an invalid damage time`,
    );
    assert.ok(
      timing.spawnAtMs >= timing.damageAtMs,
      `${skill.name} converts before damage ends`,
    );
    assert.ok(
...
test("requested rifle, focus, and sword sequence flips are cataloged", () => {
  assert.deepEqual(
    MESMER_SUPPLEMENTAL_SKILLS.filter((skill) => skill.flipParentId).map(
      (skill) => [
        skill.name,
...
test("rotation actions use the requested icons without a duplicate fixed wait", () => {
  const dodge = PSEUDO_SKILLS.find(
    (skill) => skill.name === "Dodge / Mirage Cloak",
  );
  const mirror = PSEUDO_SKILLS.find(
    (skill) => skill.name === "Pick Up Mirage Mirror",
  );
...
test("supplied utility, spear, staff, and phantasm coefficients are preserved", () => {
  ...
  assert.equal(
    strikeEffects(lancer).find((effect) => effect.actorType === "phantasm")
      .coefficient,
    1.23,
  );
