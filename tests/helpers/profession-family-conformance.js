...
const UI_LIST_CALLBACKS = Object.freeze([
  "paletteGroups",
  "resourceViews",
  "skillBarGroups",
  ...
  for (const module of modules) {
    for (const id of select(module)) {
      assert.equal(
        owners.has(id),
        false,
        `${label} ${id} is owned by both ${owners.get(id)} and ${module.id}`,
      );
...
export function assertProfessionFamilyConformance({
  ...
  for (const [name, specialization] of [
    ...
    const expectedSpecializationState = specialization
      ? specialization.state.scheduler(config)
      : {};
    ...
    const activePresentations = [
      ...
      ...(specialization
        ? [presentationFor(specialization, family.catalog)]
        : []),
    ];
    assert.deepEqual(
