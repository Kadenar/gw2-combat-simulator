...
test("Ghost Pepper Popper grants Might by day and inflicts Chilled by night", () => {
  ...
  assert.equal(
    food.icon,
    "https://render.guildwars2.com/file/1C1D9D0407CD96F3E80266DEBD2B544E799AF658/433666.png",
  );
  ...
  const dayHits = day.resolvedEvents.filter(
    (event) => event.type === "damage" && event.skillName === "Flying Cutter",
  );
  assert.equal(
    day.procSteps.filter((event) => event.type === "food_proc").length,
    2,
  );
  assert.ok(dayHits.at(-1).damage > dayHits[0].damage);
  assert.equal(
    day.resolvedEvents.some(
      (event) => event.type === "condition" && event.source === "Food",
    ),
    false,
  );
