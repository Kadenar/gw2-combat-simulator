...
test('modifier comparisons partition across a capped worker pool', () => {
  ...
  assert.deepEqual(
    partitionModifierComparisons(comparisons, 3).map((batch) => batch.map(({ index }) => index)),
    [
      [0, 3, 6],
      [1, 4, 7],
      [2, 5]
  ...
  assert.deepEqual(
    mergeModifierContributions([[{ id: 'low', dpsIncrease: 2 }], [{ id: 'high', dpsIncrease: 8 }]]).map(({ id }) => id),
    ['high', 'low']
  );
});
...
test('simulation config aggregates each weapon set sigils independently', () => {
  ...
  assert.equal(config.boons.aegis, true);
});
...
test('shared build finalization applies deltas and rebuilds derived stats', () => {
  ...
  assert.equal(result.attributes['Critical Chance'].final, original['Critical Chance'].final + 6);
  assert.equal(result.attributes['Critical Damage'].final, original['Critical Damage'].final + 1);
  assert.equal(result.attributes['Boon Duration'].final, original['Boon Duration'].final + 3);
  assert.equal(result.attributes['Condition Duration'].final, original['Condition Duration'].final + 4);
