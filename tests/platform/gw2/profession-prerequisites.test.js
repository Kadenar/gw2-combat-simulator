...
test("target-condition queries combine assumptions and chronological runtime state", () => {
  ...
  const runtime = {
    ...
    boons: new Map([
      ["target-vulnerability", [{ at: 0, expiresAt: 2, stacks: 3 }]],
    ]),
  };
    ...
    [
      {
        ...
        effects: [
          {
            description:
              "Increases strike damage while the target is poisoned.",
            status: "implemented",
          },
          {
...
test("profession event-log hooks present, hide, and diagnose custom events", () => {
  ...
  assert.deepEqual(
    simulationEventLogRows(result, null, profession).map(
      (row) => row.description,
    ),
