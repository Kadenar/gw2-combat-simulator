...
test("field interaction is inclusive and remains valid after later expiry", () => {
  const result = resolve([
    ...
    finisher(
      "delayed-outcome",
      { kind: "field-id", fieldId: "fire:delayed" },
      { at: 1, effectAt: 3 },
    ),
  ]);
  const combo = result.resolvedEvents.find((event) => event.type === "combo");
...
test("attempt IDs deduplicate packets and deterministic progress stays keyed", () => {
  ...
  assert.equal(
    result.resolvedEvents.filter((event) => event.type === "combo").length,
    1,
  );
  ...
  const keyed = resolve([
    ...
    ...Array.from({ length: 4 }, (_, index) =>
      finisher(
        `fire-keyed:${index}`,
        { kind: "field-id", fieldId: "fire:keyed" },
        { chance: 0.2 },
      ),
    ),
    finisher(
      "ice-keyed:1",
      { kind: "field-id", fieldId: "ice:keyed" },
      { chance: 0.2 },
    ),
    finisher(
      "fire-keyed:5",
  ...
  assert.deepEqual(
    keyed.resolvedEvents
      .filter((event) => event.type === "combo")
      .map((event) => event.fieldType),
    ["Fire"],
  );
});
...
test("Whirl applications do not multiply combos and authored double Blasts do", () => {
  ...
  assert.equal(
    combos.filter((event) => event.finisherType === "Whirl").length,
    1,
  );
  assert.equal(
    combos.filter((event) => event.finisherType === "Blast").length,
    2,
  );
