...
test("the universal combo table defines every field/finisher pair once", () => {
  ...
  assert.equal(
    new Set(
      COMBO_DEFINITIONS.map(
        ({ fieldType, finisherType }) => `${fieldType}|${finisherType}`,
      ),
    ).size,
    36,
  );
  for (const fieldType of COMBO_FIELD_TYPES) {
    for (const finisherType of COMBO_FINISHER_TYPES) {
      assert.equal(
        comboDefinition(fieldType, finisherType).fieldType,
        fieldType,
      );
    }
  }
  assert.throws(
    () => validateComboDefinitions(COMBO_DEFINITIONS.slice(1)),
    /all 36 field\/finisher pairs/,
  );
});
