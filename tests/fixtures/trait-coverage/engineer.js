...
const manifest = engineerCatalog.traits.map((trait) => {
  ...
  const description =
    String(trait.description || '').trim() || `Reviewed passive or utility behavior for ${trait.name}.`;
...
export const ENGINEER_TRAIT_COVERAGE = validateTraitCoverageManifest(engineerCatalog, manifest, {
