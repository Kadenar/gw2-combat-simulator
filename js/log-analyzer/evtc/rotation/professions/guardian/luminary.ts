...
function inferInitialRadiantCourage(context: EvtcProfessionReconstructionContext): EvtcRecordedRotationAction[] {
  ...
  return [
    {
      ...canonicalAction(initial.eventIndex, start, RADIANT_COURAGE, initial.event.skillId, 'initial-state'),
      precast: true
    }
  ];
}
...
export function reconstructLuminaryActions(
  ...
  return [...alignInitialRadiantForge(context, actions), ...inferInitialRadiantCourage(context)];
}
