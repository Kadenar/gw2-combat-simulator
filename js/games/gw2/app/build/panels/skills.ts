...
export function skillBarDisplaySkill(
  ...
  while (current.flipSkillId != null && !visited.has(Number(current.id))) {
    ...
    if (
      skillFlipVisible(
        flips[flip.id],
        Number(app.results?.planningState?.atSeconds ?? app.results?.rotationEndTime ?? 0)
      )
    )
      display = flip;
    current = flip;
              ...
              .map(
                (entry) =>
                  `<button type="button" class="dd-item fixed-loadout-option${
                    ...
                    aria-selected="${entry.value === selector.value}" ${wikiTooltipAttributes(entry.label)}${entry.disabled ? ' disabled' : ''}>
