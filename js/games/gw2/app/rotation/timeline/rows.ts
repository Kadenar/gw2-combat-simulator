...
import { skillTooltipAttributes } from '#gw2/app/shared/wiki-tooltip.js';
  ...
  const timelineLines = rows.map((row, rowNumber) => {
    ...
    row.skills.forEach(({ entry, index }) => {
      ...
      const entryHtml = `${deadTimeHtml}<div class="rot-skill${item.concurrentOffsetMs != null ? ' rot-concurrent' : ''}${invalid ? ' rot-invalid' : ''}${chargeMismatch ? ' rot-charge-mismatch' : ''}${cancelledWithoutDamage ? ' rot-cancelled' : ''}"${readOnly ? '' : ' draggable="true"'}
                    data-idx="${index}"${combatTimeAttribute} data-skill-highlight-key="${esc(highlightKey)}" ${skill ? `${skillTooltipAttributes(skill, skillTooltip + titleSuffix + resourceTitle)} data-wiki-delay="700" tabindex="0"` : `title="${esc(skillTooltip + titleSuffix + resourceTitle)}"`} style="--att-border:${cancelledWithoutDamage ? '#ff3b45' : '#9d7bd0'}">
                    <img src="${esc(icon)}" alt="" />
