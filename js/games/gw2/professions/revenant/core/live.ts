...
import {
  ...
  REVENANT_EMIT_TASK,
  revenantLiveCombatActive
} from '#gw2/professions/revenant/core/live-events.js';
import {
...
} from '#gw2/professions/revenant/core/live-upkeep.js';
import {
...
} from '#gw2/professions/revenant/core/live-weapons.js';
import {
...
export const revenantLiveEnergy: ResourcePolicy<RevenantRuntime> = {
  kind: 'continuous',
...
export function revenantLiveEnduranceRate(runtime: RevenantRuntime, vigor: boolean): number {
...
export const revenantLiveEndurance: EndurancePolicy<RevenantRuntime> = {
  state: (runtime) => runtime.profession.core,
  maximum: () => REVENANT_MAXIMUM_ENDURANCE,
  regenerationRate: (runtime, vigor) => revenantLiveEnduranceRate(runtime, vigor)
};
...
function revenantAvailability(runtime: RevenantRuntime, skill: Skill, _command: CastCommand): AvailabilityResult {
  ...
  const cost = liveRevenantEnergyCost(runtime, skill as RevenantSkill);
...
/** Live Core: Energy, upkeeps, legends, weapon follow-ups, and actual hit/application trait reactions. */
export const revenantCoreLiveMechanics: Partial<RuntimeProfession<RevenantRuntimeState>> = {
  resources: { energy: revenantLiveEnergy },
  endurance: revenantLiveEndurance,
  initialize(runtime) {
  ...
  onCastStart(runtime, cast) {
    ...
    else if (isRevenantUpkeep(skill)) upkeepCosts.set(cast, liveRevenantEnergyCost(runtime, skill));
    else if (skill.id !== ID.SWAP_LEGENDS)
      runtime.resourceController.spend('energy', liveRevenantEnergyCost(runtime, skill));
  ...
  onCooldownReset(runtime) {
    ...
    if (!revenantLiveCombatActive(runtime)) return;
