...
export function grantLiveKallasFervor(
...
function ashenDemeanor(runtime: RevenantRuntime, cast: RuntimeCast): void {
  ...
  for (let stack = 0; stack < Math.max(0, balanceProfileNumber(profile, 'fervorStacks')); stack += 1)
    grantLiveKallasFervor(runtime, { sourceId: TRAIT.ASHEN_DEMEANOR, sourceName: profile.name });
...
function criticalTraits(runtime: RevenantRuntime, event: Gw2ResolverEvent, hit?: Gw2HitResolutionContext): void {
  ...
  if (ambush && (Boolean(runtime.config.target?.defiant) || critical))
    grantLiveKallasFervor(runtime, { sourceId: TRAIT.AMBUSH_COMMANDER, sourceName: 'Ambush Commander', cause: event });
  const state = renegadeState.from(runtime);
...
function furyTraits(runtime: RevenantRuntime, event: Gw2ResolverEvent): void {
  ...
  if (
    ...
    grantLiveKallasFervor(runtime, { sourceId: TRAIT.BLOOD_FURY, sourceName: 'Blood Fury', cause: event });
  }
}
...
function invokeRenegade(runtime: RevenantRuntime): void {
  if (runtime.profession.core.activeLegendId !== LEGEND.RENEGADE || !revenantLiveCombatActive(runtime)) return;
  ...
  for (let index = 0; index < 2; index += 1)
    grantLiveKallasFervor(runtime, { sourceId: TRAIT.SONG_OF_THE_MISTS, sourceName: song.name });
}

/** Renegade owns Fervor, warband summons, Kalla's commands, and their actual hit/boon reactions. */
export const renegadeLiveMechanics: Partial<RuntimeProfession<RevenantRuntimeState>> = {
  initialize(runtime) {
