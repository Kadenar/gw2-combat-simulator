...
import { grantNecromancerLifeForce, necromancerLiveLifeForce } from '#gw2/professions/necromancer/core/live.js';
...
import { ritualistSpellLiveMechanics } from '#gw2/professions/necromancer/specializations/ritualist/live-spells.js';
...
export const ritualistLiveMechanics: Partial<RuntimeProfession<NecromancerRuntimeState>> = {
  ...ritualistSpellLiveMechanics,
  resources: {
    lifeForce: {
      ...necromancerLiveLifeForce,
      recovery(runtime) {
        if (
        ...
        return necromancerLiveLifeForce.recovery(runtime);
      }
    }
  },
  initialize(runtime) {
  ...
  modifyEffects(runtime, cast, effects) {
    ...
    return RITUALIST_SPIRIT_PROFILE_BY_SKILL_ID[Number(cast.skill.id)] ||
      ...
      ? []
      : ritualistSpellLiveMechanics.modifyEffects!(runtime, cast, effects);
  },
  ...
  onCastComplete(runtime, cast) {
    ritualistSpellLiveMechanics.onCastComplete!(runtime, cast);
  ...
  tasks: {
    ...ritualistSpellLiveMechanics.tasks,
    [AUTO]: auto,
    [PACKET](runtime, data) {
