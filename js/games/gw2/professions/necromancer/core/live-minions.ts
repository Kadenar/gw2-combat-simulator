...
export function ownsLiveMinionSkill(skill: NecromancerSkill): boolean {
  return Boolean(
...
export function liveMinionAvailability(runtime: NecromancerRuntime, skill: NecromancerSkill) {
...
export function completeLiveMinion(runtime: NecromancerRuntime, cast: RuntimeCast): void {
  const skill = cast.skill as NecromancerSkill;
  if (!ownsLiveMinionSkill(skill)) return;
