...
import {
  ...
  liveWeaponTasks
} from '#gw2/professions/necromancer/core/live-weapons.js';
import {
  scheduleNecromancerConditions,
  reactToLiveNecromancerConditions,
  liveConditionTasks
} from '#gw2/professions/necromancer/core/live-conditions.js';
import { NECROMANCER_LICH_SKILL_IDS } from '#gw2/professions/necromancer/core/skills/index.js';
import {
  observeLiveNecromancerAutoattackTransition,
  necromancerLiveSwordTasks
...
import {
  ...
  livePassiveTasks
} from '#gw2/professions/necromancer/core/live-passives.js';
import {
...
import {
  completeLiveMinion,
  liveMinionAvailability,
  liveMinionTasks,
  ownsLiveMinionSkill
} from '#gw2/professions/necromancer/core/live-minions.js';
...
export const necromancerLiveLifeForce: ResourcePolicy<NecromancerRuntime> = {
  kind: 'continuous',
...
function complete(runtime: NecromancerRuntime, cast: RuntimeCast): void {
  ...
  completeLiveMinion(runtime, cast);
...
export const necromancerLiveMechanics: Partial<RuntimeProfession<NecromancerRuntimeState>> = {
  rechargeStart: (_runtime, cast, at) => modifyNecromancerRechargeStart(cast, at),
  resources: { lifeForce: necromancerLiveLifeForce },
  initialize(runtime) {
  ...
  availability(runtime, rawSkill) {
    ...
    const minionGate = liveMinionAvailability(runtime, skill);
  ...
  modifyEffects: (runtime, cast, effects) =>
    ownsLiveMinionSkill(cast.skill as NecromancerSkill) || cast.skill.id === ID.DEVOURING_DARKNESS
      ? []
  ...
  onAutoattackChainTransition: observeLiveNecromancerAutoattackTransition,
  ...
  reactions: {
    ...
    'damage.resolved'(runtime, event, details) {
      ...
      reactToLiveNecromancerConditions(runtime, event);
