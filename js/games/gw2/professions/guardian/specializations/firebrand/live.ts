...
import { applyGuardianVirtueActivationTraits } from '#gw2/professions/guardian/core/live.js';
...
import {
  ...
  initializeLiveMantras,
  liveMantraAvailability,
  liveMantraWake,
  refreshLiveMantras
} from '#gw2/professions/guardian/specializations/firebrand/live-mantras.js';
import {
  firebrandEffectTasks,
  startLiveAshes,
  reactToFirebrandLiveDamage,
  reactToFirebrandLiveBuff,
  reactToFirebrandLiveControl
} from '#gw2/professions/guardian/specializations/firebrand/live-effects.js';
...
export const firebrandLiveMechanics: Partial<RuntimeProfession<GuardianRuntimeState>> = {
  resources: {
    tomePages: {
      kind: 'discrete',
  ...
  onCastComplete(runtime, cast) {
    ...
    completeLiveMantra(runtime, cast);
    const state = firebrandState.from(runtime);
  ...
  onCooldownReset: refreshLiveMantras,
  reactions: {
    'damage.resolved'(runtime, event, details) {
      reactToFirebrandLiveDamage(runtime, event, details as NativeResolvedDamageDetails);
    ...
    'buff.applied'(runtime, event) {
      ...
      reactToFirebrandLiveBuff(runtime, event);
    },
    'control.resolved': reactToFirebrandLiveControl,
    'condition.applied'(runtime, event) {
      if (event.condition === 'Immobilized' || event.condition === 'Slow') reactToFirebrandLiveControl(runtime, event);
    }
  },
  tasks: { ...firebrandEffectTasks, [COURAGE]: courage, [FIREBRAND_MANTRA_WAKE]: liveMantraWake }
};
