export const MESMER_SKILL_IDS = Object.freeze({
  DODGE_MIRAGE_CLOAK: -1,
  PICK_UP_MIRAGE_MIRROR: -2,
  SWAP_WEAPONS: -3,
  CONTINUUM_SHIFT: -4,
  DODGE_TROUBADOUR: -5,

  CONFUSING_IMAGES: 10168,
  CHAOS_STORM: 10169,
  MIND_SLASH: 10170,
  MIND_GASH: 10171,
  MIND_SPIKE: 10172,
  PHANTASMAL_SWORDSMAN: 10174,
  PHANTASMAL_DUELIST: 10175,
  PHANTASMAL_MAGE: 10189,
  CRY_OF_FRUSTRATION: 10190,
  MIND_WRACK: 10191,
  DISTORTION: 10192,
  POWER_SPIKE: 10212,
  PHANTASMAL_WARLOCK: 10216,
  MIND_STAB: 10218,
  SPATIAL_SURGE: 10219,
  PHANTASMAL_BERSERKER: 10221,
  MAGIC_BULLET: 10229,
  PHANTASMAL_DISENCHANTER: 10267,
  WINDS_OF_CHAOS: 10273,
  ILLUSIONARY_COUNTER: 10276,
  ILLUSIONARY_RIPOSTE: 10280,
  PHANTASMAL_WARDEN: 10282,
  DIVERSION: 10287,
  ETHER_BOLT: 10289,
  ETHER_BLAST: 10290,
  ETHER_CLONE: 10291,
  SWAP: 10337,
  MIRROR_BLADE: 10333,
  BLURRED_FRENZY: 10334,
  COUNTER_BLADE: 10358,
  INTO_THE_VOID: 10363,
  COUNTERSPELL: 10314,
  PHANTASMAL_DEFENDER: 10341,
  SIGNET_OF_THE_ETHER: 21750,
  CONTINUUM_SPLIT: 29830,
  WELL_OF_SENILITY: 29856,
  GRAVITY_WELL: 30359,
  WELL_OF_CALAMITY: 30525,
  ECHO_OF_MEMORY: 30769,
  WELL_OF_ACTION: 30814,
  MIRROR_STRIKES: 41164,
  ETHER_BARRAGE: 42304,
  LINGERING_THOUGHTS: 45243,
  AXES_OF_SYMMETRY: 43761,
  IMAGINARY_AXES: 44321,
  LACERATING_CHOP: 44791,
  ETHEREAL_CHOP: 44840,
  MIRAGE_THRUST: 45230,
  RAIN_OF_SWORDS: 45425,
  TIME_SINK: 56873,
  REWINDER: 56928,
  SPLIT_SECOND: 56930,
  FLYING_CUTTER: 62510,
  CUTTER_BURST: 62554,
  TROUBADOUR_BLADECALL: 62560,
  BLADECALL: 69311,
  BLADESONG_DISSONANCE: 62602,
  UNSTABLE_BLADESTORM: 62607,
  BLADESONG_SORROW: 62616,
  BLADESONG_HARMONY: 62617,
  BLADETURN_REQUIEM: 62597,
  BLADESONG_DISTORTION: 68273,
  VIRTUOSO_TROUBADOUR_LINGERING_THOUGHTS: 69344,
  VIRTUOSO_TROUBADOUR_AXES_OF_SYMMETRY: 69385,
  PHANTOM_RAZOR: 69389,
  EFFERVESCENCE: 71800,
  DIMENSIONAL_APERTURE: 71792,
  ABSTRACTION: 72076,
  PHANTASMAL_SHARPSHOOTER: 72007,
  PHANTASMAL_LANCER: 72946,
  MIND_THE_GAP: 73093,
  PSYCUT: 73154,
  PSYSTRIKE: 73066,
  MIND_PIERCE: 73095,
  FRACTURED_GLASS: 73067,
  SPLIT_SURGE: 44241,
  CHAOS_VORTEX: 40184,
  MIRAGE_MIRROR_DAMAGE: 44677,

  LIVELY_LUTE: 76552,
  FLUSTERING_FLUTE: 76746,
  CRESCENDO: 76931,
  HARMONIOUS_HARP: 76960,
  HARMONIOUS_HARP_ALTERNATE: 77077,
  DEAFENING_DRUM: 77079,
  TALE_OF_THE_TORTURED_MASTERMIND: 77066,
  LIVELY_LUTE_ALTERNATE: 77306,

  // Generated coverage for every skill in the API metadata snapshot.
  ILLUSIONARY_LEAP: 10173, // Illusionary Leap
  ETHER_FEAST: 10176, // Ether Feast
  MIRROR: 10177, // Mirror
  TEMPORAL_CURTAIN: 10186, // Temporal Curtain
  MIRROR_IMAGES: 10202, // Mirror Images
  MANTRA_OF_PAIN: 10211, // Mantra of Pain
  MANTRA_OF_RECOVERY: 10213, // Mantra of Recovery
  ILLUSIONARY_WAVE: 10220, // Illusionary Wave
  SIGNET_OF_DOMINATION: 10232, // Signet of Domination
  SIGNET_OF_MIDNIGHT: 10234, // Signet of Midnight
  MASS_INVISIBILITY: 10245, // Mass Invisibility
  SIGNET_OF_ILLUSIONS: 10247, // Signet of Illusions
  THE_PRESTIGE: 10285, // The Prestige
  FEEDBACK: 10302, // Feedback
  PHASE_RETREAT: 10310, // Phase Retreat
  TIME_WARP: 10311, // Time Warp
  CHAOS_ARMOR: 10331, // Chaos Armor
  THOUSAND_CUTS: 24755, // Thousand Cuts
  SIGNET_OF_HUMILITY: 29519, // Signet of Humility
  WELL_OF_PRECOGNITION: 29526, // Well of Precognition
  MIMIC: 29578, // Mimic
  WELL_OF_ETERNITY: 30305, // Well of Eternity
  TIDES_OF_TIME: 30643, // Tides of Time
  SWORD_OF_DECIMATION: 35637, // Sword of Decimation
  FALSE_OASIS: 40200, // False Oasis
  CRYSTAL_SANDS: 41065, // Crystal Sands
  MIRAGE_ADVANCE: 42851, // Mirage Advance
  SAND_THROUGH_GLASS: 43064, // Sand through Glass
  BLADE_RENEWAL: 43343, // Blade Renewal
  ILLUSIONARY_AMBUSH: 45046, // Illusionary Ambush
  JAUNT: 45449, // Jaunt
  TWIN_BLADE_RESTORATION: 62522, // Twin Blade Restoration
  BLADE_LEAP: 62568, // Blade Leap
  FRIENDLY_FIRE: 71892, // Friendly Fire
  JOURNEY: 71897, // Journey
  INSPIRING_IMAGERY: 72005, // Inspiring Imagery
  SINGULARITY_SHOT: 72008, // Singularity Shot
  MENTAL_COLLAPSE: 72957, // Mental Collapse
  IMAGINARY_INVERSION: 73152, // Imaginary Inversion
  TALE_OF_THE_HONORABLE_ROGUE: 76611, // Tale of the Honorable Rogue
  TALE_OF_THE_SECOND_SCION: 76695, // Tale of the Second Scion
  TALE_OF_THE_SOULKEEPER: 76850, // Tale of the Soulkeeper
  TALE_OF_THE_AUGUST_QUEEN: 76971, // Tale of the August Queen
  TALE_OF_THE_VALIANT_MARSHAL: 77178 // Tale of the Valiant Marshal
});

export const MESMER_TRAIT_IDS = Object.freeze({
  ALTERED_CHORD: 2441,
  BLINDING_DISSIPATION: 1889,
  BLOODSONG: 2223,
  BOUNTIFUL_BLADES: 686,
  CHAOTIC_INTERRUPTION: 669,
  CHRONOPHANTASMA: 1890,
  COMPOUNDING_POWER: 723,
  CRY_OF_PAIN: 734,
  DANGER_TIME: 2009,
  DAZZLING: 694,
  DEADLY_BLADES: 2204,
  DECEPTIVE_EVASION: 704,
  DELAYED_REACTIONS: 1838,
  DESERT_DISTORTION: 2178,
  DUNE_CLOAK: 2169,
  EGOTISM: 713,
  ELUSIVE_MIND: 2113,
  EMPOWERED_ILLUSIONS: 682,
  FENCERS_FINESSE: 708,
  FLOW_OF_TIME: 1927,
  FORTISSIMO: 2353,
  FRAGILITY: 1941,
  ILLUSIONARY_MEMBRANE: 667,
  ILLUSIONARY_REVERSION: 1913,
  INEPTITUDE: 1950,
  INFINITE_FORGE: 2206,
  INFINITE_HORIZON: 2070,
  JAGGED_MIND: 2202,
  MAIM_THE_DISILLUSIONED: 1690,
  MALICIOUS_SORCERY: 753,
  MASTER_FENCER: 707,
  MASTER_OF_FRAGMENTATION: 2035,
  MAYHEM: 2427,
  MASTER_OF_MISDIRECTION: 731,
  MENTAL_ANGUISH: 680,
  MENTAL_FOCUS: 2208,
  METHOD_OF_MADNESS: 670,
  MIRAGE_MANTLE: 2174,
  NOMADS_ENDURANCE: 2069,
  PHANTASMAL_BLADES: 2205,
  PHANTASMAL_FORCE: 733,
  PHANTASMAL_FURY: 701,
  PHANTASMAL_HASTE: 729,
  PHANTOM_PAIN: 2098,
  THE_PLEDGE: 691,
  QUIET_INTENSITY: 2193,
  RENEWING_OASIS: 2082,
  RIDDLE_OF_SAND: 2110,
  SELF_DECEPTION: 2141,
  SHARPER_IMAGES: 710,
  SHATTER_STORM: 721,
  SEIZE_THE_MOMENT: 2022,
  SHREDDING: 2343,
  STRETCHED_TIME: 1942,
  SUPERIORITY_COMPLEX: 692,
  SYNCOPATE: 2432,
  RACONTEUR: 2326,
  LIFE_OF_THE_PARTY: 2367,
  CALL_AND_RESPONSE: 2414,
  HARMONIZE: 2374,
  TIME_CATCHES_UP: 1995,
  TIME_BOMB: 1978,
  VICIOUS_EXPRESSION: 681,
  // Named IDs let presentation cover catalog traits without numeric keys.
  ILLUSION_OF_VULNERABILITY: 685,
  RENDING_SHATTER: 687,
  SHATTERED_CONCENTRATION: 693,
  FURIOUS_INTERRUPTION: 712,
  POWER_BLOCK: 1688,
  CRITICAL_INFUSION: 706,
  MENTAL_GYMNASTICS: 705,
  DUELISTS_DISCIPLINE: 700,
  WANDERING_MIND: 1960,
  METAPHYSICAL_REJUVENATION: 666,
  CHAOTIC_PERSISTENCE: 1865,
  ILLUSIONARY_DEFENSE: 675,
  MASTER_OF_MANIPULATION: 677,
  AUSPICIOUS_ANGUISH: 673,
  CHAOTIC_TRANSFERENCE: 668,
  SHAPER_OF_CHAOS: 671,
  PRISMATIC_UNDERSTANDING: 674,
  BOUNTIFUL_DISILLUSIONMENT: 1687,
  MENDERS_PURITY: 757,
  INSPIRING_DISTORTION: 1852,
  ILLUSIONARY_INSPIRATION: 1915,
  MEDICS_FEEDBACK: 756,
  RESTORATIVE_MANTRAS: 738,
  SYMPATHETIC_VISAGE: 744,
  WARDENS_FEEDBACK: 751,
  EGO_RESTORATION: 740,
  TEMPORAL_ENCHANTER: 1980,
  MENTAL_DEFENSE: 2005,
  RESTORATIVE_ILLUSIONS: 1866,
  BLURRED_INSCRIPTIONS: 752,
  PERSISTENCE_OF_MEMORY: 1869,
  ESCAPE_ARTIST: 722,
  TIME_SPLITTER: 2030,
  TIME_MARCHES_ON: 1859,
  ALLS_WELL_THAT_ENDS_WELL: 1987,
  MIRAGE_CLOAK: 2150,
  SPEED_OF_SAND: 2117,
  PSYCHIC_BLADES: 2216,
  BLADETURN_REFRAIN: 2212,
  DUELISTS_REVERSAL: 2215,
  SHARPENING_SORROW: 2207,
  PSYCHIC_RIPOSTE: 2211,
  WANDERING_MINSTREL: 2386,
  SYMPHONIC_RESONANCE: 2424,
  LOVE_SONG: 2422
});
