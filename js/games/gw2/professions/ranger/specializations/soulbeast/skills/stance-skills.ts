...
export const SOULBEAST_STANCE_SKILL_MECHANICS: Readonly<Record<number, Partial<Skill>>> = Object.freeze({
  [ID.VULTURE_STANCE]: {
    castTimeMs: 0,
    effects: []
    // Custom: Opens the Vulture Stance proc window with stance-duration traits; see `soulbeast/hooks.ts`.
  },
  [ID.BEAR_STANCE]: {
    effects: [],
    castTimeMs: 500
  ...
  [ID.ONE_WOLF_PACK]: {
    ...
    effects: [],
    castTimeMs: 360
    // Custom: Opens the One Wolf Pack proc window with stance-duration traits; see `soulbeast/hooks.ts`.
  }
});
