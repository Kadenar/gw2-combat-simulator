...
export const SOULBEAST_BEASTMODE_SKILL_MECHANICS: Readonly<Record<number, Partial<Skill>>> = Object.freeze({
  [ID.BEASTMODE]: {
    ...
    paletteTileOrder: 1,
    effects: [],
    // Custom: Enters Beastmode, disables the pet actor, and applies toggle traits; see `soulbeast/hooks.ts`.
  ...
  [ID.LEAVE_BEASTMODE]: {
    ...
    paletteTileOrder: 2,
    effects: [],
    // Custom: Leaves Beastmode, restores the pet actor, and applies toggle traits; see `soulbeast/hooks.ts`.
    inputCategory: 'bar-swap' // Count the explicit bar-changing input in effort summaries.
  },
  [ID.ETERNAL_BOND]: {
    castTimeMs: 0,
