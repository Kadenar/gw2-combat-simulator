...
function truncatedBarrageActions(
  ...
  return context.log.events.flatMap((event, eventIndex) => {
    ...
    return [
      {
        ...directAction(eventIndex, start, event.skillId, BARRAGE.name, BARRAGE, 'initial-state'),
        end: event.time,
...
export function reconstructGaleshotActions(
  ...
  const actions = normalizeFalseInterruptions(context, [...recordedActions, ...barrage]);
