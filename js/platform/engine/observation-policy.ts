...
export function normalizeObservationPolicy(
  ...
  if (policy.kind === "tail") {
    ...
    if (!Number.isFinite(durationMs) || durationMs < 0) {
      throw new TypeError(
        "Observation tail durationMs must be a non-negative finite number.",
      );
    }
  ...
  if (policy.kind === "absolute") {
    ...
    if (!Number.isFinite(endTimeMs) || endTimeMs < 0) {
      throw new TypeError(
        "Absolute observation endTimeMs must be a non-negative finite number.",
      );
    }
  ...
  throw new TypeError(
    `Unknown observation policy kind "${String(
      (policy as { readonly kind?: unknown }).kind,
    )}".`,
  );
}

/** Resolves a normalized policy after the entered command timeline is known. */
export function observationEndTime(
  policy: NormalizedObservationPolicy,
  ...
  if (!Number.isFinite(normalizedRotationEnd) || normalizedRotationEnd < 0) {
    throw new TypeError(
      "Rotation end time must be a non-negative finite number.",
    );
  }
  ...
  if (absoluteEnd < normalizedRotationEnd - EPSILON) {
    throw new RangeError(
      "Absolute observation endTimeMs cannot precede rotation end.",
    );
  }
