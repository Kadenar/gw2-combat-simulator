...
export function assertSimulationEvent(candidate: unknown): SimulationEvent {
  ...
  if (
    !COMMON_EVENT_TYPE_SET.has(event.type) &&
    !LEGACY_EVENT_TYPES.has(event.type) &&
    !event.type.includes(".")
  ) {
  ...
  if (
    ...
    !ACTOR_TYPES.has(event.actorType as SimulationActorType)
  ) {
    throw new Error("Event actorType is invalid.");
  }
  if (
    event.activationId !== undefined &&
  ...
  if (
    ...
    throw new Error(
      "Event weaponStrengthProfileId must be a non-empty string.",
    );
  }
  if (
    event.weaponStrength !== undefined &&
    !isFiniteNumber(event.weaponStrength)
  ) {
  ...
  if (event.type === "damage") {
    ...
    const hasDamageValue = [
      event.coefficient,
      event.flatDamage,
      event.flatStrikeBase,
      event.flatStrikePowerCoeff,
    ].some(isFiniteNumber);
    if (!hasDamageValue) {
      throw new Error(
        "Damage events require a finite coefficient or flat strike value.",
      );
    }
    if (
      event.hits !== undefined &&
      (!isFiniteNumber(event.hits) ||
