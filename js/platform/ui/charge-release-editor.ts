...
export function openChargeReleaseEditor(
  options: ChargeReleaseEditorOptions,
  ...
  const currentAvailable = options.rows.some(
    (row) => !row.disabled && row.charges === current,
  );
  const addChoice = (
    value: string,
    labelText: string,
  ...
  addChoice(
    "maximum",
    "Release at maximum",
    options.currentReleaseAtCharges == null || !currentAvailable,
  );
