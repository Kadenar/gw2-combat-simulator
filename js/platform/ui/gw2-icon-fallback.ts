...
export function nextGw2IconFallback(
  source: string,
  stage?: Gw2IconFallbackStage,
): Gw2IconFallback | null {
  if (stage === "backup") {
...
export function mountGw2IconFallback(root: Document = document): void {
  ...
  root.addEventListener(
    ...
    (event) => {
      ...
      const stage =
        currentStage === "backup" || currentStage === "placeholder"
          ? currentStage
          : undefined;
      const fallback = nextGw2IconFallback(
