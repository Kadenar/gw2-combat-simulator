...
export function openDurationEditor(
  options: DurationEditorOptions,
): DurationEditorHandle {
  closeDurationEditor();
  ...
  const label = editor.querySelector<HTMLLabelElement>(
    ".activation-editor-label",
  );
  ...
  const applyChanges = (): void => {
    const validation = validateDurationMs(
      input.value,
      options.minimumMs,
      options.maximumMs,
    );
