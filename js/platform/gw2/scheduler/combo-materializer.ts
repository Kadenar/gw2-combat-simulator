...
function fieldDescriptors(
  ...
  const descriptors = Array.isArray(event.comboFields)
    ...
    : event.type === "action" && Array.isArray(skill?.comboFields)
      ...
      : [];
  return descriptors
    .filter((raw): raw is SchedulerRecord =>
...
function hasEffectFinishers(
  effects: readonly SchedulerRecord[] | undefined,
): boolean {
  return Boolean(
    effects?.some(
      (effect) =>
...
function finisherDescriptors(
  ...
  if (Array.isArray(event.comboFinishers)) {
    ...
    const actionWithoutStrikes =
      event.type === "action" &&
      !skill.effects?.some((effect) => effect.type === "strike");
    const damagingPacket =
      event.type === "damage" && Number(event.coefficient || 0) > 0;
  ...
  return descriptors
    ...
    .map((raw) => ({
      ...
      successfulCombos: Math.max(
        1,
        Math.trunc(Number(raw.successfulCombos ?? 1)),
      ),
    }));
}
...
function projectilePacketIdentity(
  event: SimulationEvent,
  parentEventOrder: number,
): string {
...
function produceFinisher(
  ...
  const at =
    event.type === "action" ? Number(event.endsAt ?? event.at) : event.at;
  const {
    binding,
    fieldAt: boundFieldAt,
    warnOnUnbound,
  } = descriptorBinding(context, descriptor, at);
  ...
  for (let attempt = 1; attempt <= descriptor.attempts; attempt += 1) {
    ...
    context.emitDerived(event, {
      ...
      effectAt: Math.max(
        resolvedAt,
        Number(descriptor.effectAt ?? at + Number(descriptor.effectDelay || 0)),
      ),
      source: event.source,
      sourceId: event.sourceId,
...
export function produceGw2OwnedComboEvents(
  context: SchedulerContext,
  event: SimulationEvent,
): void {
