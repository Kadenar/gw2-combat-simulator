...
export function resolvedWeaponStrength(
  ...
  if (!profileId) {
    const fallback = Number(
      context.helpers.weaponStrength(event, context.config),
    );
    return {
      activationId:
        typeof event.activationId === "string" ? event.activationId : null,
  ...
  const result = {
    profileId: profile.id,
    value: sampleWeaponStrength(
      profile,
      context.random.next(`weapon-strength:${streamActor(event)}`),
    ),
  };
