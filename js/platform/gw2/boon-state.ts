 ...
 */
export function remainingDurationStackSeconds<
  T extends DurationStackApplication,
>(
  ...
  for (const application of matching) {
    ...
    const stacks =
      application.stacks == null ? 1 : Math.max(0, Number(application.stacks));
    remaining = Math.min(
      Math.max(0, Number(maximum)),
      remaining + Math.max(0, applicationDuration) * stacks,
    );
