...
function defineProfile(
  ...
  rawMin: unknown,
  rawMax: unknown,
): Readonly<Gw2WeaponStrengthProfile> {
...
export const WEAPON_STRENGTH_PROFILES: Readonly<
  Record<string, Readonly<Gw2WeaponStrengthProfile>>
> = Object.freeze(
  Object.fromEntries(
...
export function weaponStrengthProfile(
  id: unknown,
): Readonly<Gw2WeaponStrengthProfile> {
  const key = String(id || "");
  const profile = WEAPON_STRENGTH_PROFILES[key];
  if (!profile)
    throw new RangeError(`Unknown weapon-strength profile: ${key}.`);
  return profile;
}

export function weaponStrengthProfileForName(
  name: unknown,
): Readonly<Gw2WeaponStrengthProfile> | null {
  const value = String(name || "")
    .trim()
    .toLowerCase();
  const id =
    (Object.hasOwn(WEAPON_STRENGTH_PROFILES, value) ? value : null) ||
    NAME_TO_PROFILE_ID[value];
  return id ? weaponStrengthProfile(id) : null;
}

export function weaponStrengthMidpoint(
  profile: Gw2WeaponStrengthProfile,
): number {
  return (Number(profile.min) + Number(profile.max)) / 2;
}

export function weaponStrengthHalfRange(
  profile: Gw2WeaponStrengthProfile,
): number {
  return (Number(profile.max) - Number(profile.min)) / 2;
}

export function sampleWeaponStrength(
  profile: Gw2WeaponStrengthProfile,
  unitIntervalValue: number,
): number {
