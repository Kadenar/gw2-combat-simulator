...
const ZERO_CRITICAL_CHANCE_PRECISION =
  LEVEL_80_BASE_PRECISION -
  BASE_CRITICAL_CHANCE_FRACTION * PRECISION_PER_CRITICAL_CHANCE_FRACTION;
const PRECISION_PER_CRITICAL_CHANCE_PERCENT =
  PRECISION_PER_CRITICAL_CHANCE_FRACTION / PERCENT_SCALE;
const BASE_CRITICAL_DAMAGE_PERCENT =
  BASE_CRITICAL_DAMAGE_MULTIPLIER * PERCENT_SCALE;
const FEROCITY_PER_CRITICAL_DAMAGE_PERCENT =
  FEROCITY_PER_CRITICAL_DAMAGE_MULTIPLIER / PERCENT_SCALE;
const EXPERTISE_PER_CONDITION_DURATION_PERCENT =
  EXPERTISE_PER_CONDITION_DURATION_MULTIPLIER / PERCENT_SCALE;
...
export function criticalChancePercentFromPrecision(precision: number): number {
  return (
    (precision - ZERO_CRITICAL_CHANCE_PRECISION) /
...
export function criticalChanceFractionFromPrecision(precision: number): number {
  return (
    BASE_CRITICAL_CHANCE_FRACTION +
    (precision - LEVEL_80_BASE_PRECISION) /
...
export function criticalDamagePercentFromFerocity(ferocity: number): number {
  return (
    ...
    ferocity / FEROCITY_PER_CRITICAL_DAMAGE_PERCENT
  );
}

export function criticalDamageMultiplierFromFerocity(
  ferocity: number,
): number {
  return (
    BASE_CRITICAL_DAMAGE_MULTIPLIER +
    ferocity / FEROCITY_PER_CRITICAL_DAMAGE_MULTIPLIER
  );
}

export function conditionDurationPercentFromExpertise(
  expertise: number,
): number {
  return expertise / EXPERTISE_PER_CONDITION_DURATION_PERCENT;
}

export function conditionDurationFractionFromExpertise(
  expertise: number,
): number {
