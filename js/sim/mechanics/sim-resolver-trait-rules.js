...
export function handleCriticalTraits(
  ...
  if (
    ...
    ctx.traitsState.sharperImagesProgress += hitContext.critical.chance;
    const procCount = Math.floor(
      ctx.traitsState.sharperImagesProgress + EPSILON,
    );
    if (procCount > 0) {
      ctx.traitsState.sharperImagesProgress -= procCount;
      applyCondition(ctx, {
        type: "condition",
        at: event.at,
...
export function triggerIneptitude(
  ...
  if (
    defiant
    && event.at < ctx.traitsState.ineptitudeReadyAt - EPSILON
  ) return;
  if (defiant) ctx.traitsState.ineptitudeReadyAt = event.at + 3;
  const count = defiant
    ? 1
