/**
 * Skill normalization: transforms catalog skills into usable form with activation/cooldown overrides.
 * Defines autoattack chains (skill progressions), pseudo-skills (Swap, Dodge, Continuum Shift),
 * and choice overrides (measured activations, damage corrections).
 */
...
/**
 * Normalizes a catalog skill: applies measured activation/cooldown overrides, autoattack chain metadata.
 * Clears damage/conditions for engine-handled skills (Shatters, Instruments, Crescendo).
 * @param {Object} skill - Catalog skill
 * @returns {Object} Normalized skill with overrides applied
 */
export function normalizedSkill(skill) {
  const override = choiceOverrides[skill.name] || {};
  const chain = autoattackChainPositions.get(skill.name);
  const normalized = {
    ...skill,
    ...override,
    damage: override.damage || skill.damage,
    conditions: override.conditions || skill.conditions,
    resource:
      Object.hasOwn(override, "resource") ? override.resource : skill.resource,
  };
  if (chain) {
    normalized.chainRoot = chain.root;
    normalized.chainStep = chain.step;
  }
  if (handledByEngine.has(skill.name)) {
    normalized.damage = [];
