...
import {
  GUARDIAN_CORE_BALANCE_PROFILES,
  GUARDIAN_CORE_BALANCE_PROFILE_IDS as PROFILE,
} from "./profiles.js";

const profile = (id: string | number) =>
  GUARDIAN_CORE_BALANCE_PROFILES.find((entry) => entry.id === id)!;
const effect = (id: string | number, index = 0) =>
  (profile(id).effects || [])[index]!;
...
export const GUARDIAN_CORE_MECHANICS = Object.freeze({
  spear: Object.freeze({
    illuminatedMultiplierBySkillId: Object.freeze({
      ...
      [ID.GLEAMING_DISC]: Number(
        profile(PROFILE.spearGleamingDisc).damageMultiplier,
      ),
      [ID.SOLAR_STORM]: Number(
        profile(PROFILE.spearSolarStorm).damageMultiplier,
      ),
    }),
    ...
    symbolLuminanceDurationMs:
      Number(effect(PROFILE.spearLuminance).duration) * 1000,
  }),
