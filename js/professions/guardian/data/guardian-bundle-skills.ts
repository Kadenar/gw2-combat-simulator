// Guardian bundle skills missing from the profession API payload.
// Simulation behavior lives exclusively in mechanics/skill-mechanics.js.

import type { GuardianSkill } from "../types.js";

function wikiSkillIcon(fileName: string): string {
  return `https://wiki.guildwars2.com/wiki/Special:Redirect/file/${
    encodeURIComponent(fileName)
  }`;
}

export const GUARDIAN_BUNDLE_SKILLS: readonly GuardianSkill[] = Object.freeze([
  {
    "id": 41258,
    "name": "Chapter 1: Searing Spell",
    "description": "Tome. Incite a swelling of heat, damaging and burning foes in front of you.",
    "icon": wikiSkillIcon("Chapter 1- Searing Spell.png"),
    "type": "Bundle",
    "weapon": "",
    "slot": "Weapon_1",
    "specialization": "Firebrand",
    "categories": [],
    "recharge": 0,
    "ammo": 0,
    "ammoRecharge": 0,
    "nextChainId": null,
    "flipSkillId": null,
    "tome": "justice",
    "pageCost": 1
  },
  {
    "id": 40635,
    "name": "Chapter 2: Igniting Burst",
    "description": "Tome. Ignite nearby foes and weaken them.",
    "icon": wikiSkillIcon("Chapter 2- Igniting Burst.png"),
    "type": "Bundle",
    "weapon": "",
    "slot": "Weapon_2",
    "specialization": "Firebrand",
    "categories": [],
    "recharge": 8,
    "ammo": 0,
    "ammoRecharge": 0,
    "nextChainId": null,
    "flipSkillId": null,
    "tome": "justice",
    "pageCost": 1
  },
  {
    "id": 42449,
    "name": "Chapter 3: Heated Rebuke",
    "description": "Tome. Pull nearby enemies toward you with a heated rebuke.",
    "icon": wikiSkillIcon("Chapter 3- Heated Rebuke.png"),
    "type": "Bundle",
    "weapon": "",
    "slot": "Weapon_3",
    "specialization": "Firebrand",
    "categories": [],
    "recharge": 10,
    "ammo": 0,
    "ammoRecharge": 0,
    "nextChainId": null,
    "flipSkillId": null,
    "tome": "justice",
    "pageCost": 1
  },
  {
    "id": 40015,
    "name": "Chapter 4: Scorched Aftermath",
    "description": "Tome. Create a pulsing fire field that burns and bleeds enemies.",
    "icon": wikiSkillIcon("Chapter 4- Scorched Aftermath.png"),
    "type": "Bundle",
    "weapon": "",
    "slot": "Weapon_4",
    "specialization": "Firebrand",
    "categories": [],
    "recharge": 15,
    "ammo": 0,
    "ammoRecharge": 0,
    "nextChainId": null,
    "flipSkillId": null,
    "tome": "justice",
    "pageCost": 1,
    "comboField": "Fire",
    "duration": 4
  },
  {
    "id": 42898,
    "name": "Epilogue: Ashes of the Just",
    "description": "Tome. Your next attacks inflict burning on their targets.",
    "icon": wikiSkillIcon("Epilogue- Ashes of the Just.png"),
    "type": "Bundle",
    "weapon": "",
    "slot": "Weapon_5",
    "specialization": "Firebrand",
    "categories": [],
    "recharge": 20,
    "ammo": 0,
    "ammoRecharge": 0,
    "nextChainId": null,
    "flipSkillId": null,
    "tome": "justice",
    "pageCost": 1
  },
  {
    "id": 45022,
    "name": "Chapter 1: Desert Bloom",
    "description": "Tome. Tales of desert blooms create a wave of healing for your allies.",
    "icon": wikiSkillIcon("Chapter 1- Desert Bloom.png"),
    "type": "Bundle",
    "weapon": "",
    "slot": "Weapon_1",
    "specialization": "Firebrand",
    "categories": [],
    "recharge": 0,
    "ammo": 0,
    "ammoRecharge": 0,
    "nextChainId": null,
    "flipSkillId": null,
    "tome": "resolve",
    "pageCost": 1
  },
  {
    "id": 40679,
    "name": "Chapter 2: Radiant Recovery",
    "description": "Tome. Release magic from pages detailing the rebuilding of Vabbi, cleansing conditions on nearby allies. Allies are healed for each condition removed.",
    "icon": wikiSkillIcon("Chapter 2- Radiant Recovery.png"),
    "type": "Bundle",
    "weapon": "",
    "slot": "Weapon_2",
    "specialization": "Firebrand",
    "categories": [],
    "recharge": 4,
    "ammo": 0,
    "ammoRecharge": 0,
    "nextChainId": null,
    "flipSkillId": null,
    "tome": "resolve",
    "pageCost": 1
  },
  {
    "id": 45128,
    "name": "Chapter 3: Azure Sun",
    "description": "Tome. Inspired by countless poems describing the comforting powers of the water-reflected sun, grant boons to allies.",
    "icon": wikiSkillIcon("Chapter 3- Azure Sun.png"),
    "type": "Bundle",
    "weapon": "",
    "slot": "Weapon_3",
    "specialization": "Firebrand",
    "categories": [],
    "recharge": 8,
    "ammo": 0,
    "ammoRecharge": 0,
    "nextChainId": null,
    "flipSkillId": null,
    "tome": "resolve",
    "pageCost": 1
  },
  {
    "id": 42008,
    "name": "Chapter 4: Shining River",
    "description": "Tome. Release a torrent of pages describing the water cycle of the Elon River. Heal allies and grant them swiftness.",
    "icon": wikiSkillIcon("Chapter 4- Shining River.png"),
    "type": "Bundle",
    "weapon": "",
    "slot": "Weapon_4",
    "specialization": "Firebrand",
    "categories": [],
    "recharge": 10,
    "ammo": 0,
    "ammoRecharge": 0,
    "nextChainId": null,
    "flipSkillId": null,
    "tome": "resolve",
    "pageCost": 1
  },
  {
    "id": 42925,
    "name": "Epilogue: Eternal Oasis",
    "description": "Tome. Convert conditions to boons and increase incoming healing.",
    "icon": wikiSkillIcon("Epilogue- Eternal Oasis.png"),
    "type": "Bundle",
    "weapon": "",
    "slot": "Weapon_5",
    "specialization": "Firebrand",
    "categories": [],
    "recharge": 20,
    "ammo": 0,
    "ammoRecharge": 0,
    "nextChainId": null,
    "flipSkillId": null,
    "tome": "resolve",
    "pageCost": 2
  },
  {
    "id": 42986,
    "name": "Chapter 1: Unflinching Charge",
    "description": "Tome. Grant swiftness and protection to nearby allies.",
    "icon": wikiSkillIcon("Chapter 1- Unflinching Charge.png"),
    "type": "Bundle",
    "weapon": "",
    "slot": "Weapon_1",
    "specialization": "Firebrand",
    "categories": [],
    "recharge": 0,
    "ammo": 0,
    "ammoRecharge": 0,
    "nextChainId": null,
    "flipSkillId": null,
    "tome": "courage",
    "pageCost": 1
  },
  {
    "id": 41968,
    "name": "Chapter 2: Daring Challenge",
    "description": "Tome. Taunt nearby enemies and grant resolution to allies.",
    "icon": wikiSkillIcon("Chapter 2- Daring Challenge.png"),
    "type": "Bundle",
    "weapon": "",
    "slot": "Weapon_2",
    "specialization": "Firebrand",
    "categories": [],
    "recharge": 4,
    "ammo": 0,
    "ammoRecharge": 0,
    "nextChainId": null,
    "flipSkillId": null,
    "tome": "courage",
    "pageCost": 1
  },
  {
    "id": 41836,
    "name": "Chapter 3: Valiant Bulwark",
    "description": "Tome. Create a reflective barrier at the target area.",
    "icon": wikiSkillIcon("Chapter 3- Valiant Bulwark.png"),
    "type": "Bundle",
    "weapon": "",
    "slot": "Weapon_3",
    "specialization": "Firebrand",
    "categories": [],
    "recharge": 15,
    "ammo": 0,
    "ammoRecharge": 0,
    "nextChainId": null,
    "flipSkillId": null,
    "tome": "courage",
    "pageCost": 1
  },
  {
    "id": 40988,
    "name": "Chapter 4: Stalwart Stand",
    "description": "Tome. Create a light field that grants resistance and breaks stun.",
    "icon": wikiSkillIcon("Chapter 4- Stalwart Stand.png"),
    "type": "Bundle",
    "weapon": "",
    "slot": "Weapon_4",
    "specialization": "Firebrand",
    "categories": [],
    "recharge": 20,
    "ammo": 0,
    "ammoRecharge": 0,
    "nextChainId": null,
    "flipSkillId": null,
    "tome": "courage",
    "pageCost": 1
  },
  {
    "id": 44455,
    "name": "Epilogue: Unbroken Lines",
    "description": "Tome. Grant protection, stability, aegis, and toughness to allies.",
    "icon": wikiSkillIcon("Epilogue- Unbroken Lines.png"),
    "type": "Bundle",
    "weapon": "",
    "slot": "Weapon_5",
    "specialization": "Firebrand",
    "categories": [],
    "recharge": 25,
    "ammo": 0,
    "ammoRecharge": 0,
    "nextChainId": null,
    "flipSkillId": null,
    "tome": "courage",
    "pageCost": 2
  },
  {
    "id": 76982,
    "name": "Glaring Burst",
    "description": "Create a burst whose effect changes with the equipped radiant weapon.",
    "icon": "https://render.guildwars2.com/file/7E68406FAAC4FFF878CDD307B257C9D6AECE2935/3680152.png",
    "type": "Bundle",
    "weapon": "",
    "slot": "Weapon_1",
    "specialization": "Luminary",
    "categories": [],
    "recharge": 0,
    "ammo": 0,
    "ammoRecharge": 0,
    "nextChainId": null,
    "flipSkillId": null,
    "radiantForgeSkill": true
  },
  {
    "id": 77339,
    "name": "Dazzling Hammer",
    "description": "Smash the ground with a radiant hammer, dazing nearby foes.",
    "icon": "https://render.guildwars2.com/file/4ECA0C916F5D54C3C12AC135AB5A1E2D4BC792E5/3680142.png",
    "type": "Bundle",
    "weapon": "",
    "slot": "Weapon_2",
    "specialization": "Luminary",
    "categories": [],
    "recharge": 7,
    "ammo": 0,
    "ammoRecharge": 0,
    "nextChainId": null,
    "flipSkillId": 76910,
    "radiantForgeSkill": true,
    "radiantWeapon": "hammer"
  },
  {
    "id": 76910,
    "name": "Shining Spin",
    "description": "Strike nearby foes, dealing increased damage to disabled enemies.",
    "icon": "https://render.guildwars2.com/file/2E5878CE9AFA2634D7F6DBAB10389D20C1BFF75E/3680143.png",
    "type": "Bundle",
    "weapon": "",
    "slot": "Weapon_2",
    "specialization": "Luminary",
    "categories": [],
    "recharge": 0,
    "ammo": 0,
    "ammoRecharge": 0,
    "nextChainId": null,
    "flipSkillId": null,
    "radiantForgeSkill": true,
    "radiantWeapon": "hammer"
  },
  {
    "id": 76708,
    "name": "Luminous Staff",
    "description": "Slam a radiant staff into the ground and create a symbol.",
    "icon": "https://render.guildwars2.com/file/06BD9362B2F9FAE403181D0F9BE96A12664C22CA/3680148.png",
    "type": "Bundle",
    "weapon": "",
    "slot": "Weapon_3",
    "specialization": "Luminary",
    "categories": [],
    "recharge": 15,
    "ammo": 0,
    "ammoRecharge": 0,
    "nextChainId": null,
    "flipSkillId": 77136,
    "radiantForgeSkill": true,
    "radiantWeapon": "staff"
  },
  {
    "id": 77136,
    "name": "Restorative Glow",
    "description": "Heal and remove conditions from nearby allies.",
    "icon": "https://render.guildwars2.com/file/9B72587791FE50B415B94BE92AA9050F43F6A848/3680149.png",
    "type": "Bundle",
    "weapon": "",
    "slot": "Weapon_3",
    "specialization": "Luminary",
    "categories": [],
    "recharge": 0,
    "ammo": 0,
    "ammoRecharge": 0,
    "nextChainId": null,
    "flipSkillId": null,
    "radiantForgeSkill": true,
    "radiantWeapon": "staff"
  },
  {
    "id": 76924,
    "name": "Gleaming Blade",
    "description": "Leap to the target and slash, inflicting cripple and vulnerability.",
    "icon": "https://render.guildwars2.com/file/79B0DCC5E4A0A8BBE04AC0024D01A0F36D0B0C79/3680151.png",
    "type": "Bundle",
    "weapon": "",
    "slot": "Weapon_4",
    "specialization": "Luminary",
    "categories": [],
    "recharge": 15,
    "ammo": 0,
    "ammoRecharge": 0,
    "nextChainId": null,
    "flipSkillId": 77366,
    "radiantForgeSkill": true,
    "radiantWeapon": "blade"
  },
  {
    "id": 77366,
    "name": "Lucent Thrust",
    "description": "Stab your foe and launch a blinding ray that bounces between foes.",
    "icon": "https://render.guildwars2.com/file/1967D3BD670E976B0B554AAE3C0E070E0BF672D8/3680150.png",
    "type": "Bundle",
    "weapon": "",
    "slot": "Weapon_4",
    "specialization": "Luminary",
    "categories": [],
    "recharge": 0,
    "ammo": 0,
    "ammoRecharge": 0,
    "nextChainId": null,
    "flipSkillId": null,
    "radiantForgeSkill": true,
    "radiantWeapon": "blade"
  },
  {
    "id": 77197,
    "name": "Radiant Bulwark",
    "description": "Block attacks and grant aegis to nearby allies.",
    "icon": "https://render.guildwars2.com/file/06735858B4DE086359A7DB1D4B2B650F97D7DD03/3680144.png",
    "type": "Bundle",
    "weapon": "",
    "slot": "Weapon_5",
    "specialization": "Luminary",
    "categories": [],
    "recharge": 25,
    "ammo": 0,
    "ammoRecharge": 0,
    "nextChainId": null,
    "flipSkillId": 76978,
    "radiantForgeSkill": true,
    "radiantWeapon": "bulwark"
  },
  {
    "id": 76978,
    "name": "Brilliant Slam",
    "description": "Slam foes with your shield, dazing them.",
    "icon": "https://render.guildwars2.com/file/BAD4D149039913DCF7FA724D4BFE9C9EB17E68D5/3680145.png",
    "type": "Bundle",
    "weapon": "",
    "slot": "Weapon_5",
    "specialization": "Luminary",
    "categories": [],
    "recharge": 0,
    "ammo": 0,
    "ammoRecharge": 0,
    "nextChainId": null,
    "flipSkillId": null,
    "radiantForgeSkill": true,
    "radiantWeapon": "bulwark"
  },
  {
    "id": 30083,
    "name": "Wings of Resolve",
    "description": "Virtue. Dart forward and heal allies near your destination.",
    "icon": "https://render.guildwars2.com/file/A054A5300BF26FBAA601611BDA07B6001830FF92/1012864.png",
    "type": "Profession",
    "weapon": "",
    "slot": "Profession_2",
    "specialization": "Dragonhunter",
    "categories": [
      "Virtue"
    ],
    "recharge": 25,
    "ammo": 0,
    "ammoRecharge": 0,
    "nextChainId": null,
    "flipSkillId": null
  },
  {
    "id": 30029,
    "name": "Shield of Courage",
    "description": "Virtue. Grant aegis and block attacks in front of you.",
    "icon": "https://render.guildwars2.com/file/09DE3F47930AFF2B2B3F4C20A401DCA9DC7344CC/1012865.png",
    "type": "Profession",
    "weapon": "",
    "slot": "Profession_3",
    "specialization": "Dragonhunter",
    "categories": [
      "Virtue"
    ],
    "recharge": 45,
    "ammo": 0,
    "ammoRecharge": 0,
    "nextChainId": null,
    "flipSkillId": null
  }
]);
