...
export function thiefTargetHasCondition(context, condition) {
...
function targetConditionCount(context) {
  ...
  return [...names].filter(condition =>
    thiefTargetHasCondition(context, condition)).length;
}
...
export const thiefCoreModifierRules = Object.freeze([
  {
  ...
  },
  {
    ...
    when: context =>
      thiefPlayerEvent(context)
      && Number(
        thiefRuntimeState(context).distractingThrowBuffUntil || 0,
      ) > context.time,
  },
  {
  ...
  },
  {
    ...
    when: context => {
      ...
      return thiefPlayerEvent(context)
...
function modifyThiefCoreAttributes(context, attributes) {
...
export function compileThiefModifierRules(rules) {
