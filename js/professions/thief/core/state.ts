...
export function createThiefCoreState(config: ThiefConfig = {}): ThiefCoreState {
  ...
  if (
    ...
    maximumHealth +=
      Number(config.stats?.power ?? config.attributes?.power ?? 1000) * 0.7;
  }
...
export const THIEF_PUBLIC_END_STATE_KEYS: readonly (keyof ThiefState)[] =
  Object.freeze([
    "initiative",
    "maximumInitiative",
    "stealthUntil",
...
export function projectThiefEndState({
  ...
  const compatibility = {
    ...state,
    artifactStealthAttacksRemaining:
      state.artifactStealthAttacksRemaining ?? state.stealthAttackCharges ?? 0,
    artifactStealthAttackExpiresAt:
      state.artifactStealthAttackExpiresAt ?? state.stealthAttackExpiresAt ?? 0,
  };
