...
const SCUFFLE_INTERVAL = 3;
const SCUFFLE_DURATION = 15;
const SCOUTREL_LUCK_ICD = 20;
...
function emitBoon(context, at, boon, duration, stacks, name) {
  context.emit({
    type: "buff",
    at,
    source: "thief",
...
function reduceSkrittSwipeRecharge(context, at) {
...
function extendExhilaratingEphemera(state, at) {
...
function applyArtifactIdentity(context, skill, at) {
  ...
  if (skill.id === ID.METAL_LEGION_GUITAR) {
    state.artifactStealthAttacksRemaining = 3;
    state.artifactStealthAttackExpiresAt = at + (meticulous ? 12 : 10);
...
function riskyDoubleEdge(context, skill) {
...
export function peekDoubleEdgeOutcome(context, skill) {
...
function emitCannonBackfire(context, at) {
  context.emit({
    type: "damage",
    at: at + 2,
    source: "thief",
...
function emitCannonSuccess(context) {
