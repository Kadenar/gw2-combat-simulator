...
function stableNameIndex(
  entries: readonly (readonly [string, SkillId])[],
...
export const THIEF_SKILL_IDS: Readonly<Record<string, number>> = Object.freeze({
  SWAP_WEAPONS: -3,
  DODGE: -5,
  ...stableNameIndex([
    ...SKILLS.map(skill => [skill.name, skill.id] as const),
    ...THIEF_SUPPLEMENTAL_SKILLS.map(skill => [skill.name, skill.id] as const),
  ]),
});
export const THIEF_TRAIT_IDS: Readonly<Record<string, number>> = stableNameIndex(
...
export const THIEF_SPECIALIZATION_IDS: Readonly<Record<string, number>> = Object.freeze(
  Object.fromEntries(
