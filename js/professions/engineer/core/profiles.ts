...
export const ENGINEER_CORE_BALANCE_PROFILES: readonly BalanceProfile[] =
  Object.freeze([
    {
    ...
    trait(
      ENGINEER_CORE_BALANCE_PROFILE_IDS.streamlinedKits,
      "Streamlined Kits",
      {
        internalCooldown: 20,
        effects: [
    ...
    trait(
      ENGINEER_CORE_BALANCE_PROFILE_IDS.optimizedActivation,
      "Optimized Activation",
      { effects: [{ type: "boon", boon: "vigor", stacks: 1, duration: 4 }] },
    ),
    trait(
      ENGINEER_CORE_BALANCE_PROFILE_IDS.staticDischarge,
      "Static Discharge",
      {
    ...
    trait(
      ENGINEER_CORE_BALANCE_PROFILE_IDS.explosiveEntrance,
      "Explosive Entrance",
      { effects: [{ type: "strike", coefficient: 1.25, hits: 1 }] },
    ),
    trait(
      ENGINEER_CORE_BALANCE_PROFILE_IDS.steelPackedPowder,
      "Steel-Packed Powder",
      {
        effects: [
          {
    ...
    trait(
      ENGINEER_CORE_BALANCE_PROFILE_IDS.explosiveTemper,
      "Explosive Temper",
      {
        maximumStacks: 10,
        attributePerStack: 20,
        effects: [
          { type: "buff", kind: "explosive-temper", stacks: 1, duration: 10 },
        ],
      },
    ),
    ...
    trait(
      ENGINEER_CORE_BALANCE_PROFILE_IDS.incendiaryPowder,
      "Incendiary Powder",
      {
        internalCooldown: 10,
        durationMultiplier: 0.33,
    ...
    trait(
      ENGINEER_CORE_BALANCE_PROFILE_IDS.aimAssistedRocket,
      "Aim-Assisted Rocket",
      {
        internalCooldown: 3,
        maximumStacks: 5,
    ...
    trait(ENGINEER_CORE_BALANCE_PROFILE_IDS.thermalVision, "Thermal Vision", {
      attributeBonus: 150,
      effects: [
        { type: "buff", kind: "thermal-vision", stacks: 1, duration: 4 },
      ],
    }),
    ...
    trait(
      ENGINEER_CORE_BALANCE_PROFILE_IDS.energyAmplifier,
      "Energy Amplifier",
      {
        attributeBonus: 250,
      },
...
export function engineerBalanceProfile(
  context: unknown,
  id: SkillId,
...
export function engineerBalanceValue(
  context: unknown,
  id: SkillId,
  field: string,
  fallback: number,
...
export function engineerBalanceEffectValue(
  ...
  const effect = engineerBalanceProfileEffect(
    engineerBalanceProfile(context, id),
    type,
    index,
