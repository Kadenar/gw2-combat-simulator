...
function emitSpearEvent(context, skill, at, eventType) {
  context.emit({
    type: eventType,
    at,
    source: "engineer",
...
export function scheduleLightningRod(context, skill) {
...
export function scheduleConduitSurge(context, skill) {
...
export function scheduleElectricArtillery(context, skill) {
  const state = context.state.profession;
  const at = context.effectiveEnd;
...
export function handleLightningRodCharge(context, task) {
  const state = context.state.profession;
...
export function handleElectricArtilleryReady(context, task) {
  const state = context.state.profession;
  if (state.lightningRodActivationId !== task.payload.activationId) return;
...
export function handleElectricArtilleryExpire(context, task) {
  const state = context.state.profession;
  if (state.lightningRodActivationId !== task.payload.activationId) return;
  state.lightningRodActivationId = "";
  state.lightningRodChargeExpiries = [];
...
export function scheduleRoilingSkiesControl(context, skill) {
  const isFocused =
...
export function scheduleDevastatorFollowup(context, skill) {
