...
const MECH_COMMAND_DURATIONS: Readonly<Record<number, number>> = Object.freeze({
...
export function applyEngineerMechCastTraits(
  ...
  if (state.mech.active && isEngineerMechCommand(skill)) {
    const busyUntil =
      at + Number(MECH_COMMAND_DURATIONS[Number(skill.id)] || 0);
...
export function handleEngineerMechAttack(
  ...
  if (hasEngineerTrait(context.config, TRAIT.MECH_ARMS_JADE_CANNONS)) {
    ...
    scheduleMechAttack(
      context,
      task.at + (firstArm ? JADE_CANNON_ARM_GAP : JADE_CANNON_CYCLE_GAP) / rate,
      { phase: firstArm ? 1 : 0 },
    );
    return;
  }
  ...
  scheduleMechAttack(context, task.at + MELEE_CHAIN_INTERVALS[phase] / rate, {
    phase: (phase + 1) % 3,
  });
}
