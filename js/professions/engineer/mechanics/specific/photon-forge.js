...
const BASE_PASSIVE_HEAT_PER_SECOND = 2;
const LIGHT_DENSITY_BONUS_HEAT_PER_SECOND = 1;
const SOLAR_FOCUSING_LENS_DURATION = 4;
const ENHANCED_CAPACITY_HEAT_THRESHOLD = 100;
const PHOTONIC_BLAST_DELAY = 1.56;
const CORONA_QUICKNESS_PULSE_OFFSETS_MS = Object.freeze([
  400,
  760,
  1120,
  1480,
...
const PHOTON_BLITZ_PULSE_OFFSETS_MS = Object.freeze([
  240,
  400,
  480,
  640,
...
function passiveHeatRate(context) {
...
function appendHeatSegment(segments, state, start, end, rate) {
  if (!(end > start)) return;
...
function coolInactiveForge(context, target, segments) {
...
function highHeatInterval(segment) {
  const endHeat =
...
function emitEnhancedCapacityMight(context, at) {
  context.emit({
    type: "buff",
    at,
    source: "Trait",
...
function materializeEnhancedCapacityMight(context, segments) {
  if (
    !hasEngineerTrait(
      context.config,
...
export function grantSolarFocusingLens(context, at, stacks) {
...
function emitOverheatEffects(context, at, photonicBlastingModule) {
  context.emit({
    type: "proc",
    at,
    source: "engineer",
...
export function handlePhotonForgeHeat(context, task) {
  const state = context.state.profession;
  if (
...
export function triggerThermalReleaseValve(context, skill, at) {
  if (
...
export function observeHolosmithScheduledEvent(context, event) {
  if (
