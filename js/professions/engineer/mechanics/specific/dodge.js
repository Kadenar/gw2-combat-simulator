...
import { triggerThermalReleaseValve } from "./photon-forge.js";
...
function reduceMatchingCooldowns(context, predicate, seconds, at) {
  const ids = new Set([
...
export function performEngineerDodge(context, skill) {
  const state = context.state.profession;
  const at = context.start;
  state.endurance = Math.max(
    0,
    Number(state.endurance || 0) - DODGE_ENDURANCE_COST,
  );
  ...
  triggerThermalReleaseValve(context, skill, at);
