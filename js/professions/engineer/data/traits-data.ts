...
import type {
  EngineerSpecializationSelection,
} from "../types.js";
...
export function getActiveTraits(
  specializations: readonly EngineerSpecializationSelection[] = [],
): EngineerApiTrait[] {
  const active: EngineerApiTrait[] = [];
  for (const selection of specializations) {
    ...
    const picks = String(selection.traits || "").split("-").map(Number);
