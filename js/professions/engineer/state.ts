...
export function hasEngineerTrait(
  configOrTraits: EngineerConfig | Set<SkillId>,
  traitId: SkillId,
): boolean {
...
function skillId(name: string): SkillId | null {
  return engineerCatalog.skillsByName.get(name)?.id ?? null;
}

export function selectedMechCommands(
  traits: EngineerConfig | Set<SkillId>,
): SkillId[] {
  const pick = (
    groups: readonly (readonly [SkillId, string])[],
  ): SkillId | null => {
