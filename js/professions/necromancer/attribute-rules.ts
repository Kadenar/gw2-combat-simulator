...
function necromancerEvent(
  context: Gw2ModifierContext,
): NecromancerSimulationEvent | undefined {
  return (context.event || undefined) as
    | NecromancerSimulationEvent
    | undefined;
}

function necromancerRuntimeState(
  context: Gw2ModifierContext,
...
function isShroudSkill(context: Gw2ModifierContext): boolean {
  return Boolean(eventSkill(context)?.shroud);
}

function activeShroud(context: Gw2ModifierContext): string {
...
function activeBlight(context: Gw2ModifierContext): number {
  const event = necromancerEvent(context);
  return Math.max(
    0,
    Number(
...
function anguishSpiritActive(context: Gw2ModifierContext): boolean {
  return Boolean(necromancerRuntimeState(context).activeSpirits?.anguish);
}

function targetChilled(context: Gw2ModifierContext): boolean {
  return (
...
function criticalStrikeTraitFactor(context: Gw2ModifierContext): number {
...
export const necromancerModifierRules: readonly Gw2ModifierRule[] =
  Object.freeze([
  {
...
function modifyNecromancerMaximumAmmo(
...
function modifyNecromancerCastDuration(
