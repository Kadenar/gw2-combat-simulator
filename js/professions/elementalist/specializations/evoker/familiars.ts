...
export function afterCast(
  ...
  if (skill.name === "Ignite") {
    ...
    if (
      ...
      elementalistBalanceValue(context, PROFILE.ignite, "threshold", 15)
    ) {
      state.igniteTier = 0;
    }
...
function grantFamiliarProwess(
  ...
  const baseDuration = elementalistBalanceValue(
    context,
    PROFILE.familiarsProwess,
    "durationMultiplier",
    5,
  );
  ...
  const maximumDuration = elementalistBalanceValue(
    context,
    PROFILE.familiarsProwess,
    "maximumStacks",
    15,
...
export function onCastComplete(
  context: ElementalistCastContext,
  skill: Skill,
): void {
  const state = evokerState.from(context);
  ...
  if (skill.name === "Zap") {
    const zap = elementalistBalanceEffect(
      context,
      PROFILE.familiarUtility,
      "buff",
      "Zap Window",
  ...
  if (skill.name === "Hare's Agility") {
    ...
    const fireMight = elementalistBalanceEffect(
      context,
      PROFILE.familiarUtility,
      "boon",
      "Fox Fire Bonus",
    ...
    emitElementalistBuff(
      ...
      Number(might?.stacks ?? 8) +
        (state.element === "Fire" ? Number(fireMight?.stacks ?? 3) : 0),
  ...
  if (familiarElement && hasTrait(context, "Specialized Elements")) {
    ...
    applyWeaponSkillRechargeMultiplier(
      ...
      elementalistBalanceValue(
        context,
        basic
          ? PROFILE.specializedElementsBasicRecharge
          : PROFILE.specializedElementsEmpoweredRecharge,
        "rechargeMultiplier",
        basic ? 0.9 : 0.67,
      ),
    );
