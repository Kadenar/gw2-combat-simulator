...
export function triggerSpecializedElementEntry(
  ...
  if (element === "Fire") {
    ...
    if (hasTrait(context, "One with Air")) {
      const superspeed = elementalistBalanceEffect(
        context,
        CORE_PROFILE.oneWithAir,
        "boon",
        "Superspeed",
      );
    ...
    if (hasTrait(context, "Fresh Air")) {
      const freshAir = elementalistBalanceEffect(
        context,
        CORE_PROFILE.freshAir,
        "buff",
      );
      emitElementalistBuff(
        context as never,
    ...
    if (Number(core.procReadyAt.latentStamina || 0) <= at + context.epsilon) {
      core.procReadyAt.latentStamina =
        ...
        elementalistBalanceValue(
          context,
          TEMPEST_PROFILE.latentStamina,
          "internalCooldown",
          10,
        );
      const vigor = elementalistBalanceEffect(
        context,
        TEMPEST_PROFILE.latentStamina,
        "boon",
        "Vigor",
