...
const CATALYST_SPHERE_SKILL_IDS = Object.freeze(
  Object.values(ELEMENTALIST_JADE_SPHERE_SKILL_IDS),
);
...
function catalystPaletteAvailability(
  ...
  const energy = Number(
    state.energy ?? build?.initialCatalystEnergy ?? CATALYST_MAXIMUM_ENERGY,
  );
  ...
  return {
    ...
    message: available
      ? ""
      : `Requires ${CATALYST_SPHERE_COST} Energy; currently ${energy}`,
  };
}
