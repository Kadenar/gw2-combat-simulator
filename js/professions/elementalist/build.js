...
export function migrateElementalistBuild(saved) {
  const source = saved?.build && !saved.profession ? saved.build : saved;
...
export function validateElementalistBuild(build) {
  const errors = [];
