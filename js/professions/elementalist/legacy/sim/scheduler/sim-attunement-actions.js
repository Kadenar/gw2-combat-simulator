...
function buildCooldownDisplayMeta(
  state,
  startedAt,
  displayDurationMs,
  { applyAlacrity = true } = {},
...
function pickAttunementCooldownDisplayMeta(
  state,
  attunement,
  nextReadyAt,
  nextMeta,
...
function getNextOffAttunementReadyAt(
  ctx,
  state,
  attunement,
  defaultReadyAt,
  { preservedRemainingMs = null, defaultDurationMs } = {},
) {
  const existingReadyAt = getAttunementCooldownReadyAt(state, attunement);
...
export function handleAttunementSwap(
  ctx,
  sk,
  isConcurrent,
  concurrents,
  { attunements, offAttCd, catalystEnergyMax, swapIcon },
) {
  const state = ctx.S;
  ...
  if (state._hasSpecializedElements) {
    ctx.log({
      t: state.t,
      type: "err",
      msg: "Cannot swap attunement with Specialized Elements",
    });
    return;
  }
  ...
  for (const attunement of attunements) {
    preservedOffCooldowns[attunement] = Math.max(
      0,
      getAttunementCooldownReadyAt(state, attunement) - preAdvanceTime,
    );
  }
  ...
  const rawPrevBaseCd =
    isEvoker && prev === evoEl ? offAttCd : Math.round(sk.recharge * 1000);
  const prevBaseCd = ctx.attunementCooldownMs(rawPrevBaseCd);
  const existingCD = getAttunementCooldownReadyAt(state, prev);
  const nextPrevReadyAt =
    state.t + ctx.alacrityAdjustedCooldown(prevBaseCd, state.t);
  ...
  const defaultDurationMs = ctx.alacrityAdjustedCooldown(
    ctx.attunementCooldownMs(offAttCd),
    state.t,
  );
  for (const other of attunements) {
    ...
    if (
      other === "Air" &&
      state._hasFreshAir &&
      procState.freshAirResetAt >= state.t
    ) {
    ...
    ctx.setAttunementCooldown(
      ...
      pickAttunementCooldownDisplayMeta(
        ...
        preserveShortCooldown
          ? buildCooldownDisplayMeta(state, state.t, preservedRemainingMs, {
              applyAlacrity: false,
            })
          : buildCooldownDisplayMeta(
              state,
              state.t,
              ctx.attunementCooldownMs(offAttCd),
            ),
  ...
  if (combatActive && state._hasElemBalance && target === evokerState.element) {
    const eb = ctx.incrementEvokerElemBalance(state.t, {
      activateEvery: 2,
      durationMs: 5000,
    });
    if (eb.activated) {
...
export function handleWeaverSwap(
  ctx,
  target,
  sk,
  isConcurrent,
  concurrents,
  { attunements, weaverSwapCd },
) {
  const { S } = ctx;
  ...
  if (weaveSelfWasActive) {
    ...
    if (S.weaveSelfVisited.size >= 4) {
      ...
      const wsStack =
        S.allCondStacks &&
        [...S.allCondStacks].reverse().find((s) => s.cond === "Weave Self");
  ...
  const weaveSelfSwapCD = weaveSelfWasActive
    ? 2000
    : ctx.attunementCooldownMs(weaverSwapCd);
  ...
  if (unravelActive) {
    ...
    if (S._hasElemAttunement && target !== prevPrimary)
      ctx.applyElemAttunementBoon(target, S.t);
  ...
  ctx.log({
    t: S.t,
    type: "swap",
    from: `${prevPrimary}/${prevSecondary}`,
    to: toLabel,
  });
  ctx.addStep({
    skill: sk.name,
...
export function handleOverload(ctx, sk, concurrents, { overloadDwell }) {
  const state = ctx.S;
  if (state.eliteSpec !== "Tempest") {
    ctx.log({
      t: state.t,
      type: "err",
      msg: `Overloads require Tempest specialization`,
    });
    return;
  }
  ...
  const baseDwell = state._hasTranscendentTempest
    ...
    : overloadDwell;
  const dwellEffMs = ctx.alacrityAdjustedCooldown(
    baseDwell,
    state.attEnteredAt,
  );
  ...
  if (
    state._hasPyroPuissance &&
    state.att === "Fire" &&
    isCombatActiveAt(state, end)
  ) {
  ...
  if (
    state._hasTranscendentTempest &&
    !state._suppressTranscendentTempestContributionBuff
  ) {
