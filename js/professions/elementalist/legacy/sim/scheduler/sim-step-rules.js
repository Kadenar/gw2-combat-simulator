...
import {
  getHammerOrbLastCast,
  hasUsedHammerOrbSkill,
} from "../mechanics/sim-hammer.js";
...
export function applyPreludeStateRules(ctx) {
  ...
  if (
    S.weaveSelfUntil > 0 &&
    S.t >= S.weaveSelfUntil &&
    S.perfectWeaveUntil <= S.t
  ) {
...
export function validateTailoredVictory(ctx, name) {
  ...
  ctx.log({
    t: S.t,
    type: "err",
    msg: "Tailored Victory requires Perfect Weave to be active",
  });
  return false;
}
...
export function validateConjureWeaponRequirements(ctx, sk, name) {
  ...
  if (ctx.conjureWeapons.has(sk.weapon) && S.conjureEquipped !== sk.weapon) {
    ctx.log({
      t: S.t,
      type: "err",
      msg: `Need ${sk.weapon} equipped for ${name}`,
    });
    return false;
  }
  if (
    ...
    !ctx.conjureWeapons.has(sk.weapon)
  ) {
    ctx.log({
      t: S.t,
      type: "err",
      msg: `Cannot use ${name} while wielding ${S.conjureEquipped}`,
    });
    return false;
  }
  return true;
...
export function validateAttunementAccess(ctx, sk, name, isAACarryover) {
  ...
  ctx.log({
    t: S.t,
    type: "err",
    msg: `Wrong attunement for ${name} (need ${sk.attunement}, in ${inDesc})`,
  });
  return false;
}
...
export function validateEtchingAccess(ctx, name) {
  ...
  if (name === etchChain.lesser) {
    ...
    ctx.log({
      t: S.t,
      type: "err",
      msg: `Etching: need to cast ${etchChain.etching} first`,
    });
    return false;
  }
  ...
  if (name === etchChain.full) {
    ...
    ctx.log({
      t: S.t,
      type: "err",
      msg: `Etching: ${etchChain.full} requires 3 other Spear casts after ${etchChain.etching}`,
    });
    return false;
  }
...
export function applyHammerPreludeRules(ctx, sk, name) {
  ...
  if (isOrbSkill && hasUsedHammerOrbSkill(S, name)) {
    ctx.log({
      t: S.t,
      type: "err",
      msg: `${name}: must cast Grand Finale before using this orb skill again`,
    });
    return false;
  }
  ...
  ctx.log({
    t: S.t,
    type: "err",
    msg: `Grand Finale: need an orb from current attunement (${S.att}${S.att2 ? "/" + S.att2 : ""})`,
  });
  return false;
}
