...
export function beginScheduledCast(S, { skill, start, att, dur }) {
  pushSchedulerLog(S, { t: start, type: "cast", skill, att, dur });
}

export function finishScheduledCast(S, { skill, end, setCastUntil = false }) {
...
export function setSchedulerAttunementCooldown(
  S,
  attunement,
  readyAt,
  meta = undefined,
...
export function setSchedulerChainProgress(
  S,
  chainRoot,
  nextSkill,
  expiryAt = null,
...
export function takeScheduledConjurePickup(S, weapon, time) {
  const pickupIndex = S.conjurePickups.findIndex(
    (p) => p.weapon === weapon && time <= p.expiresAt,
  );
...
export function addScheduledWeaveSelfVisited(S, attunement) {
  ...
  if (!S._disableWeaveSelfBonuses && S.weaveSelfUntil > S.t) {
    if (attunement === "Fire")
      S.wsFireBonusWindows.push({ start: S.t, end: S.weaveSelfUntil });
    if (attunement === "Air")
      S.wsAirBonusWindows.push({ start: S.t, end: S.weaveSelfUntil });
  }
...
export function activateScheduledCatalystSphere(
  S,
  attunement,
  startTime,
  durationMs,
...
export function addScheduledEvokerEmpowered(
  S,
  amount,
