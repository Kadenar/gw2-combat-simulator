...
function applyConjureProgression(ctx, sk, end) {
  ...
  ctx.log({
    t: end,
    type: "conjure",
    weapon: conjureWeapon,
    pickupExpires: end + ctx.conjurePickupDuration,
  });
...
function applyEvokerChargeProgression(ctx, sk, time) {
  ...
  if (
    ...
    ctx.evokerNoChargeSkills.has(sk.name)
  )
    return;
  if (sk.weapon === "Spear") {
...
function armSpearNextSkillProgression(ctx, sk, name, end) {
  ...
  if (name === "Seethe") {
    ...
    ctx.log({
      ...
      type: "skill_proc",
      skill: "Seethe",
      detail: "next Spear +20% strike armed",
    });
  } else if (name === "Ripple") {
    ...
    ctx.log({
      ...
      type: "skill_proc",
      skill: "Ripple",
      detail: "next Spear -33% recharge armed",
    });
    ...
    ctx.log({
      ...
      type: "skill_proc",
      skill: "Energize",
      detail: "next Spear guaranteed crit armed",
    });
  } else if (name === "Harden") {
    ...
    ctx.log({
      ...
      type: "skill_proc",
      skill: "Harden",
      detail: "next Spear first hit CC armed",
    });
  }
}
...
function resetWeaverDualSpearCooldown(ctx, sk, name, end) {
  ...
  if (S.eliteSpec !== "Weaver" || sk.weapon !== "Spear" || sk.slot !== "3")
    return;
  if (!sk.attunement || !sk.attunement.includes("+") || S.att === S.att2)
    return;
  ...
  ctx.log({
    ...
    type: "skill_proc",
    skill: name,
    detail: `${S.att} attunement CD reset`,
  });
}
...
function handleElementalExplosionProgression(ctx, name, end) {
  ...
  ctx.log({
    ...
    type: "skill_proc",
    skill: name,
    detail: "all bullets consumed, aura granted",
  });
}
...
export function applyStandardCastProgression(
  ctx,
  sk,
  name,
  { key, start, end, scaleOff },
) {
