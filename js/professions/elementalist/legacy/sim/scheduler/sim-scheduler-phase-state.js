...
export function createSchedulerPhaseState(runState, { eventQueue = [] } = {}) {
  const phaseState = {
  ...
  for (const key of getSchedulerWorkingStateKeys(runState)) {
    if (
      key === "schedulerTimingWindowState" ||
      key === "schedulerCooldownState" ||
      key === "schedulerCombatState" ||
      key === "schedulerConditionState" ||
      key === "schedulerReportingState" ||
      key === "schedulerIntentState"
    )
      continue;
  ...
  if (!Array.isArray(phaseState.wsFireBonusWindows))
    phaseState.wsFireBonusWindows = [];
  if (!Array.isArray(phaseState.wsAirBonusWindows))
    phaseState.wsAirBonusWindows = [];
