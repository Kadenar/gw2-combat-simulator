...
const FAMILIAR_BASIC_BY_EMPOWERED = Object.freeze(
  Object.fromEntries(
    Object.entries(FAMILIAR_INTERRUPT_WINDOWS).map(([basic, value]) => [
      value.empowered,
      basic,
    ]),
  ),
...
function removeFutureFamiliarFieldRecords(state, skillName, fromTime) {
  ...
  if (Array.isArray(combatState.fields)) {
    combatState.fields = combatState.fields.filter(
      (field) => !(field.skill === skillName && field.start >= fromTime),
    );
  }
  if (Array.isArray(reportingState.log)) {
    reportingState.log = reportingState.log.filter(
      (entry) =>
        !(
          entry.type === "field" &&
...
export function handleJadeSphere(
  ctx,
  sk,
  concurrents,
  { catalystEnergyMax, catalystSphereCost },
) {
  const state = ctx.S;
  ...
  if (state.eliteSpec !== "Catalyst") {
    ctx.log({
      t: state.t,
      type: "err",
      msg: `Jade Sphere requires Catalyst specialization`,
    });
    return;
  }
  if (sk.attunement !== state.att) {
    ctx.log({
      t: state.t,
      type: "err",
      msg: `Need ${sk.attunement} for ${sk.name}`,
    });
    return;
  }
  ...
  if (catalystState.energy < catalystSphereCost) {
    ctx.log({
      t: state.t,
      type: "err",
      msg: `Not enough energy (${catalystState.energy}/${catalystSphereCost})`,
    });
    return;
  }
  ...
  if (sk.recharge > 0) {
    ...
    ctx.setSkillCooldown(
      cdKey,
      state.t + ctx.alacrityAdjustedCooldown(baseCdMs, state.t),
      {
        startedAt: state.t,
  ...
  if (
    ...
    state.att === "Fire" &&
    isCombatActiveAt(state, state.t)
  ) {
...
export function handleFamiliar(
  ctx,
  sk,
  concurrents,
  { evokerElementMap, evokerFamiliarSelectors },
) {
  const state = ctx.S;
  ...
  if (state.eliteSpec !== "Evoker") {
    ctx.log({
      t: state.t,
      type: "err",
      msg: `Familiar skills require Evoker specialization`,
    });
    return;
  }
  ...
  if (evokerState.element !== famElement) {
    ctx.log({
      t: state.t,
      type: "err",
      msg: `Need ${famElement} familiar selected for ${sk.name} (have ${evokerState.element || "none"})`,
    });
    return;
  }
  ...
  if (isBasic) {
    if (evokerState.empowered >= 3) {
      ctx.log({
        t: state.t,
        type: "err",
        msg: `Empowered skill ready — cannot use ${sk.name}`,
      });
      return;
    }
    if (evokerState.charges < chargesNeeded) {
      ctx.log({
        t: state.t,
        type: "err",
        msg: `Need ${chargesNeeded} familiar charges for ${sk.name} (have ${evokerState.charges})`,
      });
      return;
    }
    ...
    ctx.log({
      t: state.t,
      type: "err",
      msg: `Need 3 empowered charges for ${sk.name} (have ${evokerState.empowered})`,
    });
    return;
  }
  ...
  const recentEmpowered = interruptRule
    ? procState.lastEmpoweredFamiliarByBasic?.[sk.name]
    : null;
  const interruptedByRecentEmpowered = !!(
    interruptRule &&
    recentEmpowered &&
  ...
  if (!interruptedByRecentEmpowered && sk.name === "Ignite") {
    ...
    const igniteTier = ctx.consumeEvokerIgniteTier(start, {
      staleAfterMs: 15000,
      maxTier: 3,
    });
  ...
  const fieldSpawnTime =
    sk.name === "Buoyant Deluge"
      ? Math.max(
          end,
          start +
            (ctx.skillHits?.[sk.name]?.find((hit) => (hit.hit || 0) === 1)
              ?.startOffsetMs || 0),
        )
      : end;
  ...
  if (sk.recharge > 0) {
    ...
    ctx.setSkillCooldown(
      sk.name,
      end + ctx.alacrityAdjustedCooldown(baseCdMs, end),
      {
        startedAt: end,
        displayDurationMs: baseCdMs,
        alacrityUntil: state.alacrityUntil || 0,
