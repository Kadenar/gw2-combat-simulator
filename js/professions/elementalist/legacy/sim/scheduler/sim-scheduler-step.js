...
import {
  applyStandardCastProgression,
  armSpearEtchingOnCastStart,
} from "./sim-cast-followups.js";
...
export function executeStandardSkillCast(
  ctx,
  sk,
  name,
  { concurrents, isAACarryover, rotationMeta },
) {
  const { S } = ctx;
  ...
  const hasInterrupt =
    Number.isFinite(configuredInterruptMs) &&
  ...
  if (name !== "Grand Finale")
    ctx.scheduleHits(sk, start, scaleOff, hasInterrupt ? end : null);
...
export function dispatchPreparedStepAction(
  ctx,
  prepared,
  { name, skipCastUntil = false, concurrents = [], rotationMeta = {} },
) {
  const { sk } = prepared;
  switch (prepared.actionType) {
    ...
    case STEP_ACTIONS.STANDARD_CAST: {
      ...
      if (!castCtx.interrupted)
        applyStandardCastProgression(ctx, sk, name, castCtx);
      return;
    }
    default:
      ctx.log({
        t: ctx.S.t,
        type: "err",
        msg: `Unknown scheduled action: ${prepared.actionType || "none"}`,
      });
  }
}
