...
export function prepareStandardSkillAvailability(ctx, sk, key) {
  ...
  if (isCharged) {
    ...
    if (chargeState.count <= 0) {
      ...
      const baseMs = ctx.weaponRechargeMs(
        sk,
    ...
    ctx.adjustChargeCount(key, -1);
    if (
      chargeState.nextChargeAt === Infinity &&
      chargeState.count < sk.maximumCount
    ) {
      const baseMs = ctx.weaponRechargeMs(
        sk,
...
export function prepareStandardHitScheduling(ctx, name) {
  ...
  ctx.setFlag(
    ...
    name === "Frigid Flurry" && S.pistolBullets.Water === true,
  );
  ctx.setFlag(
    "_purblindingCDReduce",
    name === "Purblinding Plasma" && S.pistolBullets.Air === true,
  );
}
...
function resolveStandardSkillCooldown(ctx, sk, name, key, start, end) {
  ...
  if (ctx.arcaneEchoActiveAt(end) && sk.type === "Weapon skill") {
    ...
    const absorbedCdMs = ctx.weaponRechargeMs(
      sk,
    ...
    if (echoSkill) {
      ...
      const newReadyAt =
        echoReadyAt + ctx.alacrityAdjustedCooldown(absorbedCdMs, end);
    ...
    ctx.log({
      ...
      type: "skill_proc",
      skill: "Arcane Echo",
      detail: `${name} CD → 1s (Echo +${(absorbedCdMs / 1000).toFixed(0)}s)`,
    });
  } else {
    ...
    if (
      procState.dazingDischargeUntil > end &&
      sk.weapon === "Pistol" &&
      sk.type === "Weapon skill" &&
      sk.slot !== "1"
    ) {
    ...
    if (name === "Purblinding Plasma" && S._purblindingCDReduce) {
      ...
      ctx.log({
        t: end,
        type: "skill_proc",
        skill: "Purblinding Plasma",
        detail: "Air bullet → CD -33%",
      });
    }
...
function updateStandardSkillChainProgress(
  ctx,
  sk,
  end,
  { interrupted = false } = {},
...
export function finalizeStandardSkillBookkeeping(
  ctx,
  sk,
  name,
  {
  ...
  if (name === "Hurl") {
    ...
    if (rootSkill) {
      ...
      const baseCdMs = ctx.weaponRechargeMs(
        rootSkill,
        Math.round(rootSkill.recharge * 1000),
      );
