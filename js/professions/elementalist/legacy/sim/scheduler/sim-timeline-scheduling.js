...
export function applyPrimordialStance(ctx, att1, att2, time) {
  const stanceEffects = {
    ...
    Earth: () =>
      ctx.applyCondition("Bleeding", 2, 6, time, "Primordial Stance"),
  };
...
export function findFreshAirResetTimeInRange(ctx, fromTime, upTo) {
  ...
  for (const intent of intentState.freshAirHitIntents) {
    ...
    if (
      processFreshAirCandidate(ctx, intent.time, ccPct, {
...
export function scheduleSkillHits(
  ctx,
  sk,
  castStart,
  scaleOff = (off) => off,
  interruptAt = null,
  extraEventProps = null,
) {
  const { engine, S } = ctx;
  ...
  const hitboxCap =
    ctx.hitboxSize === "small"
      ? (SMALL_HITBOX_CAPS.get(sk.name) ?? Infinity)
      : Infinity;

  const hammerOrbElement =
    hammerOrbSkills[sk.name] || (hammerDualOrbSkills[sk.name] ? "Dual" : null);

  const isSpearWeapon =
    sk.weapon === "Spear" && sk.type === "Weapon skill" && sk.slot !== "1";
  ...
  for (const h of rows) {
    ...
    if (raw === "Duration") {
      ...
      if (
        S._hasPersistingFlames &&
        fireFieldSkills.has(sk.name) &&
        h.isFieldTick
      ) {
        effectiveDur += 2;
      }
    ...
    const perHit = durBased
      ? h.damage
      : count > 1
        ? h.damage / count
        : h.damage;
    const effectiveRep =
      rep > 0 ? rep : durBased && count > 1 ? (h.interval || 1) * 1000 : 0;

    const orbPreScheduled =
      hammerOrbElement &&
      effectiveRep > 0 &&
      count === 1 &&
    ...
    for (let i = 0; i < count; i++) {
      const t =
        castStart +
        off +
        (effectiveRep > 0 && count > 1 ? i * effectiveRep : 0);
      ...
      const event = {
        ...
        ...(h.flatDamage > 0
          ? {
              flatStrikeBase:
                h.flatDamage *
                (ctx.glyphBoonedElementals && sk.name === "Glyph of Elementals"
                  ? 1.7
                  : 1),
        ...
        hammerOrbRepeatMs:
          hammerOrbElement && !orbPreScheduled
            ? effectiveRep > 0
              ? effectiveRep
              : (h.interval || 0) > 0
                ? h.interval * 1000
