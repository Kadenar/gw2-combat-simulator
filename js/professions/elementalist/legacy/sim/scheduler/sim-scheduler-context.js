...
export function createSchedulerContext(engine, S, config = {}) {
  ...
  const doJadeSphereHandler =
    config.doJadeSphere || missingHandler("doJadeSphere");
  ...
  return {
    ...
    detectAACarryoverFromAttunement(attunement) {
      return this.withPrimaryAttunement(attunement, () =>
        this.detectAACarryover(),
      );
    },
    ...
    activateCatalystSphere(attunement, startTime, durationMs) {
      return activateScheduledCatalystSphere(
        S,
        attunement,
        startTime,
        durationMs,
