...
export function applyDisabledStatAdjustments(
  engine,
  attributes,
  disabled,
  sigilStatMap,
  ...
  if (disTrait) {
    ...
    if (disTrait === "Burning Rage")
      subStat("Condition Damage", traitObj("Burning Rage")?.conditionDamage);
    if (disTrait === "Aeromancer's Training")
      subStat("Ferocity", traitObj("Aeromancer's Training")?.ferocity);
    if (disTrait === "Zephyr's Speed")
      subStat("Critical Chance", traitObj("Zephyr's Speed")?.criticalChance);
    if (disTrait === "Serrated Stones")
      subStat(
        "Bleeding Duration",
        traitObj("Serrated Stones")?.bleedingDuration,
      );
...
export function createRunState(
  ...
  const S = {
    ...
    pistolBullets: startPistolBullets
      ? {
          Fire: !!startPistolBullets.Fire,
          Water: !!startPistolBullets.Water,
          Air: !!startPistolBullets.Air,
          Earth: !!startPistolBullets.Earth,
        }
...
export function applyPermanentField(S, permaField, permaExpiry) {
  ...
  S.fields.push({
    ...
    skill: "Permanent Field",
    perma: true,
  });
}

export function applyRunSetupState(
  engine,
  S,
  {
    disTrait,
  ...
  if (S._hasEnhancedPotency && evokerState.element === "Fire")
    S._mightCondDmgBonus = 35;
  if (S._hasEnhancedPotency && evokerState.element === "Air")
    S._furyCritBonus = 40;
