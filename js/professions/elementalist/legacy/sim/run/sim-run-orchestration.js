...
export function assertPreparedRunModel(model) {
  ...
  throw new Error(
    "Invalid prepared run model: expected input/state/resolver/cleanup models",
  );
}
...
export function prepareRunContext(
  ...
  const disabledCtx = engine._applyDisabledStatAdjustments(
    attributes,
    disabled,
  );
  const { disSigil, disRelic, disTrait, disFood, statAdj } = disabledCtx;
  ...
  engine._activeProcSigils = (engine.attributes.sigils || []).filter(
    (name) => name !== disSigil && engine._isProcSigil(name),
  );

  const activeRelic =
    disRelic === engine.attributes.relic
      ? null
      : engine.attributes.relic || null;
  ...
  const realStartAtt2 = engine._normalizeSecondaryAttunement(
    eliteSpec,
    startAtt2,
    realStartAtt,
  );
...
export function executeRunPhases(engine, runCtx, { stopAtTime, targetHP }) {
  ...
  exitSetupPhase(S);
  const { deathTime, rotationEndTime: rotEnd } = resolveScheduledStream(
    engine,
    S,
    scheduledStream,
    resolverConfig,
...
export function executeRunPhasesCapture(
  engine,
  runCtx,
  { stopAtTime, targetHP },
) {
...
export function executeRunPhasesWithCache(
  engine,
  runCtx,
  cachedStream,
  { stopAtTime, targetHP },
...
export function buildRunDamageWindow(S, rotEnd, deathTime) {
  ...
  const dpsStart =
    S.firstHitTime ?? (S.hasExplicitCombatStart ? (S.combatStartTime ?? 0) : 0);
...
export function finalizeRunResults(
  engine,
  S,
  rotEnd,
  deathTime,
  ...
  return engine._buildDetailedResults(
    S,
    rotEnd,
    dpsWindowMs,
    effectiveDmg,
