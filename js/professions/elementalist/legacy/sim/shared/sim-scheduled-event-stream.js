...
export function createScheduledEventStreamFromState(
  S,
  rotationEndTime,
  options = {},
) {
...
export function assertScheduledEventStream(stream) {
  if (!isScheduledEventStream(stream)) {
    throw new Error(
      "Invalid scheduled event stream: expected kind/version/events/resolverHandoff/rotationEndTime envelope",
    );
  }
  if (!Number.isFinite(stream.rotationEndTime)) {
    throw new Error(
      "Invalid scheduled event stream: rotationEndTime must be finite",
    );
  }
