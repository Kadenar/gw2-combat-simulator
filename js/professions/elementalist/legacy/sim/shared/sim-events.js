...
export function getQueuedEventValidationError(event) {
  ...
  if (!isFiniteNumber(event.time))
    return `event ${event.type} is missing a finite time`;
  if (event.priority !== undefined && !isFiniteNumber(event.priority)) {
    return `event ${event.type} has a non-numeric priority`;
  }

  if (isHitEvent(event)) {
    if (typeof event.skill !== "string" || event.skill.length === 0)
      return "hit event is missing skill";
    if (!isFiniteNumber(event.hitIdx))
      return `hit event ${event.skill} is missing hitIdx`;
    return null;
  }

  if (isConditionTickEvent(event)) {
    if (typeof event.cond !== "string" || event.cond.length === 0)
      return "condition tick event is missing cond";
    return null;
  }

  if (isApplyEffectEvent(event)) {
    if (typeof event.effect !== "string" || event.effect.length === 0)
      return "apply-effect event is missing effect";
    if (!isFiniteNumber(event.duration))
      return `apply-effect event ${event.effect} is missing duration`;
    return null;
  }

  if (isRelicActivateEvent(event)) {
    if (typeof event.relic !== "string" || event.relic.length === 0)
      return "relic-activate event is missing relic";
    return null;
  }

  if (isRuntimeActionEvent(event)) {
    if (!isPlainObject(event.action))
      return "runtime-action event is missing action payload";
    if (
      typeof event.action.type !== "string" ||
      event.action.type.length === 0
    ) {
...
export function buildRelicActivateEvent({ time, relic, applyEffects = true }) {
  return buildQueuedEvent(EVENT_TYPES.RELIC_ACTIVATE, {
    time,
    relic,
    applyEffects,
  });
...
export function buildRuntimeActionEvent({ time, action, priority = -1 }) {
  return buildQueuedEvent(EVENT_TYPES.RUNTIME_ACTION, {
    time,
    action,
    priority,
  });
