...
export function applyRuntimeAction(ctx, action) {
  ...
  if (action.type === "aura_followup") {
    ...
    if (S._hasElemEpitome)
      ctx.grantElemEmpowerment(1, action.time, action.skill);
    ...
    ctx.log({
      ...
      detail: "next 3 hits apply Bleed (10s)",
    });
    ...
    relicState.nourysActiveUntil = Math.max(
      0,
      relicState.nourysActiveUntil || 0,
    );
    ...
    if (
      action.time > (relicState.thornsReadyAt || 0) &&
