...
export function getAdjustedCastTime(
  S,
  csvCastMs,
  castStart,
...
export function catchUpCharges(ctx, key, sk) {
  ...
  const baseMs = getAdjustedWeaponRechargeMs(
    ctx.engine,
    sk,
  ...
  while (ch.count < sk.maximumCount && ch.nextChargeAt <= S.t) {
    ...
    if (ch.count < sk.maximumCount) {
      setChargeReadyAt(
        S,
        key,
        gainedAt + getAlacrityAdjustedCooldown(S, baseMs, gainedAt),
      );
    } else {
...
export function getAdjustedWeaponRechargeMs(engine, sk, baseMs) {
  if (sk.type === "Weapon skill") {
    if (engine._hasTrait("Pyromancer's Training") && sk.attunement === "Fire")
      baseMs = Math.round(baseMs * 0.8);
    if (engine._hasTrait("Aeromancer's Training") && sk.attunement === "Air")
      baseMs = Math.round(baseMs * 0.8);
    if (engine._hasTrait("Geomancer's Training") && sk.attunement === "Earth")
      baseMs = Math.round(baseMs * 0.8);
    if (engine._hasTrait("Aquamancer's Training") && sk.attunement === "Water")
      baseMs = Math.round(baseMs * 0.8);
    // Flow State: -20% recharge on Dual Attack skills (weaver slot 3, two elements)
    if (
      engine._hasTrait("Flow State") &&
      sk.slot === "3" &&
      sk.attunement &&
      sk.attunement.includes("+")
    ) {
      baseMs = Math.round(baseMs * 0.8);
    }
  }
...
export function getWeaponStrength(engine, skill) {
  ...
  if (weapon === "Profession mechanic")
    return engine.weapons["Profession mechanic"]?.weaponStrength || 1100;
  ...
  if (weapon && engine.weapons[weapon])
    return engine.weapons[weapon].weaponStrength;
