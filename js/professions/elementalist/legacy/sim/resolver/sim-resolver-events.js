import {
  sortQueuedEvents,
  takeNextEvent,
} from "../../../../../platform/engine/event-queue.js";
import {
  isHitEvent,
...
export function buildQueuedEventContext(ctx, time) {
  ...
  const thornsCondDmg =
    S.activeRelic === "Thorns"
      ? (getRelicState(S).thornsStacks || 0) *
        (ctx.getRelicProc("Thorns")?.conditionDamagePerStack || 0)
      : 0;
  let polyCondDmg = 0;
...
export function drainQueuedEvents(
  ctx,
  queue,
  { rotationEndTime, stopAtTime, targetHP },
) {
  const { S } = ctx;
