...
function hasActiveConditionOrPermaBoon(S, cond, time) {
  return (
    getActiveConditionStacks(S, cond, time).length > 0 || !!S.permaBoons?.[cond]
  );
}
...
function buildHitEffectSnapshot(ctx, ev) {
  ...
  return {
    ...
    arcaneLightningActive:
      S._hasArcaneLightning && effectStacks("Arcane Lightning") > 0,
    ...
    persistingFlamesStacks: S._hasPersistingFlames
      ...
      : 0,
    tempestuousAriaUp:
      S._hasTempestuousAria && effectStacks("Tempestuous Aria") > 0,
    transcendentTempestUp:
      S._hasTranscendentTempest &&
      !S._suppressTranscendentTempestContributionBuff &&
      effectStacks("Transcendent Tempest") > 0,
    elementsOfRageUp:
      S._hasElementsOfRage && effectStacks("Elements of Rage") > 0,
    empoweringAurasStacks: S._hasEmpoweringAuras
      ? Math.min(effectStacks("Empowering Auras"), 5)
      : 0,
    familiarsProwessUp:
      S._hasFamiliarsProwess && effectStacks("Familiar's Prowess") > 0,
    relentlessFireUp: effectStacks("Relentless Fire") > 0,
    bountifulPowerUp:
      S._hasBountifulPower && effectStacks("Bountiful Power Active") > 0,
...
function buildPowerAndCritContext(
  ctx,
  ev,
  {
    basePower,
  ...
  if (S._hasPowerOverwhelming && might >= 10)
    powOvr = hitAtt === "Fire" ? 300 : 150;

  let polyPow = 0;
  let polyFer = 0;
  if (S._hasElemPolyphony) {
    const atts =
      hitAtt2 !== null ? new Set([hitAtt, hitAtt2]) : new Set([hitAtt]);
  ...
  const arcaneLightningFer = effectSnapshot.arcaneLightningActive
    ? 150 / 15
    : 0;
  const effectiveCritDmg =
    critDmg +
    ragingFerocity +
    aeroFerocity +
    freshAirFerocity +
  ...
  const mistburnCritBonus =
    S.activeRelic === "Mistburn" &&
...
function buildStrikeAndConditionMultipliers(
  ctx,
  ev,
  {
    sigilMuls,
  ...
  const { S } = ctx;
  const evokerState = getEvokerState(S);
  const critCtx = buildCritMultiplierContext(
    ctx,
    ev,
    tgtHP,
    cc,
  ...
  const fpPct = famProwessUp ? (S._hasFamiliarsFocus ? 0.1 : 0.05) : 0;
  ...
  const nourysStrikeAdd = effectSnapshot.nourysActive
    ? ctx.getRelicProc("Nourys")?.strikeDmgA || 0
    : 0;
  const nourysCondAdd = effectSnapshot.nourysActive
  ...
  const addStrike =
    ...
    (elemRageUp ? 0.15 : 0) +
    empAurasStacks * 0.01 +
    (relentlessFireUp ? 0.1 : 0) +
    (bountifulPowerUp ? 0.2 : 0) +
    (wsAirBonus ? 0.1 : 0) +
    fpStrike +
  ...
  const baseStrike =
    (1 + sigilMuls.strikeAdd + addStrike) * sigilMuls.strikeMul;
  const baseCond = (1 + sigilMuls.condAdd + addCond) * sigilMuls.condMul;

  const targetHasBurning =
    S._hasPyroTraining || S._hasFieryMight
  ...
  const flowLikeWaterMul = S._hasFlowLikeWater ? 1.1 : 1;
  const boltMul =
    S._hasBoltToHeart &&
    tgtHP < Infinity &&
    S.totalStrike + S.totalCond >= tgtHP * 0.5
      ? 1.2
      : 1;

  const zapMul = effectSnapshot.zapBuff ? 1.03 : 1;
  const targetHasVuln = targetSnapshot.targetHasVuln;
  const piercingShardsMul =
    S._hasPiercingShards && targetHasVuln
      ? hitAtt === "Water"
        ? 1.14
        : 1.07
      : 1;
...
function buildProcEvent(ctx, ev) {
  let procEv = ev;
  const resolveActions = Array.isArray(ev.onResolveActions)
    ? ev.onResolveActions
    : [];
  for (const action of resolveActions) {
    ...
    if (action.type === "primordial_stance") {
      procEv =
        procEv === ev ? { ...ev, conds: null } : { ...procEv, conds: null };
    }
  }
...
export function buildHitResolutionContext(
  ctx,
  ev,
  {
    sigilMuls,
