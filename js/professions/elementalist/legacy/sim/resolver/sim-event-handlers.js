...
export function enqueueTriggeredHit(
  ctx,
  queuedHit,
  { logEntry = null, stepEntry = null },
) {
  const { S } = ctx;
...
export function runPreHitRules(ctx, ev) {
  ...
  if (ev.familiarCastId) {
    const canceledBySkill =
      procState.familiarCanceledCastIds?.[ev.familiarCastId];
    if (canceledBySkill) {
  ...
  if (ctx.shouldSkipHammerOrbHit(ev)) {
    pushReportingLog(S, {
      ...
      type: "skip",
      skill: ev.skill,
      reason: "orb consumed by Grand Finale",
    });
    return false;
  }
...
export function handleCritAndCcTriggers(ctx, ev, hitCtx, triggerCtx) {
  ...
  if (triggerCtx.directStrike) {
    ...
    if (S._hasArcanePrecision)
      ctx.checkArcanePrecision(ev.time, hitCtx.cc, hitCtx.hitAtt);
  ...
  if (
    ...
    else if (hitCtx.hitAtt === "Water")
      ctx.trackEffect("Regeneration", 1, 10, ev.time);
    else if (hitCtx.hitAtt === "Air") ctx.trackEffect("Fury", 1, 5, ev.time);
    else if (hitCtx.hitAtt === "Earth")
      ctx.trackEffect("Protection", 1, 4, ev.time);
  }
}
...
export function handleOverloadHitEffects(ctx, ev) {
  ...
  if (S._hasLucidSingularity && ev.skill.startsWith("Overload ")) {
    if (ev.hitIdx >= 1 && ev.hitIdx <= 4)
      ctx.trackEffect("Alacrity", 1, 1, ev.time);
...
export function handlePostHitConditions(ctx, ev, triggerCtx) {
  ...
  if (
    procState.shatteringStoneHits > 0 &&
    ev.time <= procState.shatteringStoneUntil &&
    ...
    pushReportingLog(S, {
      ...
      detail: `Bleed proc (${procState.shatteringStoneHits} left)`,
    });
  }
...
export function handleHitEvent(
  ctx,
  ev,
  { tgtHP, might, empMul, condDmg, vulnMul },
) {
