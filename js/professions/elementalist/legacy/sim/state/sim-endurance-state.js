...
function getRelevantVigorBoundaries(S, fromTime, toTime) {
  ...
  for (const stack of stacks) {
    ...
    if (stack.expiresAt > fromTime && stack.expiresAt < toTime)
      boundaries.add(stack.expiresAt);
  }
...
function getNextVigorBoundaryAfter(S, time) {
  ...
  for (const stack of stacks) {
    ...
    if (stack.expiresAt > time && stack.expiresAt < nextBoundary)
      nextBoundary = stack.expiresAt;
  }
  return nextBoundary;
}
...
export function catchUpEndurance(S, time) {
  ...
  if (!Number.isFinite(time) || time <= S.enduranceUpdatedAt)
    return S.endurance;
  ...
  for (const boundary of boundaries) {
    ...
    endurance = clampEndurance(
      endurance + ((boundary - cursor) / 1000) * ratePerSecond,
    );
    cursor = boundary;
