...
export function capSkillCooldownReadyAt(S, key, readyAt) {
  return setSkillCooldownReadyAt(
    S,
    key,
    Math.min(getSkillCooldownReadyAt(S, key, readyAt), readyAt),
  );
}
...
export function setAttunementCooldownReadyAt(
  S,
  attunement,
  readyAt,
  meta = undefined,
...
export function capAttunementCooldownReadyAt(S, attunement, readyAt) {
  return setAttunementCooldownReadyAt(
    S,
    attunement,
    Math.min(getAttunementCooldownReadyAt(S, attunement, readyAt), readyAt),
  );
}
...
export function reduceSkillCooldownRemaining(S, key, time, pct) {
  ...
  return setSkillCooldownReadyAt(
    S,
    key,
    time + Math.round(remaining * (1 - pct)),
  );
}
