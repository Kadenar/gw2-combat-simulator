...
export function getActiveTimedStacks(
  S,
  cond,
  time,
  { includePerma = true, predicate = null } = {},
) {
...
export function getActiveConditionStacks(S, cond, time) {
  ...
  if (!cs) return [];
  return cs.stacks.filter(
    (stack) => stack.t <= time && stack.expiresAt >= time,
  );
}
...
export function activateConditionTicks(
  S,
  cond,
  time,
...
export function scheduleNextConditionTick(
  S,
  cond,
  time,
  { queueConditionTick = null } = {},
