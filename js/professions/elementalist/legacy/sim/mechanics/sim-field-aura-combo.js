...
import {
  buildAuraFollowupAction,
  queueAuraFollowupAction,
} from "../shared/sim-deferred-runtime-actions.js";
import {
  grantElementalEmpowerment,
  trackEffect,
  refreshEffect,
} from "./sim-effect-state.js";
import {
  applyOnAuraGainEffects,
  createAuraEffectsContext,
  grantEmpoweringAuras,
} from "./sim-aura-effects.js";
import { pushTimedStack } from "../state/sim-runtime-state.js";
import {
  pushCombatField,
  pushCombatAura,
} from "../state/sim-combat-record-state.js";
...
export function trackField(
  ...
  sk,
  castEnd,
  { fireFieldSkills, log = (entry) => pushReportingLog(S, entry) },
) {
...
export function applyAura(engine, S, auraName, durMs, time, skill, opts = {}) {
  ...
  const log =
    typeof opts.log === "function"
      ? opts.log
      : (entry) => pushReportingLog(S, entry);
  const pushCondStack =
    typeof opts.pushCondStack === "function"
      ? opts.pushCondStack
      : (entry) => pushTimedStack(S, entry);
  ...
  const followupMode =
    opts.followupMode || (isSetupPhase(S) ? "queued" : "immediate");
    ...
    refreshEffect: (effectName, durationSec, at) => {
      ...
      if (opts.boons && opts.relicProcs) {
        return refreshEffect(
          {
            S,
            trackEffect: (effect, stacks, duration, timePoint) =>
              trackEffect(engine, S, effect, stacks, duration, timePoint, {
                boons: opts.boons,
                relicProcs: opts.relicProcs,
                log,
                pushCondStack,
  ...
  if (followupMode === "queued") {
    ...
    queueAuraFollowupAction(
      runtimeActionTarget,
      buildAuraFollowupAction({ time, skill }),
    );
  }
}
...
export function trackAura(engine, S, sk, castEnd, opts = {}) {
  ...
  if (aDur > 0)
    applyAura(engine, S, `${aType} Aura`, aDur, castEnd, sk.name, opts);
}
