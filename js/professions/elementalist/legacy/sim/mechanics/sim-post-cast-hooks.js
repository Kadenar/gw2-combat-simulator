...
export function applyGenericPostCastHooks(ctx, sk, { key, end }) {
  ...
  if (S._hasAltruisticAspect && sk.type === "Meditation") {
    ...
    else if (sk.name === "Toad's Fortitude")
      ctx.trackEffect("Stability", 1, 5, end);
    else if (sk.name === "Elemental Procession")
      ctx.trackEffect("Resistance", 1, 5, end);
  }

  if (
    S._hasEarthsEmbrace &&
    sk.type === "Healing skill" &&
  ...
  if (sk.type === "Signet") {
    if (S._hasWrittenInStone) {
      if (sk.name === "Signet of Restoration")
        ctx.applyAura("Frost Aura", 4000, end, "Written in Stone");
      else if (sk.name === "Signet of Fire")
        ctx.applyAura("Fire Aura", 4000, end, "Written in Stone");
      else if (sk.name === "Signet of Earth")
        ctx.applyAura("Magnetic Aura", 3000, end, "Written in Stone");
    }
  ...
  // Fervent Stance: while the stance is active, each Dual Attack grants Might.
  // The window lasts for the skill's own duration (8s in Skills_data).
  if (sk.name === "Fervent Stance") {
    S.ferventStanceUntil = end + (sk.duration || 8) * 1000;
  }

  if (S._hasArcaneLightning && sk.type === "Arcane") {
    ctx.refreshArcaneLightningBuff(end);
    if (sk.name === "Arcane Brilliance")
      ctx.trackEffect("Protection", 1, 3.5, end);
    else if (sk.name === "Arcane Wave") {
      ctx.trackEffect("Immobilize", 1, 2, end);
      ctx.triggerShackles(end, sk.name);
    } else if (sk.name === "Arcane Blast")
      ctx.trackEffect("Blindness", 1, 5, end);
    else if (sk.name === "Arcane Echo") ctx.trackEffect("Quickness", 1, 4, end);
  }

  const isDual =
    sk.slot === "3" && sk.attunement && sk.attunement.includes("+");
  if (isDual) {
    ...
    // Fervent Stance active: Dual Attacks grant 3 stacks of Might (8s).
    if (S.ferventStanceUntil && end <= S.ferventStanceUntil) {
      ctx.trackEffect("Might", 3, 8, end);
    }
