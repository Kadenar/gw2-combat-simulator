...
function consumeBullet(ctx, element, end, skillName) {
  ...
  ctx.log({
    ...
    type: "skill_proc",
    skill: skillName,
    detail: `${element} bullet consumed`,
  });
}
...
function grantBullet(ctx, element, end, skillName, { permaExpiry }) {
  ...
  ctx.log({
    ...
    type: "skill_proc",
    skill: skillName,
    detail: `${element} bullet granted`,
  });
}

export function applyPistolDualConsumeEffect(
  ctx,
  sk,
  name,
  element,
  ...
  if (canConsume && hasIt) {
    ...
    if (name === "Raging Ricochet") {
      ...
      ctx.log({
        ...
        type: "skill_proc",
        skill: "Frozen Fusillade",
        detail: `Ice explosion queued at t=${hitTime}ms (5×Bleed)`,
      });
      ...
      ctx.log({
        ...
        detail: "next Pistol CD -33% armed (5s)",
      });
