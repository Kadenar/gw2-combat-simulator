...
export function triggerFlameExpulsion(ctx, time) {
  ...
  if (shouldBlockPrecombatProc(S, time)) return;
  if (S.eliteSpec === "Evoker" && !isTraitIcdReady(S, "FlameExpulsion", time))
    return;
...
export function triggerEarthenBlast(ctx, time) {
  ...
  if (shouldBlockPrecombatProc(S, time)) return;
  if (S.eliteSpec === "Evoker" && !isTraitIcdReady(S, "EarthenBlast", time))
    return;
...
export function grantRockSolid(ctx, time) {
  const { S } = ctx;
  if (S.eliteSpec === "Evoker" && !isTraitIcdReady(S, "RockSolid", time))
    return;
