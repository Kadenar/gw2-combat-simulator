...
function activateTimedRelicBuff(S, proc, time) {
  ...
  relicState.buffUntil = Math.max(
    relicState.buffUntil,
    time + proc.effectDuration,
  );
  return !wasActive;
}
...
export function triggerShackles(ctx, time, sourceSkill = "Immobilize") {
  ...
  if (relic !== "Shackles" || proc?.trigger !== "immobilize_tether")
    return false;
...
export function getRelicStrikeMultiplier(engine, S, ev, tgtHP) {
  ...
  if (proc.trigger === "blast_combo") {
    return relicState.bloodstoneExplosionUntil > ev.time
      ? 1 + proc.strikeDmgM
      : 1;
  }
  ...
  if (proc.trigger === "weapon_recharge_hit") {
    return relicState.thiefStacks > 0 && relicState.thiefUntil > ev.time
      ? 1 + relicState.thiefStacks * proc.stackDmgPer
      : 1;
  }
  if (proc.trigger === "heal_skill") {
    return effectStacksAt(S, "Fire Aura", ev.time) > 0
      ? 1 + proc.strikeDmgM
      : 1;
  }
  return proc.strikeDmgM > 0 && relicState.buffUntil > ev.time
    ? 1 + proc.strikeDmgM
    : 1;
}
  ...
  switch (proc.trigger) {
    ...
    case "cc_any":
      if (ev.cc) {
        ...
        if (proc.effectDuration > 0) {
          ...
          const skill =
            relic === "Claw" ? "Relic of the Claw" : `Relic of ${relic}`;
          logRelicProcCtx(
            ctx,
            relic,
            ev.time,
    ...
    case "weapon_recharge20":
      if (ev.dmg > 0 && ev.ws > 0) {
        ...
        if (sk) {
          const isWeapon =
            sk.type === "Weapon skill" && !conjureWeapons.has(sk.weapon);
          const isOverload = sk.name.startsWith("Overload");
          if (
            (isWeapon || isOverload) &&
            sk.recharge >= 20 &&
            activateTimedRelicBuff(S, proc, ev.time)
          ) {
            logRelicProcCtx(ctx, relic, ev.time, proc.icon);
          }
        }
    ...
    case "apply_weakness_vuln":
      if (ev.conds && isRelicIcdReady(S, relic, ev.time)) {
        const hasWV = Object.keys(ev.conds).some(
          (key) =>
            (key === "Weakness" || key === "Vulnerability") &&
            ev.conds[key]?.stacks > 0 &&
            ev.conds[key]?.duration > 0,
        );
        if (hasWV) {
          ...
          if (trigKey !== relicState.aristocracyLastTrigger) {
            ...
            armRelicIcd(S, relic, ev.time, proc.icd);
            if (relicState.aristocracyUntil <= ev.time)
              relicState.aristocracyStacks = 0;
            relicState.aristocracyStacks = Math.min(
              relicState.aristocracyStacks + 1,
              proc.maxStacks,
            );
    ...
    case "gain_protection_resolution":
      if (ev.conds && isRelicIcdReady(S, relic, ev.time)) {
        const hasPR = Object.keys(ev.conds).some(
          (key) =>
            (key === "Protection" || key === "Resolution") &&
            ev.conds[key]?.stacks > 0 &&
            ev.conds[key]?.duration > 0,
        );
        if (hasPR) {
    ...
    case "weapon_recharge_hit":
      if (ev.dmg > 0 && ev.ws > 0) {
        ...
        if (sk && sk.type === "Weapon skill" && sk.recharge > 0) {
          ...
          relicState.thiefStacks = Math.min(
            relicState.thiefStacks + 1,
            proc.maxStacks,
          );
...
export function checkRelicOnCast(
  engine,
  S,
  sk,
  start,
...
export function trackBlightbringerPoison(
  S,
  time,
  skillName,
  castStart,
  { damagingConditions, applyCondition, trackEffect },
) {
  const relicState = getRelicState(S);
  const key =
    castStart != null ? `${skillName}_${castStart}` : `${skillName}_${time}`;
  ...
  if (
    relicState.blightbringerCount >= 6 &&
    isRelicIcdReady(S, "Blightbringer", time)
  ) {
...
export function checkBloodstoneBlast(ctx, time) {
  ...
  if (volatility) {
    for (const stack of volatility) {
      if (stack.t <= time && stack.expiresAt > time)
        stack.expiresAt = time + proc.volatilityDuration;
    }
  }
  pushTimedStack(S, {
    t: time,
    cond: "Bloodstone Volatility",
    expiresAt: time + proc.volatilityDuration,
  });
  ctx.log({
    t: time,
    type: "skill_proc",
  ...
  if (volatilityStacks >= proc.stacksNeeded) {
    ...
    const refreshedVolatility = getActiveTimedStacks(
      S,
    ...
    pushTimedStack(S, {
      t: time,
      cond: "Bloodstone Fervor",
      expiresAt: time + proc.effectDuration,
    });
