...
export function grantEmpoweringAuras(ctx, time) {
  ...
  const active = auraStacks
    ? auraStacks.filter(
        (stack) => stack.t <= time && stack.expiresAt > time && !stack.perma,
      )
    : [];
...
export function applyOnAuraGainEffects(ctx, time) {
  ...
  if (S._hasTempestuousAria) {
    const existing = findActiveTimedStack(S, "Tempestuous Aria", time, {
      includePerma: false,
    });
    if (existing) {
