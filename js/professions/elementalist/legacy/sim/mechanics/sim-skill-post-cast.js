...
export function applySkillSpecificPostCast(ctx, sk, { start, end, scaleOff }) {
  ...
  if (sk.name === "Elemental Celerity") {
    ...
    ctx.log({
      t: end,
      type: "skill_proc",
      skill: "Elemental Celerity",
      detail: `${S.att} CDs reset`,
    });

    if (catalystState.sphereExpiry.Fire > end)
      ctx.trackEffect("Might", 5, 6, end);
    if (catalystState.sphereExpiry.Water > end)
      ctx.trackEffect("Vigor", 1, 6, end);
    if (catalystState.sphereExpiry.Air > end)
      ctx.trackEffect("Fury", 1, 6, end);
    if (catalystState.sphereExpiry.Earth > end)
      ctx.trackEffect("Protection", 1, 4, end);
  }
  ...
  if (sk.name === "Hare's Agility") {
    ...
    ctx.log({
      t: end,
      type: "skill_proc",
      skill: "Hare's Agility",
      detail: "+5 electric enchantment",
    });
    ctx.addStep({
  ...
  if (sk.name === "Fox's Fury") {
    ...
    ctx.queueHitEvent({
      ...
      conds: {
        Burning: {
          stacks: foxBurnStacks[foxTier],
          duration: foxBurnDurs[foxTier],
        },
      },
      att: S.att,
      att2: S.att2,
    ...
    ctx.log({
      ...
      type: "skill_proc",
      skill: "Fox's Fury",
      detail: `tier ${foxTier}, ${foxMightCount} Might`,
    });
  }
  ...
  if (sk.name === "Elemental Procession") {
    ...
    ctx.log({
      t: end,
      type: "skill_proc",
      skill: "Elemental Procession",
      detail: "empowered familiars released",
    });
  }
  ...
  if (sk.name === "Weave Self") {
    ...
    ctx.log({
      ...
      type: "skill_proc",
      skill: "Weave Self",
      detail: `armed, starting in ${S.att}`,
    });
  }
  ...
  if (sk.name === "Unravel") {
    ...
    ctx.log({
      ...
      type: "skill_proc",
      skill: "Unravel",
      detail: `${durMs / 1000}s single-attunement mode`,
    });
  }
  ...
  if (sk.name === "Tailored Victory") {
    ...
    ctx.log({
      t: end,
      type: "skill_proc",
      skill: "Tailored Victory",
      detail: "Perfect Weave consumed",
    });
  }
}
