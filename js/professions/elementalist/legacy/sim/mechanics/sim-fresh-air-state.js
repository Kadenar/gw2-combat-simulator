...
export function isFreshAirEligibleHitEvent(hitEvent) {
  ...
  if (hitEvent.isSigilProc || hitEvent.isRelicProc || hitEvent.isTraitProc)
    return false;
...
export function buildFreshAirHitCandidate(hitEvent) {
  ...
  if (hitEvent._queueSeq !== undefined)
    candidate._queueSeq = hitEvent._queueSeq;
  return candidate;
}
...
export function applyFreshAirReset(
  S,
  time,
  { log = null, detail = "Air attunement recharged" } = {},
) {
  const procState = getProcState(S);
...
export function processFreshAirCandidate(
  ctx,
  candidateTime,
  critChancePct,
