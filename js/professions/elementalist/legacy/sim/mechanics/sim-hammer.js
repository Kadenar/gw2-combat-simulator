...
function grantOrbs(ctx, granted, skillName, end, durationMs, buffKeys) {
  ...
  for (const element of granted) {
    ...
    const existingDamageWindow = findActiveHammerOrbDamageWindow(
      S,
      element,
      end,
    );
    if (existingDamageWindow) existingDamageWindow.end = end;
    getHammerOrbDamageWindows(S, element).push({
      start: end,
      end: end + durationMs,
    });
  ...
  for (let i = 0; i < consumed.length; i++) {
    ...
    ctx.queueHitEvent({
      ...
      conds: condData
        ? {
            [condData.cond]: {
              stacks: condData.stacks,
              duration: condData.dur,
            },
          }
        : null,
...
export function handleHammerPostCast(ctx, sk, name, start, end) {
  ...
  if (singleEl || dualEls) {
    ...
    grantOrbs(
      ...
      ctx.hammerOrbDurationMs,
      ctx.hammerOrbBuffKey,
    );
