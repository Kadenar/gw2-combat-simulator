...
import { applyCondition } from "./sim/resolver/sim-condition-resolution.js";
import { refreshArcaneLightningBuff } from "./sim/mechanics/sim-crit-sigil-helpers.js";
import { getProcState } from "./sim/state/sim-proc-state.js";
import {
  getCatalystState,
  getEvokerState,
} from "./sim/state/sim-specialization-state.js";
...
function computeFreshAirEffectContribution(results) {
  ...
  for (const entry of results.log || []) {
    ...
    if (!(entry.coeff > 0) || !(entry.diag.ws > 0) || !(entry.diag.power > 0))
      continue;

    const withoutCritDmg = entry.diag.critDmg - FRESH_AIR_FEROCITY_CRIT_DAMAGE;
    const withoutCritMult =
      entry.skill === "Electric Discharge"
    ...
    total +=
      rawStrike *
      (withCritMult - withoutCritMult) *
      (entry.diag.strikeMul || 0);
  }
...
function computeTranscendentTempestEffectContribution(results) {
  const windows = buildEffectWindows(
    results.allCondStacks,
  ...
  for (const entry of results.log || []) {
    if (
      entry.type === "hit" &&
      entry.diag &&
      isEffectActiveAt(windows, entry.t)
    ) {
      if (!(entry.coeff > 0) || !(entry.diag.ws > 0) || !(entry.diag.power > 0))
        continue;
      ...
      const baseStrikeWithout =
        (1 + sigilStrikeAdd + addStrikeWithout) * sigilStrikeMul;
      ...
      total +=
        rawStrike * (entry.diag.critMul || 0) * (strikeMul - strikeMulWithout);
      continue;
    }

    if (
      entry.type === "cond_tick" &&
      entry.diag &&
      isEffectActiveAt(windows, entry.t)
    ) {
      ...
      const otherAdd =
        (entry.diag.hammerFireOrb || 0) +
        (entry.diag.tempAria || 0) +
        (entry.diag.elemRage || 0) +
        (entry.diag.empAuras || 0) +
        (entry.diag.famProwess || 0);
      const condMulWithout =
        (1 + sigilCondAdd + otherAdd) *
        sigilCondMul *
        (entry.diag.vulnMul || 1);
      total +=
        (entry.diag.baseTick || 0) *
        (entry.stacks || 0) *
        ((entry.diag.condMul || 0) - condMulWithout);
    }
  }
...
export const RELIC_PROCS = {
  Akeem: {
    trigger: "cc_5torment_confusion",
    icd: 10000,
    strikeDmgM: 0,
    effectDuration: 0,
    conditions: {
      Confusion: { stacks: 2, dur: 10 },
      Torment: { stacks: 2, dur: 10 },
    },
    icon: "https://render.guildwars2.com/file/594C437E9606A167F4F372BCEB0C2B7C7828037B/3122330.png",
  },
  Fireworks: {
    trigger: "weapon_recharge20",
    icd: 0,
    strikeDmgM: 0.07,
    effectDuration: 6000,
    conditions: null,
  ...
  Blightbringer: {
    ...
    poisonCountNeeded: 6,
    conditions: {
      Poisoned: { stacks: 3, dur: 10 },
      Cripple: { stacks: 1, dur: 5 },
      Weakness: { stacks: 1, dur: 5 },
    },
    icon: "https://render.guildwars2.com/file/286C60AC6FA239B0070293039091A44476A35E90/3375219.png",
  },
  Brawler: {
    trigger: "gain_protection_resolution",
    icd: 8000,
    strikeDmgM: 0.1,
    effectDuration: 4000,
    conditions: null,
  ...
  Fractal: {
    trigger: "bleed_6stacks",
    icd: 20000,
    strikeDmgM: 0,
    effectDuration: 0,
    conditions: {
      Burning: { stacks: 2, dur: 8 },
      Torment: { stacks: 3, dur: 8 },
    },
  ...
  Krait: {
    ...
    strikeWs: 690.5,
    conditions: {
      Bleeding: { stacks: 1, dur: 8 },
      Poisoned: { stacks: 1, dur: 8 },
      Torment: { stacks: 1, dur: 8 },
    },
    icon: "https://render.guildwars2.com/file/645EFCBFFBB7B1C6630CBB7C0FB268CA27B703AC/3122355.png",
  },
  Thief: {
    trigger: "weapon_recharge_hit",
    icd: 0,
    strikeDmgM: 0,
    effectDuration: 6000,
    stackDmgPer: 0.01,
  ...
  Steamshrieker: {
    trigger: "water_blast_leap_combo",
    icd: 0,
  ...
  Thorns: {
    trigger: "enemy_hit",
    icd: 5000,
    conditionDamagePerStack: 30,
    maxStacks: 10,
    bossAuraHitInterval: 3000,
    defaultHitInterval: 5000,
    conditions: null,
...
const SWAP_ICON =
  "https://render.guildwars2.com/file/F0C7F54A6FC70D079E1628FFE871980CAEBFD70D/1012290.png";
...
const EVOKER_FAMILIAR_SELECTORS = new Set([
  "Ignite",
  "Splash",
  "Zap",
  "Calcify",
...
const SPEAR_NEXT_BUFF_SKILLS = new Set([
  "Seethe",
  "Ripple",
  "Energize",
  "Harden",
...
export const LARGE_HITBOX_DURATION_BONUS = new Map([["Wildfire", 2]]);
...
export class SimulationEngine {
  ...
  _hasTrait(name) {
    return this.activeTraitNames.has(name);
  }

  addSkill(name) {
    this.rotation.push(name);
  }
  removeSkill(idx) {
  ...
  _skillInContext(name, S) {
    ...
    if (S.conjureEquipped) {
      return (
        matches.find((s) => s.weapon === S.conjureEquipped) ||
  ...
  run(
    ...
    try {
      ...
      this.results = this._finalizeRunResults(
        S,
        rotEnd,
        deathTime,
        targetHP,
  ...
  runCaptureSchedule(
    ...
    try {
      const { S, rotEnd, deathTime, capturedStream } = executeRunPhasesCapture(
        this,
        runCtx,
        {
          stopAtTime: null,
  ...
  runWithCachedSchedule(
    ...
    try {
      const { S, rotEnd, deathTime } = executeRunPhasesWithCache(
        this,
        runCtx,
        stream,
        {
  ...
  _finalizeRunResults(
    ...
    deathTime,
    targetHP,
    { effectiveDmg, dpsWindowMs },
  ) {
    return finalizeRunResults(this, S, rotEnd, deathTime, targetHP, {
      effectiveDmg,
      dpsWindowMs,
    });
  }
  ...
  _applyRunSetupState(
    S,
    { disTrait, disFood, permaBoons, eliteSpec, a, realStartAtt2 },
  ) {
  ...
  _buildPerSkillSummary(S) {
    const perSkill = Object.fromEntries(
      Object.entries(S.perSkill || {}).map(([name, stats]) => [
        name,
        { ...stats },
      ]),
    );
    ...
    const excludedStepTypes = new Set([
      "swap",
      "wait",
      "drop",
      "pickup",
    ...
    for (const step of S.steps || []) {
      if (
        ...
        excludedStepTypes.has(step.type)
      )
        continue;
      stepCounts[step.skill] = (stepCounts[step.skill] || 0) + 1;
      stepCastMs[step.skill] =
        (stepCastMs[step.skill] || 0) +
        Math.max(0, (step.end || 0) - (step.start || 0));
    }

    const procLogCounts = {};
    const procKeys = new Set();
    const procLogTypes = new Set([
      "trait_proc",
      "relic_proc",
      "sigil_proc",
      "jade_sphere",
    ...
    for (const ev of S.log || []) {
      ...
      if (fallbackEventTypes.has(ev.type)) {
        ...
        if (!fallbackKeys.has(key)) {
          ...
          fallbackEventCounts[ev.skill] =
            (fallbackEventCounts[ev.skill] || 0) + 1;
        }
      }
    }
    ...
    for (const [name, entry] of Object.entries(perSkill)) {
      const backfilledCasts =
        stepCounts[name] ||
        procLogCounts[name] ||
        fallbackEventCounts[name] ||
        0;
      if (backfilledCasts > entry.casts) entry.casts = backfilledCasts;

      const backfilledCastMs = stepCastMs[name] || 0;
      if (backfilledCastMs > entry.castTimeMs)
        entry.castTimeMs = backfilledCastMs;
  ...
  _buildEndStateSnapshot(S, rotEnd) {
    ...
    return {
      ...
      skillCD: { ...S.skillCD },
      skillCDMeta: S.skillCDMeta
        ? JSON.parse(JSON.stringify(S.skillCDMeta))
        : {},
      traitICD: { ...S.traitICD },
      ...
      conjurePickups: S.conjurePickups
        .filter((p) => p.expiresAt > rotEnd)
        .map((p) => ({ ...p })),
      eliteSpec: S.eliteSpec,
      energy: catalystState.energy,
      sphereActiveUntil: catalystState.sphereActiveUntil,
      sphereWindows: catalystState.sphereWindows
        .filter((w) => w.end > rotEnd)
        .map((w) => ({ ...w })),
      ...
      evokerFamiliarLockouts: JSON.parse(
        JSON.stringify(getProcState(S).lastEmpoweredFamiliarByBasic || {}),
      ),
  ...
  _scheduleRotation(S) {
    S.hasExplicitCombatStart = this.rotation.some(
      (item) =>
    ...
    const scheduledStream = createScheduledEventStreamFromState(
      schedulerPhaseState,
      rotationEndTime,
      {
  ...
  computeContributions(
    ...
    if (
      permaBoons.Might ||
      fullResults.allCondStacks.some((s) => s.cond === "Might")
    )
      modifiers.push({ id: "Might", name: "Might" });
    if (
      permaBoons.Fury ||
      fullResults.allCondStacks.some((s) => s.cond === "Fury")
    )
      modifiers.push({ id: "Fury", name: "Fury" });
    if (
      permaBoons.Vulnerability ||
      fullResults.allCondStacks.some((s) => s.cond === "Vulnerability")
    )
    ...
    if (foodEntry?.proc)
      modifiers.push({
        id: `Food:${foodEntry.proc.name}`,
        name: `Food: ${foodEntry.proc.name}`,
      });

    // ── Trait contributions ──
    // Stat-based (flat bonus subtracted from attributes before running)
    if (ht("Burning Rage"))
      modifiers.push({ id: "Trait:Burning Rage", name: "Burning Rage" });
    if (ht("Zephyr's Speed"))
      modifiers.push({ id: "Trait:Zephyr's Speed", name: "Zephyr's Speed" });
    if (ht("Gathered Focus"))
      modifiers.push({ id: "Trait:Gathered Focus", name: "Gathered Focus" });
    if (ht("Elemental Enchantment"))
      modifiers.push({
        id: "Trait:Elemental Enchantment",
        name: "Elemental Enchantment",
      });
    if (ht("Serrated Stones"))
      modifiers.push({ id: "Trait:Serrated Stones", name: "Serrated Stones" });
    // Per-hit power / ferocity bonuses (flag-based)
    if (ht("Aeromancer's Training"))
      modifiers.push({
        id: "Trait:Aeromancer's Training",
        name: "Aeromancer's Training",
      });
    if (ht("Empowering Flame"))
      modifiers.push({
        id: "Trait:Empowering Flame",
        name: "Empowering Flame",
      });
    if (ht("Power Overwhelming"))
      modifiers.push({
        id: "Trait:Power Overwhelming",
        name: "Power Overwhelming",
      });
    if (ht("Raging Storm"))
      modifiers.push({ id: "Trait:Raging Storm", name: "Raging Storm" });
    if (ht("Fresh Air"))
      modifiers.push({ id: "Trait:Fresh Air", name: "Fresh Air" });
    if (ht("Elemental Polyphony"))
      modifiers.push({
        id: "Trait:Elemental Polyphony",
        name: "Elemental Polyphony",
      });
    if (ht("Elemental Empowerment"))
      modifiers.push({
        id: "Trait:Elemental Empowerment",
        name: "Elemental Empowerment",
      });
    if (ht("Enhanced Potency") && (permaBoons.Fury || permaBoons.Might))
      modifiers.push({
        id: "Trait:Enhanced Potency",
        name: "Enhanced Potency",
      });
    if (ht("Superior Elements"))
      modifiers.push({
        id: "Trait:Superior Elements",
        name: "Superior Elements",
      });
    if (ht("Burning Precision"))
      modifiers.push({
        id: "Trait:Burning Precision",
    ...
    if (ht("Persisting Flames"))
      modifiers.push({
        id: "Trait:Persisting Flames",
        name: "Persisting Flames",
      });
    if (ht("Pyromancer's Training"))
      modifiers.push({
        id: "Trait:Pyromancer's Training",
        name: "Pyromancer's Training",
      });
    if (ht("Fiery Might"))
      modifiers.push({ id: "Trait:Fiery Might", name: "Fiery Might" });
    if (ht("Stormsoul"))
      modifiers.push({ id: "Trait:Stormsoul", name: "Stormsoul" });
    ...
    if (ht("Bolt to the Heart") && (baselineStop !== null || boltFires))
      modifiers.push({
        id: "Trait:Bolt to the Heart",
        name: "Bolt to the Heart",
      });
    if (ht("Transcendent Tempest"))
      modifiers.push({
        id: "Trait:Transcendent Tempest",
        name: "Transcendent Tempest",
      });
    if (ht("Elements of Rage"))
      modifiers.push({
        id: "Trait:Elements of Rage",
        name: "Elements of Rage (proc)",
      });
    if (ht("Swift Revenge"))
      modifiers.push({ id: "Trait:Swift Revenge", name: "Swift Revenge" });
    if (ht("Empowering Auras"))
      modifiers.push({
        id: "Trait:Empowering Auras",
        name: "Empowering Auras",
      });
    if (ht("Familiar's Prowess"))
      modifiers.push({
        id: "Trait:Familiar's Prowess",
        name: "Familiar's Prowess",
      });
    if (ht("Lightning Rod"))
      modifiers.push({ id: "Trait:Lightning Rod", name: "Lightning Rod" });
    if (ht("Piercing Shards"))
      modifiers.push({ id: "Trait:Piercing Shards", name: "Piercing Shards" });
    if (ht("Flow like Water"))
      modifiers.push({ id: "Trait:Flow like Water", name: "Flow like Water" });
    if (ht("Arcane Precision"))
      modifiers.push({
        id: "Trait:Arcane Precision",
        name: "Arcane Precision",
      });
    if (ht("Arcane Prowess"))
      modifiers.push({ id: "Trait:Arcane Prowess", name: "Arcane Prowess" });
    if (ht("Elemental Attunement"))
      modifiers.push({
        id: "Trait:Elemental Attunement",
        name: "Elemental Attunement",
      });
    if (ht("Elemental Lockdown"))
      modifiers.push({
        id: "Trait:Elemental Lockdown",
        name: "Elemental Lockdown",
      });
    if (ht("Arcane Lightning"))
      modifiers.push({
        id: "Trait:Arcane Lightning",
        name: "Arcane Lightning",
      });
    if (ht("Bountiful Power"))
      modifiers.push({ id: "Trait:Bountiful Power", name: "Bountiful Power" });
    if (fullResults.allCondStacks.some((s) => s.cond === "Weave Self"))
      modifiers.push({
        id: "Trait:Weave Self Bonuses",
        name: "Weave Self (Fire+Air)",
      });
    ...
    if (contribWindowSec > 0) {
      const effectOnlyOverrides = new Map([
        [
          "Trait:Fresh Air",
          computeFreshAirEffectContribution(fullResultsForContrib),
        ],
        [
          "Trait:Transcendent Tempest",
          computeTranscendentTempestEffectContribution(fullResultsForContrib),
        ],
      ]);
      for (const contribution of contributions) {
        if (!effectOnlyOverrides.has(contribution.id)) continue;
        const increase =
          effectOnlyOverrides.get(contribution.id) / contribWindowSec;
        const withoutDps = fullDpsForContrib - increase;
        contribution.dpsIncrease = increase;
        contribution.pctIncrease =
          withoutDps > 0 ? (increase / withoutDps) * 100 : 0;
      }
    }
  ...
  _refreshEffect(S, effectName, durSec, time) {
    return refreshEffect(
      {
        S,
        trackEffect: (effect, stacks, duration, at) =>
          trackEffect(this, S, effect, stacks, duration, at, {
            boons: BOONS,
            relicProcs: RELIC_PROCS,
          }),
      },
      ...
      switch (ev.type) {
        case "hit":
          row = [
            ...
            r(d.serratedMul),
            r(d.fieryMightMul),
            r(d.piercingShardsMul),
            r(d.flowLikeWaterMul),
            r(d.zapMul),
            r(d.relicStrikeMul),
            r(d.condMul),
            "",
            ...
            r(d.att),
            r(d.att2),
            ev.flatStrike ? "flat-strike" : ev.isField ? "field" : "",
          ];
          break;
        case "cond_tick":
          row = [
