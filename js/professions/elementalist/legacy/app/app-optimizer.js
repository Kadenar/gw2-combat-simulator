...
function readInfusionRangeInputs() {
  const out = {};
  document
    .querySelectorAll("#opt-infusion-ranges .opt-infusion-range-row")
    .forEach((row) => {
...
export function populateOptimizerCheckboxes(app, { foodDesc, utilityDesc }) {
      ...
      el.innerHTML = groups
        .map(
          (g) =>
            ...
            g.items
              .map((name) => mkGridItem(name, group, currentVals))
              .join(""),
        )
        .join("");
    } else {
      ...
      el.innerHTML = groups
        .map(
          (g) =>
            `<div class="opt-section-label">${esc(g.label)}</div>` +
            g.items
              .map((name) => mkConsumableItem(name, group, currentVals, descFn))
              .join(""),
        )
        .join("");
    } else {
  ...
  makeGrid(
    ...
    PREFIXES,
    "prefix",
    [curPrefix, "Assassin's"].filter((p) => PREFIXES.includes(p)),
  );
  makeGrid(
    "opt-runes",
    RUNE_NAMES,
    "rune",
    [b.rune].filter(Boolean),
    RUNE_GROUPS,
  );
  makeGrid(
    "opt-sigils-1",
  ...
  if (infEl) {
    const usedStats = new Set(
      (b.infusions || []).filter((x) => x.count > 0).map((x) => x.stat),
    );
  ...
  const curInfTotal = (b.infusions || []).reduce(
    (s, x) => s + (x.count || 0),
    0,
  );
...
export function populateSlotConstraints(app) {
  ...
  container.innerHTML = activeSlots
    .map((slot) => {
...
export function renderInfusionRanges(app) {
  ...
  const total = clampInt(
    document.getElementById("opt-inf-total")?.value ?? "18",
    0,
    18,
    18,
  );
  const existing = readInfusionRangeInputs();
  const buildInfusions = new Map(
    (app.build.infusions || []).map((entry) => [entry.stat, entry.count || 0]),
  );

  if (!selectedStats.length) {
    container.innerHTML =
      '<div class="opt-infusion-ranges-empty">No infusion stat types selected.</div>';
    return;
  }

  container.innerHTML = selectedStats
    .map((stat) => {
...
export function bindOptimizerEvents(app) {
  ...
  document
    .getElementById("opt-infusions")
    ?.addEventListener("change", () => renderInfusionRanges(app));
  document
    .getElementById("opt-inf-total")
    ?.addEventListener("input", () => renderInfusionRanges(app));

  document
    .getElementById("btn-opt-run")
    ?.addEventListener("click", () => app._runOptimizer());
  document.getElementById("btn-opt-cancel")?.addEventListener("click", () => {
    if (app._optimizer) app._optimizer.cancel();
  });
  document
    .getElementById("btn-opt-export")
    ?.addEventListener("click", () => app._exportOptimizerResults());
}
...
export async function runOptimizer(app) {
  ...
  if (!sigilPairs) {
    alert(
      "No valid sigil pairs remain. Select at least one candidate for each slot, and duplicate sigils are skipped.",
    );
    return;
  }

  const infCombos =
    infusionStats.length === 0
      ? 1
      : enumerateInfusionDistributions(infusionRanges, infusionTotal).length;
  if (!infCombos) {
    alert(
      "Infusion min/max ranges do not allow any distribution matching the selected total.",
    );
    return;
  }

  const combos =
    Math.max(space.runes.length, 1) *
    Math.max(space.relics.length, 1) *
    sigilPairs *
    Math.max(space.foods.length, 1) *
    Math.max(space.utilities.length, 1) *
    infCombos;
...
export function renderOptimizerResults(app, results) {
  ...
  const PFX_CSS = {
    "Berserker's": "pfx-berserker",
    "Assassin's": "pfx-assassin",
    "Harrier's": "pfx-harrier",
    "Viper's": "pfx-viper",
    "Sinister's": "pfx-sinister",
    Grieving: "pfx-grieving",
    "Ritualist's": "pfx-ritualist",
    Celestial: "pfx-celestial",
    "Diviner's": "pfx-diviner",
    "Dragon's": "pfx-dragon",
    "Marshal's": "pfx-marshal",
    "Plaguedoctor's": "pfx-plaguedoctor",
    "Trailblazer's": "pfx-trailblazer",
    "Seraph's": "pfx-seraph",
  ...
  const shortName = (s) => {
    ...
    const trimmed = s.replace(
      /^Bowl of |^Plate of |^Superior |^Furious |^Toxic |^Potent /i,
      "",
    );
