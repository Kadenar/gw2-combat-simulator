...
const CHAIN_SKILLS = new Map([
  [
    "Aerial Agility",
    ["Aerial Agility", "Aerial Agility (chain)", "Aerial Agility (dash)"],
  ],
]);
...
export async function fetchEIJson(logId) {
  ...
  if (!res.ok)
    throw new Error(
      `dps.report returned HTTP ${res.status}. Is the URL correct?`,
    );
...
export function findElementalistPlayers(eiJson) {
  return (eiJson.players || []).filter((p) =>
    ELEMENTALIST_SPECS.has(p.profession),
  );
}
 ...
 */
export function convertEIRotation(
  eiJson,
  player,
  toolSkillNames,
  skillAttunements = new Map(),
  weapons = [],
) {
  ...
  for (const se of player.rotation || []) {
    ...
    if (info.isTraitProc || info.isUnconditionalProc || info.isGearProc)
      continue;
    ...
    if (cast.isInterrupted) {
      ...
      if (inWindow) {
        ...
        if (known) {
          items.push({
            name: toolName,
            offset: Math.max(1, cast.castTime - anchor.castTime),
          });
        }
      ...
      anchor = {
        castTime: cast.castTime,
        endTime: cast.castTime + cast.duration,
      };
    }
  }
...
function injectBlindingFlash(allCasts, eiJson, toolSkillNames, weapons) {
  ...
  for (const target of eiJson.targets || []) {
    for (const buff of target.buffs || []) {
      const set =
        buff.id === BLIND_BUFF_ID
          ? blindTimes
          : buff.id === WEAKNESS_BUFF_ID
            ? weakTimes
            : null;
  ...
  for (const t of candidates) {
    const hasBlindSource = allCasts.some(
      (c) =>
        BF_BLIND_SOURCES.has(c.name) &&
        Math.abs(c.castTime - t) <= BF_WINDOW_MS,
    );
    const hasWeakSource = allCasts.some(
      (c) =>
    ...
    if (hasBlindData && hasWeakData && hasBlindSource && hasWeakSource)
      continue;
