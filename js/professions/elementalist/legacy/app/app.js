...
import { calculateSkillDamage } from "../../../../platform/gw2/damage.js";
...
import { resultSummaryMetrics } from "../../../../platform/ui/result-transform.js";
import {
  nextResultSortState,
  sortResultRows,
...
import { createElementalistBuildDefaults } from "../build.js";
...
const STAT_ABBR = {
  ...
  Vitality: "Vit",
  Ferocity: "Ferc",
  "Condition Damage": "CndDmg",
  Expertise: "Exprt",
  Concentration: "Conc",
  "Healing Power": "Heal",
};
const DUR_ABBR = {
  "Burning Duration": "Burn",
  "Bleeding Duration": "Bleed",
  "Poison Duration": "Poison",
  "Torment Duration": "Torment",
...
function _foodDesc(name) {
  ...
  return [
    ...
    ...Object.entries(d.durations).map(
      ([k, v]) => `+${v}% ${DUR_ABBR[k] || k}`,
    ),
  ].join(", ");
}
function _foodOptionLabel(name) {
  ...
  if (Object.keys(d.stats).length >= 6) {
    ...
    for (const [k, v] of Object.entries(d.stats))
      parts.push(`${STAT_ABBR[k] || k}+${v}`);
    for (const [k, v] of Object.entries(d.durations))
      parts.push(`${DUR_ABBR[k] || k}+${v}%`);
  }
...
const CONJURE_WEAPONS = new Set([
  "Frost Bow",
  "Lightning Hammer",
  "Fiery Greatsword",
]);
const DROP_BUNDLE_ICON =
...
const HAMMER_ORB_SKILLS_UI = new Set([
  "Flame Wheel",
  "Icy Coil",
  "Crescent Wind",
  "Rocky Loop",
...
const HAMMER_ALL_ORB_NAMES_UI = new Set([
  ...HAMMER_ORB_SKILLS_UI,
  ...Object.keys(HAMMER_DUAL_ORB_SKILLS_UI),
]);
...
class App {
  ...
  async init() {
    ...
    try {
      ...
      this.setStatus(
        "Failed to load CSV files. Make sure to run a local web server.",
      );
      console.error(e);
      return;
    }
    ...
    this.build = createElementalistBuildDefaults();
    ...
    document
      .getElementById("btn-sim-clear")
      .addEventListener("click", () => this._clearRotation());
    document
      .getElementById("btn-export-rotation")
      .addEventListener("click", () => this._exportRotation());
    document
    ...
    document
      ...
      .addEventListener("change", (e) => {
        ...
        if (file) {
          this._importRotation(file);
          e.target.value = "";
        }
      });

    document
      .getElementById("btn-dpsreport-import")
      .addEventListener("click", () => {
        this._importFromDpsReport();
      });
    document
      .getElementById("dpsreport-url")
      .addEventListener("keydown", (e) => {
        if (e.key === "Enter") this._importFromDpsReport();
      });
    document
      .getElementById("dpsreport-player-select")
    ...
    document
      ...
      .addEventListener("click", () => this._exportBuild());
    document
    ...
    document
      .getElementById("glyph-booned-cb")
      .addEventListener("change", (e) => {
        this.glyphBoonedElementals = e.target.checked;
        if (this.sim)
          this.sim.glyphBoonedElementals = this.glyphBoonedElementals;
        this._persistBuild();
        if (this.sim?.rotation.length > 0) this._autoRun();
      });
    document
      .getElementById("thorns-boss-aura-cb")
      .addEventListener("change", (e) => {
        this.thornsBossAuraOnly = e.target.checked;
        if (this.sim) this.sim.thornsBossAuraOnly = this.thornsBossAuraOnly;
        if (this._optimizer)
          this._optimizer.thornsBossAuraOnly = this.thornsBossAuraOnly;
        this._persistBuild();
        if (this.sim?.rotation.length > 0) this._autoRun();
      });
  ...
  renderWeaponSelect() {
    ...
    weaponContainer.querySelector("#sel-mh").addEventListener("change", (e) => {
      ...
      document.getElementById("oh-row").style.pointerEvents = is2H
        ? "none"
        : "";
      this._onBuildChange();
      this.renderAttunementBar();
    ...
    const consumableRow = (
      ...
      const optionsHtml = groups
        ...
        : options
            .map((value) => option(value, selected, labelFor(value)))
            .join("");
    ...
    bind("sel-rune", (e) => {
      b.rune = e.target.value;
      this._onBuildChange();
    });
    bind("sel-sig1", (e) => {
      b.sigils[0] = e.target.value;
      this._onBuildChange();
    });
    bind("sel-sig2", (e) => {
      b.sigils[1] = e.target.value;
      this._onBuildChange();
    });
    bind("sel-relic", (e) => {
      b.relic = e.target.value;
      this._onBuildChange();
    });
    bind("sel-food", (e) => {
      b.food = e.target.value;
      const h = document.getElementById("sel-food-hint");
      if (h) h.textContent = _foodDesc(e.target.value);
      this._onBuildChange();
    });
    bind("sel-utility", (e) => {
      b.utility = e.target.value;
      const h = document.getElementById("sel-utility-hint");
      if (h) h.textContent = _utilityDesc(e.target.value);
      this._onBuildChange();
    });
    bind("chk-jbc", (e) => {
      b.jadeBotCore = e.target.checked;
      this._onBuildChange();
    });
    ...
    eq.querySelectorAll(".inf-count").forEach((input) => {
      input.addEventListener("change", () => {
        ...
        const otherTotal = b.infusions.reduce(
          (s, inf, j) => (j !== idx ? s + inf.count : s),
          0,
        );
        ...
        document.getElementById("inf-total").textContent =
          `${b.infusions.reduce((s, x) => s + x.count, 0)}/18`;
  ...
  _getMHTypes() {
    return Object.entries(WEAPON_DATA)
      .filter(
        ([, d]) =>
          d.wielding === "mh" || d.wielding === "mh+oh" || d.wielding === "2h",
      )
      .map(([k]) => k);
  }
  ...
  renderAttributes() {
    ...
    const fmt = (n, v) =>
      pctSet.has(n) ? v.toFixed(2) + "%" : Math.round(v).toString();
  ...
  renderConditions() {
    ...
    const selectedSkillNames = new Set(
      Object.values(this.selectedSkills || {})
        .filter(Boolean)
        .map((s) => s.name),
    );
    ...
    const hasConjureLightningHammer = selectedSkillNames.has(
      "Conjure Lightning Hammer",
    );
    const hasConjureFieryGreatsword = selectedSkillNames.has(
    ...
    const showAttunement =
      hasPolyphony || hasEmpoweringFlame || hasAeroTraining;

    const ATTS = ["None", "Fire", "Water", "Air", "Earth"];
    const attSel = (id, cur) =>
      `<select class="cond-select" id="${id}">${ATTS.map(
        (a) => `<option${a === cur ? " selected" : ""}>${a}</option>`,
      ).join("")}</select>`;
    ...
    const attEffects = (att, isPrimary) => {
      ...
      if (isPrimary && hasEmpoweringFlame && att === "Fire")
        parts.push("+150 Pwr");
      if (isPrimary && hasAeroTraining && att === "Air")
        parts.push("+150 Ferocity");
      return parts.length ? parts.join(", ") : att;
    };

    const furyLabel = hasEnhancedPotency
      ? "+40% Crit Chance (Enhanced Potency)"
      : "+25% Crit Chance";
    const mightLabel = hasEnhancedPotency
      ? "+30 Pwr / +30–35 CondDmg (Enhanced Potency)"
      : "+30 Pwr/CondDmg";
    ...
    bind("cond-might", (e) => {
      c.might = Math.max(0, Math.min(25, parseInt(e.target.value) || 0));
      e.target.value = c.might;
      this.renderAttributes();
    });
    bind("cond-fury", (e) => {
      c.fury = e.target.checked;
      this.renderAttributes();
    });
    bind("cond-att-pri", (e) => {
      c.primaryAtt = e.target.value;
      this.renderConditions();
      this.renderAttributes();
    });
    bind("cond-att-sec", (e) => {
      c.secondaryAtt = e.target.value;
      this.renderConditions();
      this.renderAttributes();
    });
    bind("cond-ee", (e) => {
      c.elemEmpowerment = Math.max(
        0,
        Math.min(10, parseInt(e.target.value) || 0),
      );
      e.target.value = c.elemEmpowerment;
      this.renderAttributes();
    });
    bind("cond-fresh-air", (e) => {
      c.freshAir = e.target.checked;
      this.renderAttributes();
    });
    bind("cond-sup-elem", (e) => {
      c.superiorElements = e.target.checked;
      this.renderAttributes();
    });
    bind("cond-severance", (e) => {
      c.severance = e.target.checked;
      this.renderAttributes();
    });
    bind("cond-raging-storm", (e) => {
      c.ragingStorm = e.target.checked;
      this.renderAttributes();
    });
    bind("cond-arcane-lightning", (e) => {
      c.arcaneLightning = e.target.checked;
      this.renderAttributes();
    });
    bind("cond-crescent-wind", (e) => {
      c.crescentWind = e.target.checked;
      this.renderAttributes();
    });
    bind("cond-conjure-frost-bow", (e) => {
      c.conjureFrostBow = e.target.checked;
      this.renderAttributes();
    });
    bind("cond-conjure-lightning-hammer", (e) => {
      c.conjureLightningHammer = e.target.checked;
      this.renderAttributes();
    });
    bind("cond-conjure-fiery-greatsword", (e) => {
      c.conjureFieryGreatsword = e.target.checked;
      this.renderAttributes();
    });
  }
  ...
  _getConditionalAttrs() {
    ...
    if (c.might > 0) {
      const mightPwr =
        hasEnhancedPotency && isEvoker && this.evokerElement === "Fire"
          ? 35
          : 30;
      const mightCond =
        hasEnhancedPotency && isEvoker && this.evokerElement === "Fire"
    ...
    if (hasPolyphony) {
      const atts = new Set(
        [c.primaryAtt, c.secondaryAtt].filter((a) => a && a !== "None"),
      );
    ...
    const furyCC = c.fury
      ? hasEnhancedPotency && isEvoker && this.evokerElement === "Air"
        ? 40
        : 25
      : 0;
    ...
    out["Critical Chance"] = {
      ...out["Critical Chance"],
      final:
        newPrecCC + traitCC + sigilCC + furyCC + supElemCC + crescentWindCC,
    };
    ...
    const boonFixedBonus =
      (base["Boon Duration"]?.final ?? 0) -
      (base["Concentration"]?.final ?? 0) / 15;
    out["Boon Duration"] = {
      ...out["Boon Duration"],
      final: conc / 15 + boonFixedBonus,
    };

    // Condition Duration: non-expertise bonus preserved; expertise component updated
    const condFixedBonus =
      (base["Condition Duration"]?.final ?? 0) -
      (base["Expertise"]?.final ?? 0) / 15;
    const frostBowCondBonus = c.conjureFrostBow && hasConjureFrostBow ? 20 : 0;
    out["Condition Duration"] = {
      ...out["Condition Duration"],
      final: exp / 15 + condFixedBonus + frostBowCondBonus,
    };
  ...
  _getEpModifiers(att, att2) {
    ...
    mods.push({
      id: "fury",
      label: `Fury (+${furyCCBonus}% CC)`,
      cat: "stat",
      effects: { cc: furyCCBonus },
      default: true,
    });
    ...
    if (hasTrait("Empowering Flame")) {
      mods.push({
        id: "empFlame",
        icon: icon("Empowering Flame"),
        trait: "Empowering Flame",
        tip: "+150 Power",
        cat: "stat",
        effects: { power: 150 },
        default: att === "Fire",
    ...
    if (hasTrait("Aeromancer's Training")) {
      mods.push({
        id: "aeroTrain",
        icon: icon("Aeromancer's Training"),
        trait: "Aeromancer's Training",
        tip: "+150 Ferocity",
        cat: "stat",
        effects: { ferocity: 150 },
        default: att === "Air",
      });
    }
    ...
    if (hasTrait("Raging Storm")) {
      mods.push({
        id: "ragingStorm",
        icon: icon("Raging Storm"),
        trait: "Raging Storm",
        tip: "+180 Ferocity",
        cat: "stat",
        effects: { ferocity: 180 },
        default: true,
      });
    }
    if (hasTrait("Arcane Lightning")) {
      mods.push({
        id: "arcLight",
        icon: icon("Arcane Lightning"),
        trait: "Arcane Lightning",
        tip: "+150 Ferocity",
        cat: "stat",
        effects: { ferocity: 150 },
        default: att === "Air",
    ...
    if (isCatalyst) {
      ...
      const empMul = hasEmpEmp ? 0.2 : 0.1;
      ...
      mods.push({
        id: "ee",
        icon: icon("Elemental Empowerment"),
        trait: "Elemental Empowerment",
        tip: `10× (${tipParts.join(", ")})`,
        cat: "stat",
        effects: eeEffects,
        default: true,
      });
    ...
    for (const name of sigils) {
      ...
      if (s.strikeDamageA)
        mods.push({
          id: `sigil-${name}`,
          label: `${name} (+${s.strikeDamageA}%)`,
          cat: "add",
          value: s.strikeDamageA / 100,
          default: true,
        });
      if (s.strikeDamageM)
        mods.push({
          id: `sigil-${name}-m`,
          label: `${name} (×${(1 + s.strikeDamageM / 100).toFixed(2)})`,
          cat: "mul",
          value: 1 + s.strikeDamageM / 100,
          default: true,
        });
    }

    if (hasTrait("Persisting Flames"))
      mods.push({
        id: "pf",
        icon: icon("Persisting Flames"),
    ...
    if (hasTrait("Tempestuous Aria"))
      mods.push({
        id: "tempAria",
        icon: icon("Tempestuous Aria"),
        trait: "Tempestuous Aria",
        tip: "+10%",
        cat: "add",
        value: 0.1,
        default: true,
      });
    if (hasTrait("Transcendent Tempest"))
      mods.push({
        id: "transcTemp",
        icon: icon("Transcendent Tempest"),
    ...
    if (hasTrait("Elements of Rage"))
      mods.push({
        id: "elemRage",
        icon: icon("Elements of Rage"),
    ...
    if (hasTrait("Empowering Auras"))
      mods.push({
        id: "empAuras",
        icon: icon("Empowering Auras"),
        trait: "Empowering Auras",
        tip: "5×1%",
        cat: "add",
        value: 0.05,
        default: true,
      });
    if (hasTrait("Relentless Fire"))
      mods.push({
        id: "relFire",
        icon: icon("Relentless Fire"),
        trait: "Relentless Fire",
        tip: "+10%",
        cat: "add",
        value: 0.1,
        default: att === "Fire",
      });
    if (hasTrait("Bountiful Power"))
      mods.push({
        id: "bountiful",
        icon: icon("Bountiful Power"),
        trait: "Bountiful Power",
        tip: "+20%",
        cat: "add",
        value: 0.2,
        default: false,
    ...
    if (hasTrait("Familiar's Prowess")) {
      ...
      mods.push({
        id: "famProwess",
        icon: icon("Familiar's Prowess"),
        trait: "Familiar's Prowess",
        tip: `+${pct}%`,
        cat: "add",
        value: pct / 100,
        default: true,
    ...
    if (hasTrait("Pyromancer's Training"))
      mods.push({
        id: "pyro",
        icon: icon("Pyromancer's Training"),
    ...
    if (hasTrait("Fiery Might"))
      mods.push({
        id: "fieryMight",
        icon: icon("Fiery Might"),
        trait: "Fiery Might",
        tip: "×1.05",
        cat: "mul",
        value: 1.05,
    ...
    if (hasTrait("Serrated Stones"))
      mods.push({
        id: "serrated",
        icon: icon("Serrated Stones"),
        trait: "Serrated Stones",
        tip: "×1.05",
        cat: "mul",
        value: 1.05,
        default: true,
      });
    if (hasTrait("Stormsoul"))
      mods.push({
        id: "stormsoul",
        icon: icon("Stormsoul"),
        trait: "Stormsoul",
    ...
    if (hasTrait("Flow Like Water"))
      mods.push({
        id: "flowWater",
        icon: icon("Flow Like Water"),
        trait: "Flow Like Water",
        tip: "×1.10",
        cat: "mul",
        value: 1.1,
        default: true,
      });
    if (hasTrait("Bolt to the Heart"))
      mods.push({
        id: "bolt",
        icon: icon("Bolt to the Heart"),
        trait: "Bolt to the Heart",
        tip: "×1.20",
        cat: "mul",
        value: 1.2,
        default: false,
    ...
    if (hasTrait("Piercing Shards")) {
      ...
      mods.push({
        id: "piercing",
        icon: icon("Piercing Shards"),
        trait: "Piercing Shards",
        tip: `×${mul.toFixed(2)}`,
        cat: "mul",
        value: mul,
        default: true,
    ...
    if (
      hasTrait("Elemental Dynamo") ||
      specs.some((s) => s.name === "Evoker")
    ) {
      mods.push({
        id: "zap",
        icon: icon("Elemental Dynamo"),
        trait: "Zap",
        tip: "×1.03",
        cat: "mul",
        value: 1.03,
    ...
    if (selectedSkillNames.has("Signet of Fire")) {
      mods.push({
        id: "signetFire",
        label: "Signet of Fire (+180 Prec)",
        cat: "stat",
        effects: { precision: 180 },
        default: true,
        _invertedStat: true,
      });
    }
    ...
    if (RELIC_STRIKE[relic])
      mods.push({
        id: "relic",
        label: `${relic} (×${RELIC_STRIKE[relic].toFixed(2)})`,
        cat: "mul",
        value: RELIC_STRIKE[relic],
        default: true,
      });
    if (relic === "Nourys")
      mods.push({
  ...
  _getEpCondiModifiers() {
    ...
    const hasTrait = (name) => activeTraits.some((t) => t.name === name);
    const sigils = this.build?.sigils || [];
    const isWeaver = (this.data?.attributes?.specializations || []).some(
      (s) => s.name === "Weaver",
    );
    const icon = (name) => this.api.getTraitIcon(name) || null;
    ...
    for (const name of sigils) {
      const s = SIGIL_DATA[name];
      if (!s) continue;
      if (s.conditionDamageA)
        mods.push({
          id: `csigil-${name}`,
          label: `${name} (+${s.conditionDamageA}%)`,
          cat: "add",
          value: s.conditionDamageA / 100,
          default: true,
        });
      if (s.conditionDamageM)
        mods.push({
          id: `csigil-${name}-m`,
          label: `${name} (×${(1 + s.conditionDamageM / 100).toFixed(2)})`,
          cat: "mul",
          value: 1 + s.conditionDamageM / 100,
          default: true,
        });
    }
    ...
    if ((this.data?.attributes?.weapons || []).includes("Hammer"))
      mods.push({
        id: "chammerFire",
        label: "Hammer Fire (+5%)",
        cat: "add",
  ...
  _computeEp(att, att2, rowKey) {
    ...
    for (const mod of mods) {
      const checked =
        rowState[mod.id] !== undefined ? rowState[mod.id] : mod.default;
    ...
    const ccPct = Math.min(
      (precision - 895) / 21 + traitCC + sigilCC + extraCC,
      100,
    );
  ...
  _computeCondiEp() {
    ...
    const mightChecked =
      condiState["cmight"] !== undefined ? condiState["cmight"] : true;
    ...
    if (hasInferno) {
      ...
      const mightPowerPerStack =
        hasEnhPot && this.evokerElement === "Fire" ? 35 : 30;
      if (mightChecked) infernoPower += mightStacks * mightPowerPerStack;
      if (hasTrait("Empowering Flame")) infernoPower += 150;
      if (hasTrait("Power Overwhelming") && mightChecked && mightStacks >= 10)
        infernoPower += 300;
      ...
      if (isCatalyst && this._empPool?.Power) {
        ...
        const eeChecked =
          condiState["cee"] !== undefined ? condiState["cee"] : true;
        if (eeChecked)
          infernoPower += Math.round(this._empPool.Power * 10 * empMul);
      }
    ...
    for (const mod of mods) {
      const checked =
        condiState[mod.id] !== undefined ? condiState[mod.id] : mod.default;
      if (!checked) continue;
  ...
  renderEffectivePower() {
    ...
    if (isWeaver) {
      ...
      rows.push({
        ...
        att2: "Water",
        key: "Air/WE",
        label: "Air/Water·Earth",
      });
      rows.push({
        att: "Water",
        att2: "Fire",
        key: "WE/Fire",
        label: "Water·Earth/Fire",
      });
      rows.push({
        att: "Water",
        att2: "Air",
        key: "WE/Air",
        label: "Water·Earth/Air",
      });
      rows.push({
        att: "Water",
        att2: "Water",
        key: "WE/WE",
        label: "Water·Earth dual",
      });
    } else {
      for (const att of ATTS) {
      ...
      if (!row.att2) {
        ...
        const colorPart = (p) => {
          if (p.startsWith("Fire"))
            return `<span style="color:${ATT_COLORS.Fire}">${p}</span>`;
          if (p.startsWith("Air"))
            return `<span style="color:${ATT_COLORS.Air}">${p}</span>`;
    ...
    container
      .querySelectorAll('input[type="checkbox"][data-att][data-mod]')
      .forEach((cb) => {
  ...
  renderTraits() {
    ...
    container.innerHTML = specs
      .map((spec, slotIdx) => {
        ...
        const tierPairs = [
          ...
          {
            minor: "Minor Grandmaster",
            major: "Major Grandmaster",
            pick: choices[2],
          },
        ];

        const tiersHtml = tierPairs
          .map((tp, tierIdx) => {
            const minor = specTraits.find((t) => t.tier === tp.minor);
            const majors = specTraits
              .filter((t) => t.tier === tp.major)
    ...
    container.querySelectorAll(".spec-trait-major").forEach((el) => {
      el.addEventListener("click", () => {
        ...
        const picks = this.build.specializations[slotIdx].traits
          .split("-")
          .map(Number);
    ...
    container.querySelectorAll(".spec-select").forEach((el) => {
      el.addEventListener("change", () => {
        ...
        this.build.specializations[slotIdx] = {
          name: newSpec,
          traits: "1-1-1",
        };
        this._onBuildChange();
  ...
  renderAttunementBar() {
    ...
    const baseSwaps = this.data.skills.filter(
      (s) =>
        s.type === "Attunement" &&
        !s.attunement &&
        s.weapon === "Profession mechanic",
    );
    const overloads =
      eliteSpec === "Tempest"
        ? this.data.skills.filter(
            (s) =>
              s.type === "Attunement" &&
              s.attunement &&
              s.weapon === "Profession mechanic",
  ...
  setAttunement(att) {
    ...
    if (elite === "Weaver") {
      if (att === this.activeAttunement && att === this.secondaryAttunement)
        return;
  ...
  renderWeaponBar() {
    ...
    for (let slot = 1; slot <= 5; slot++) {
      ...
      if (slot === 1 && weapon === "Pistol" && allBullets) {
        const eeSk = this.data.skills.find(
          (s) => s.name === "Elemental Explosion",
        );
      ...
      if (isWeaver && slot === 3 && att !== this.secondaryAttunement) {
        ...
        skills = this._getSkillsForSlot(
          weapon,
          this.secondaryAttunement,
          String(slot),
        );
      } else {
  ...
  _getChainOrder(skills) {
    ...
    const chainTargets = new Set(
      skills.filter((s) => s.chainSkill).map((s) => s.chainSkill),
    );
  ...
  _getChainOrderWithEtching(skills) {
    ...
    if (lesserSk && !order.some((s) => s.name === lesserSk.name))
      order.push(lesserSk);
    if (fullSk && !order.some((s) => s.name === fullSk.name))
      order.push(fullSk);
    return order;
  }
  ...
  renderSkillBar() {
    ...
    container.innerHTML = SLOT_LABELS.map((slotKey) => {
      ...
      const resolvedSkill = selected
        ? this._resolveAttunementSkill(selected)
        : null;
      const icon = resolvedSkill
        ? this.api.getSkillIcon(resolvedSkill.name)
        : null;
      const borderClass =
        slotKey === "heal"
          ? "heal-border"
  ...
  _getAvailableSkillsForSlot(slotType) {
    const selectedSpecs = new Set(
      this.data.attributes.specializations.map((s) => s.name),
    );
    const activeTraits = this.data.attributes?.activeTraits || [];
    const hasElementsOfRage = activeTraits.some(
      (t) => t.name === "Elements of Rage",
    );
    ...
    for (const sk of skills) {
      ...
      if (sk.attunement && base !== sk.name) {
        ...
        seen.add(base);
        const variant =
          skills.find((s) => s.name === `${base} (${this.activeAttunement})`) ||
          sk;
        ...
        if (sk.chainSkill) {
          ...
          const isChainTarget = skills.some(
            (s) =>
              s.chainSkill === sk.name &&
              s.slot === slotType &&
              (s.recharge || 0) > 0,
          );
  ...
  _resolveAttunementSkill(skill) {
    ...
    if (
      ...
      const variant = this.data.skills.find(
        (s) => s.name === `${realBase} (${this.activeAttunement})`,
      );
  ...
  renderSkillInfoTable() {
    ...
    for (const att of ATTUNEMENTS) {
      ...
      if (eliteSpec === "Tempest") {
        const overload = this.data.skills.find(
          (s) =>
            s.weapon === "Profession mechanic" &&
            s.attunement === att &&
            s.type === "Attunement" &&
            s.name.startsWith("Overload"),
        );
        if (overload) {
    ...
    if (weaverDualSkills.length > 0) {
      html +=
        '<div class="info-att-header" style="color:#d7c06a">Weaver Dual Skills</div>';
      html += '<div class="info-rows">';
    ...
    if (specialSkills.length > 0) {
      html +=
        '<div class="info-att-header" style="color:var(--accent)">Special Skills</div>';
      html += '<div class="info-rows">';
      const selCtx = this._getEpContextForAtt(selectedAtt, selectedAtt2);
      for (const sk of specialSkills)
        html += this._renderInfoRow(sk, this._getWeaponForSkill(sk), selCtx);
      html += "</div>";
    }

    const professionMechanics =
      this._getInfoProfessionMechanicSkills(eliteSpec);
    if (professionMechanics.length > 0) {
      html +=
        '<div class="info-att-header" style="color:#66c7d8">Profession Mechanics</div>';
      html += '<div class="info-rows">';
      for (const sk of professionMechanics) {
        const mechAtt = sk.attunement || selectedAtt;
        const ctx = this._getEpContextForAtt(
          mechAtt,
          isWeaver ? selectedAtt2 : null,
        );
    ...
    if (selectedAny) {
      html +=
        '<div class="info-att-header" style="color:var(--accent)">Selected Skills</div>';
      html += '<div class="info-rows">';
      const selCtx = this._getEpContextForAtt(selectedAtt, selectedAtt2);
      for (const slotKey of SLOT_LABELS) {
        ...
        if (sel.type === "Conjure") {
          ...
          if (conjWeapon) {
            const conjSkills = this.data.skills.filter(
              (s) => s.weapon === conjWeapon,
            );
    ...
    container.innerHTML =
      html ||
      '<div class="placeholder-text">No weapon skills found for current weapon set</div>';
  }
  ...
  _getEpContextForAtt(att, att2) {
    ...
    if (!result)
      return {
        strikeMul: 1,
        condMul: 1,
        attrs: this.data.attributes.attributes,
      };
    ...
    modifiedAttrs["Critical Chance"] = {
      ...modifiedAttrs["Critical Chance"],
      final: result.cc,
    };
    modifiedAttrs["Critical Damage"] = {
      ...modifiedAttrs["Critical Damage"],
      final: result.cd,
    };
    ...
    if (mightChecked) {
      const activeTraitsLocal = this.data.attributes.activeTraits || [];
      const hasEnhPot = activeTraitsLocal.some(
        (t) => t.name === "Enhanced Potency",
      );
      const mightPerStack =
        hasEnhPot && this.evokerElement === "Fire" ? 35 : 30;
      const mightCondDmg = (this.permaBoons?.Might || 0) * mightPerStack;
      const baseCondDmg = base["Condition Damage"]?.final ?? 0;
      modifiedAttrs["Condition Damage"] = {
        ...modifiedAttrs["Condition Damage"],
        final: baseCondDmg + mightCondDmg,
      };
    }
    ...
    if (cwpMod) {
      const cwpChecked =
        condiState["cwp"] !== undefined ? condiState["cwp"] : cwpMod.default;
      if (cwpChecked) {
        const baseCondDur = base["Condition Duration"]?.final ?? 0;
        modifiedAttrs["Condition Duration"] = {
          ...modifiedAttrs["Condition Duration"],
          final: Math.min(baseCondDur + 20, 100),
        };
      }
    }
    ...
    return {
      strikeMul: result.modifierTotal,
      condMul: this._getCondiMul(),
      attrs: modifiedAttrs,
      infernoBurningTick,
    };
  }
  ...
  _getBestEpContextForDual(skill) {
    ...
    return this._getEpContextForAtt(
      this.activeAttunement,
      this.secondaryAttunement,
    );
  }
  ...
  _renderInfoRow(skill, weaponKey, epCtx) {
    ...
    const wStr =
      WEAPON_DATA[weaponKey]?.weaponStrength ||
      WEAPON_DATA[this._getWeaponForSkill(skill)]?.weaponStrength ||
      1000;
    const attrs = epCtx ? epCtx.attrs : this.data.attributes.attributes;
    const maxHit =
      this.hitboxSize === "small"
        ? (SMALL_HITBOX_CAPS.get(skill.name) ?? Infinity)
        : Infinity;
  ...
  _getInfoWeaverDualSkills(mh, oh, is2h) {
    ...
    return this._uniqueSkillsByName(
      this.data.skills.filter(
        (skill) =>
  ...
  _getInfoSpecialSkills(mh, oh) {
    ...
    if (mh === "Pistol" || oh === "Pistol") {
      const ee = this.data.skills.find(
        (skill) => skill.name === "Elemental Explosion",
      );
      if (ee) skills.push(ee);
    }
    if (mh === "Hammer" || oh === "Hammer") {
      const gf = this.data.skills.find(
        (skill) => skill.name === "Grand Finale" && skill.weapon === "Hammer",
      );
  ...
  _getInfoProfessionMechanicSkills(eliteSpec) {
    if (eliteSpec === "Catalyst") {
      return this._uniqueSkillsByName(
        this.data.skills.filter((skill) => skill.type === "Jade Sphere"),
      );
    }
    if (eliteSpec === "Evoker") {
      return this._uniqueSkillsByName(
        this.data.skills.filter((skill) => skill.type === "Familiar"),
      );
    }
    return [];
  }
  ...
  _getEliteSpec() {
    ...
    return (
      this.data.attributes.specializations.find((s) => elites.has(s.name))
        ?.name || null
    );
  }
  ...
  _renderPermaBoons() {
    ...
    el.innerHTML =
      ...
      TARGET_CONDITION_GROUPS.map((group) =>
        renderGroup(group.label, group.conditions),
      ).join("");

    el.querySelectorAll('input[type="checkbox"]').forEach((cb) => {
      cb.addEventListener("change", () => {
        const name = cb.dataset.effect;
        const stackInput = el.querySelector(
          `input[type="number"][data-effect="${name}"]`,
        );
        if (stackInput) {
          if (cb.checked) {
    ...
    el.querySelectorAll('input[type="number"]').forEach((inp) => {
      inp.addEventListener("change", () => {
        ...
        const cb = el.querySelector(
          `input[type="checkbox"][data-effect="${name}"]`,
        );
  ...
  _skillColor(skill, skillName) {
    ...
    if (
      skillName === "__drop_bundle" ||
      (skillName && skillName.startsWith("__pickup_"))
    )
      return "#ffcc44";
    ...
    for (const att of ATTUNEMENTS) {
      if (
        skill.name === `${att} Attunement` ||
        skill.name === `Overload ${att}`
      )
  ...
  _skillInContext(name) {
    ...
    if (es?.conjureEquipped) {
      return (
        matches.find((s) => s.weapon === es.conjureEquipped) ||
  ...
  _isSkillAvailable(skillName) {
    ...
    if (sk.name.startsWith("Overload ")) {
      ...
      const dwellReady =
        es.attEnteredAt + this._esAlaCd(es, dwell, es.attEnteredAt);
    ...
    if (sk.chainSkill) {
      if (AURA_TRANSMUTE_SKILLS_UI[skillName])
        return this._hasAuraTransmuteAccess(skillName, es);
    ...
    if (skillName === "Grand Finale" && sk.weapon === "Hammer") {
      ...
      const activeOrbs = Object.entries(orbs)
        .filter(([, exp]) => exp !== null && exp > t)
        .map(([el]) => el);
      ...
      if (es.eliteSpec !== "Weaver" || !sec || pri === sec) {
        ...
        if (!qualifies) {
          for (const el of activeOrbs) {
            ...
            if (parts.includes(pri) || parts.includes(sec)) {
              qualifies = true;
              break;
            }
          }
    ...
    if (skillName === "Tailored Victory" && (es.perfectWeaveUntil || 0) <= t)
      return false;
  ...
  _getChainRootName(sk) {
    const candidates = this.data.skills.filter(
      (s) =>
        s.slot === sk.slot &&
        s.attunement === sk.attunement &&
        s.weapon === sk.weapon &&
        s.chainSkill,
    );
  ...
  _hasAuraTransmuteAccess(skillName, es) {
    ...
    return allCondStacks.some(
      (stack) =>
        stack.cond === auraName &&
        stack.t <= es.time &&
        stack.expiresAt > es.time,
    );
  }
  ...
  _isVirtualAvailable(name) {
    ...
    if (name.startsWith("__pickup_")) {
      ...
      return (es.conjurePickups || []).some(
        (p) => p.weapon === weapon && es.time <= p.expiresAt,
      );
    }
    return false;
  }
  ...
  _getPendingRockBarrierReadyAt(es) {
    ...
    return (
      expiry + this._esAlaCd(es, Math.round(skill.recharge * 1000), expiry)
    );
  }
  ...
  _getResultsCombatReferenceTime(results = this.sim?.results) {
    ...
    const firstHitTime =
      results.log?.find(
        (ev) => (ev.type === "hit" && ev.strike > 0) || ev.type === "cond_tick",
      )?.t ?? null;
    const explicitCombatStart = Number.isFinite(
    ...
    return (
      firstHitTime ??
      (results.endState?.hasExplicitCombatStart
        ? (explicitCombatStart ?? 0)
        : 0)
    );
  }
  ...
  _getDisplayedCooldownMs(es, readyAt, meta = null) {
    ...
    if (
      ...
      !Number.isFinite(meta.displayDurationMs)
    ) {
    ...
    const alaElapsedMs =
      alaUntil > startedAt ? Math.max(0, Math.min(t, alaUntil) - startedAt) : 0;
    const displayElapsedMs = elapsedMs + alaElapsedMs * 0.25;
    const displayRemainingMs = Math.max(
      0,
      displayDurationMs - displayElapsedMs,
    );
  ...
  _getSkillCD(skill) {
    ...
    if (skill.type === "Attunement" && !name.startsWith("Overload")) {
      ...
      const cdMs = this._getDisplayedCooldownMs(
        es,
        es.attCD[target] || 0,
        es.attCDMeta?.[target],
      );
    ...
    if (skill.type === "Dodge" || skill.slot === "Dodge") {
      const activeTraits = this.data?.attributes?.activeTraits || [];
      const hasEvasiveArcana = activeTraits.some(
        (trait) => trait.name === "Evasive Arcana",
      );
    ...
    if (
      skill.type === "Familiar" &&
      EVOKER_FAMILIAR_INTERRUPT_WINDOWS_UI[skill.name]
    ) {
      const lockout = es.evokerFamiliarLockouts?.[skill.name];
      if (lockout?.start !== undefined) {
        const remainingMs =
          lockout.start + EVOKER_FAMILIAR_INTERRUPT_WINDOWS_UI[skill.name] - t;
  ...
  _getChargeCount(sk) {
    if (!sk.maximumCount || sk.maximumCount <= 0 || !sk.countRecharge)
      return null;
    const es = this.sim?.results?.endState;
  ...
  _palIcon(skill, available = true, opts = {}) {
    const icon =
      skill.type === "Dodge" || skill.slot === "Dodge"
        ? DODGE_ICON
    ...
    const chargeIndicator = ammoDisplay
      ? `<span class="pal-charges">${ammoDisplay.current}/${ammoDisplay.maximum}</span>
            <span class="pal-ammo-pips" aria-hidden="true">${ammoDisplay.pips
              .map(
                (filled) =>
    ...
    if (opts.cooldownSkill) {
      ...
      if (ownCd !== null)
        cdSecs = cdSecs !== null ? Math.max(cdSecs, ownCd) : ownCd;
    }
    const cdBadge =
      cdSecs !== null ? `<span class="pal-cd">${cdSecs.toFixed(1)}</span>` : "";
  ...
  _renderResults() {
    ...
    let h = `<div class="res-summary">${summary
      .map(
        (metric) => `
            <div class="res-stat">
    ...
    for (const ev of r.log) {
      ...
      switch (ev.type) {
        case "cast":
          desc = `CAST ${ev.skill} [${ev.att}] (${ev.dur}ms)`;
          break;
        case "cast_end":
          desc = `END  ${ev.skill}`;
          break;
        case "swap":
          desc = `SWAP ${ev.from} → ${ev.to}`;
          break;
        case "hit": {
          ...
          if (ev.flatStrike && d.power) detail += ` | pwr:${d.power}`;
            ...
            detail += ` | pwr:${d.power} ws:${d.ws} cc:${d.critCh?.toFixed(1)}% cd:${d.critDmg?.toFixed(1)}% cMul:${d.critMul?.toFixed(3)} sMul:${d.strikeMul?.toFixed(3)}`;
          desc = detail;
          break;
        }
        case "apply":
          desc = `EFFECT ${ev.effect} ×${ev.stacks}${ev.dur > 0 ? ` (${ev.dur}s)` : ""} [${ev.skill}]`;
          break;
        case "cond_apply": {
        ...
        case "cond_tick": {
          ...
          if (d.condMul)
            detail += ` | condDmg:${d.condDmg} base:${d.baseTick} mul:${d.condMul?.toFixed(3)} vuln:${d.vulnMul?.toFixed(2)}`;
          desc = detail;
          break;
        }
        case "field":
          desc = `FIELD ${ev.field} (${ev.dur}ms) [${ev.skill}]`;
          break;
        case "aura":
          desc = `AURA  ${ev.aura} (${ev.dur}ms) [${ev.skill}]`;
          break;
        case "conjure":
          desc = `CONJURE ${ev.weapon} equipped (pickup expires ${this._formatResultsTimeMs(ev.pickupExpires, 1, r)})`;
          break;
        case "jade_sphere":
          desc = `JADE SPHERE ${ev.att} (energy: ${ev.energy}, dur: ${ev.durMs}ms) [${ev.skill}]`;
          break;
        case "familiar_select":
          desc = `FAMILIAR ${ev.element} selected [${ev.skill}]`;
          break;
        case "familiar_basic":
          desc = `FAMILIAR ${ev.skill} used → charges:${ev.charges}/${ev.maxCharges} empowered:${ev.empowered}/3`;
          break;
        case "familiar_empowered":
          desc = `FAMILIAR ${ev.skill} used → empowered:${ev.empowered}/3`;
          break;
        case "evoker_charges": {
        ...
        case "skill_proc":
          desc = `PROC  ${ev.skill}${ev.detail ? ` (${ev.detail})` : ""}`;
          break;
        case "skip":
          desc = `SKIP  ${ev.skill}${ev.reason ? ` (${ev.reason})` : ""}`;
          break;
        case "drop":
          desc = `DROP ${ev.weapon}`;
          break;
        case "pickup":
          desc = `PICKUP ${ev.weapon}`;
          break;
        case "wait":
          desc = `WAIT ${ev.durMs}ms`;
          break;
        case "sigil_proc":
          desc = `SIGIL ${ev.sigil} proc [${ev.skill}]`;
          cls = " sigil";
          break;
        case "relic_proc":
          desc = `RELIC ${ev.relic} proc [${ev.skill}]`;
          cls = " relic";
          break;
        case "trait_proc":
          desc = `TRAIT ${ev.trait} proc [${ev.skill}]`;
          cls = " trait";
          break;
        case "err":
          desc = ev.msg;
    ...
    const skillRows = Object.entries(r.perSkill)
      .map(([name, d]) => {
        ...
        const skillDps =
          dpsWindowSec > 0 ? Math.round(total / dpsWindowSec) : 0;
        ...
        const delayCost =
          avgPerCast > 0 && cd > 0 ? Math.round(avgPerCast / cd) : null;
        return {
          name,
          icon: _lookupIcon(name),
          strike: Math.round(d.strike),
          condi: Math.round(d.condition),
          total: Math.round(total),
          dps: skillDps,
          avg: avgPerCast,
          dct: dct ?? -Infinity,
          delay: delayCost ?? -Infinity,
          casts: d.casts,
          hits: d.hits ?? 0,
          _dctNull: dct === null,
          _delayNull: delayCost === null,
        };
      })
    ...
    const condEntries = Object.entries(r.condDamage || {})
      .map(([cond, dmg]) => ({
        cond,
        dmg,
        avgStacks: (r.condAvgStacks || {})[cond] || 0,
      }))
      .filter((e) => e.dmg > 0)
  ...
  _buildChartHtml(r) {
    ...
    let h =
      '<div class="chart-wrap"><div class="chart-title">DPS &amp; Effects Over Time</div>';
    ...
    h +=
      '<div class="chart-canvas-wrap"><canvas id="rotation-chart"></canvas><div class="chart-tooltip" id="rotation-chart-tooltip"></div></div>';
    h += "</div>";
    h += '<div class="chart-panel">';
    h += '<div class="chart-panel-title">Effects</div>';
    h +=
      '<div class="chart-canvas-wrap"><canvas id="rotation-effects-chart"></canvas><div class="chart-tooltip" id="rotation-effects-tooltip"></div></div>';
    h += "</div>";
    h += "</div></div>";
    return h;
  }
  ...
  _bindChartHover() {
    const bind = (canvasId, tooltipId, kind) => {
      ...
      canvas.addEventListener("mousemove", (ev) => {
        ...
        const t = Math.max(
          0,
          Math.min(state.maxTime, ((x - minX) / layout.pw) * state.maxTime),
        );
        const timeLabel = this._formatRelativeSeconds(
          (t - (state.displayTimeOrigin || 0)) / 1000,
          2,
        );
        ...
        const active = state.durationStacksByName[effect]?.some(
          (s) => s.t <= t && s.expiresAt > t,
        );
        this._showChartTooltip(
          tooltip,
  ...
  _drawChart() {
    ...
    const maxTime =
      r.deathTime !== null &&
      r.deathTime !== undefined &&
      r.deathTime < r.rotationMs
        ? r.deathTime
        : r.rotationMs;
    if (maxTime <= 0) return;
    ...
    const activeDurEffects = allEffects.filter(
      (e) => !INTENSITY_EFFECTS.has(e) && toggles[e],
    );
    const barRowH = 8;
    const barGap = 1;
    const durationStacksByName = Object.fromEntries(
      activeDurEffects.map((name) => [
        name,
    ...
    const durationZoneH =
      activeDurEffects.length > 0
        ? activeDurEffects.length * (barRowH + barGap) + 16
        : 0;
    const effectsCssH =
      effectPlotH + effectsPad.top + effectsPad.bottom + durationZoneH;
    ...
    for (const ev of r.log) {
      ...
      if (ev.type === "hit" && ev.strike > 0)
        dmgEvents.push({ t: ev.t, d: ev.strike });
    ...
    let cum = 0;
    let ei = 0;
    const fullSamples = Math.floor(
      Math.max(0, maxTime - dpsStart) / dpsSampleStepMs,
    );
    ...
    if (maxTime > dpsStart) {
      ...
      const elapsedSeconds = Math.max(
        dpsSampleStepMs / 1000,
        (maxTime - dpsStart) / 1000,
      );
      const lastValue =
        fullSamples > 0 ? dpsLine[dpsLine.length - 1].v : cum / elapsedSeconds;
    ...
    const maxDps = Math.max(
      dpsStep,
      Math.ceil(maxDpsValue / dpsStep) * dpsStep,
    );
    ...
    for (let value = 0; value <= maxDps; value += dpsStep) {
      ...
      dpsCtx.beginPath();
      dpsCtx.moveTo(dpsPad.left, y);
      dpsCtx.lineTo(dpsCssW - dpsPad.right, y);
      dpsCtx.stroke();
      dpsCtx.fillStyle = "#888899";
      dpsCtx.font = "10px Segoe UI";
      dpsCtx.textAlign = "right";
    ...
    for (let sec = 0; sec <= Math.ceil(maxTime / 1000); sec += xTickStepSec) {
      ...
      dpsCtx.beginPath();
      dpsCtx.moveTo(x, dpsPad.top);
      dpsCtx.lineTo(x, dpsPad.top + dpsPh);
      dpsCtx.strokeStyle = "rgba(42,42,58,0.35)";
      dpsCtx.stroke();
      dpsCtx.fillStyle = "#888899";
      dpsCtx.textAlign = "center";
      dpsCtx.font = "10px Segoe UI";
      dpsCtx.fillText(
        this._formatRelativeSeconds((t - dpsStart) / 1000, 0),
        x,
        dpsPad.top + dpsPh + 16,
      );
    }
    if (toggles["dps"] !== false) {
      dpsCtx.strokeStyle = "#44bb44";
      dpsCtx.lineWidth = 2.5;
      dpsCtx.beginPath();
      for (let i = 0; i < dpsLine.length; i++) {
        ...
        if (i === 0) dpsCtx.moveTo(x, y);
        else dpsCtx.lineTo(x, y);
      }
      dpsCtx.stroke();
    }
    dpsCtx.fillStyle = "#888899";
    ...
    const stackAxisStep =
      maxStacks > 0
        ? Math.max(1, Math.round(this._getNiceAxisStep(maxStacks, 6)))
        : 1;
    const stackAxisMax =
      maxStacks > 0
        ? Math.max(
            stackAxisStep,
            Math.ceil(maxStacks / stackAxisStep) * stackAxisStep,
          )
        : 1;
    ...
    if (maxStacks > 0) {
      for (let value = 0; value <= stackAxisMax; value += stackAxisStep) {
        const y =
          intensityTop + effectPlotH - (effectPlotH / stackAxisMax) * value;
    ...
    for (const ct of allEffects) {
      ...
      effectsCtx.strokeStyle = col;
      effectsCtx.lineWidth = 1.5;
      effectsCtx.setLineDash([4, 3]);
      effectsCtx.beginPath();
      for (let i = 0; i < line.length; i++) {
        const x = effectsPad.left + (effectsPw / maxTime) * line[i].t;
        const y =
          intensityTop + effectPlotH - (effectPlotH / stackAxisMax) * line[i].v;
        if (i === 0) effectsCtx.moveTo(x, y);
      ...
      if (LABEL_EFFECTS.has(ct)) {
        ...
        for (let i = 0; i < line.length; i++) {
          ...
          const x = effectsPad.left + (effectsPw / maxTime) * line[i].t;
          const y =
            intensityTop + effectPlotH - (effectPlotH / stackAxisMax) * v;
          effectsCtx.fillStyle = "#0c0c14";
    ...
    if (activeDurEffects.length > 0) {
      for (let di = 0; di < activeDurEffects.length; di++) {
        ...
        for (let i = 0; i < n - 1; i++) {
          ...
          const x1 =
            effectsPad.left +
            (effectsPw / maxTime) * Math.min((i + 1) * interval, maxTime);
          effectsCtx.fillStyle = col;
  ...
  async _loadPresets() {
    try {
      const manifest = await fetchJsonAsset(
        "Builds/elementalist/manifest.json",
        {
          optional: true,
        },
      );
      ...
      container.innerHTML = sections
        .map((sec) => {
          const btns = (sec.presets || [])
            .map((p) => {
              const idx = allPresets.length;
              allPresets.push(p);
              const cls = p.upToDate
                ? "btn preset-btn preset-btn--current"
                : "btn preset-btn";
  ...
  async _loadPreset(preset, btn) {
    ...
    try {
      ...
      this.benchmarkInfo =
        preset.dpsReportUrl || preset.benchmarkDps
          ? {
              dpsReportUrl: preset.dpsReportUrl || null,
              benchmarkDps: preset.benchmarkDps || null,
            }
          : null;
  ...
  async _importRotation(file) {
    try {
      ...
      if (!Array.isArray(items))
        throw new Error("No rotation array found in file.");
  ...
  async _importFromDpsReport() {
    ...
    try {
      ...
      if (players.length === 0) {
        this._dpsReportStatus(
          "No Elementalist player found in this log.",
          true,
        );
        return;
      }
  ...
  _runDpsReportConversion() {
    ...
    const skillAttunements = new Map(
      this.data.skills.map((s) => [s.name, s.attunement || ""]),
    );
    const weapons = this.build?.weapons || [];
    const items = convertEIRotation(
      this._eiJson,
      player,
      toolSkillNames,
      skillAttunements,
    ...
    const waitsNote =
      waits > 0
        ? ` (${waits} unknown skill${waits > 1 ? "s" : ""} → __wait)`
        : "";
    this._dpsReportStatus(
      `Appended ${total} steps from "${player.name}"${waitsNote}.`,
    );
  }
