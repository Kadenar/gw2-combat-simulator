...
export function mergeSnapshotIntoState(
  ...
  if (snapshot.build)
    nextState.build = JSON.parse(JSON.stringify(snapshot.build));
  if (snapshot.activeAttunement)
    nextState.activeAttunement = snapshot.activeAttunement;
  if (snapshot.secondaryAttunement)
    nextState.secondaryAttunement = snapshot.secondaryAttunement;
  if ("evokerElement" in snapshot)
    nextState.evokerElement = snapshot.evokerElement;
  if ("evokerStartCharges" in snapshot)
    nextState.evokerStartCharges = snapshot.evokerStartCharges;
  if ("evokerStartEmpowered" in snapshot)
    nextState.evokerStartEmpowered = snapshot.evokerStartEmpowered;
  if (snapshot.hitboxSize) nextState.hitboxSize = snapshot.hitboxSize;
  if ("glyphBoonedElementals" in snapshot)
    nextState.glyphBoonedElementals = !!snapshot.glyphBoonedElementals;
  if ("thornsBossAuraOnly" in snapshot)
    nextState.thornsBossAuraOnly = !!snapshot.thornsBossAuraOnly;
  if ("permaField" in snapshot)
    nextState.permaField = snapshot.permaField || "";
  ...
  if (snapshot.selectedSkills && skillDefs) {
    nextState.selectedSkills = resolveSelectedSkills(
