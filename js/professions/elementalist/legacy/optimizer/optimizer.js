...
export class GearOptimizer {
  ...
  async optimize(config, onProgress) {
    ...
    if (!nonGearCombos.length)
      throw new Error("No valid non-gear combinations to evaluate.");
    ...
    const gearComboCount = this._estimateGearCombos(
      activeSlots,
      space.prefixes,
      slotConstraints,
    );
  ...
  _estimateGearCombos(activeSlots, prefixes, slotConstraints) {
    ...
    const groups = this._buildEquivGroups(
      activeSlots,
      prefixes,
      slotConstraints,
    );
  ...
  _applyCombo(sim, baseBuild, selectedSkills, combo, gear) {
    const testBuild = {
      ...
      infusions:
        combo.infusions != null ? combo.infusions : baseBuild.infusions || [],
    };
    const attrs = calcAttributes(testBuild, selectedSkills);
    sim.attributes = attrs;
    sim.activeTraitNames = new Set(
      (attrs.activeTraits || []).map((t) => t.name),
    );
  }
  ...
  _nonGearCombos(space) {
    ...
    const infusions =
      infStats.length === 0
        ? [null]
        : this._infusionDistributions(
            infStats,
            infTotal,
            space.infusionRanges || [],
          );
    const sigilPairs = this._sigilPairs(
      space.sigils1 || [],
      space.sigils2 || [],
    );
  ...
  _infusionDistributions(stats, total, rawRanges = []) {
    ...
    const ranges = stats.map((stat) => {
      ...
      const max = Math.max(
        min,
        Math.min(total, parseInt(raw.max, 10) || total),
      );
      return { stat, min, max };
    });
...
function _meetsConstraints(attrs, constraints) {
  ...
  if (
    constraints.minBoonDuration != null &&
    (attrs["Boon Duration"]?.final ?? 0) < constraints.minBoonDuration
  )
    return false;
  if (
    constraints.minCritChance != null &&
    (attrs["Critical Chance"]?.final ?? 0) < constraints.minCritChance
  )
    return false;
  if (
    constraints.minToughness != null &&
    (attrs["Toughness"]?.final ?? 0) < constraints.minToughness
  )
    return false;
  if (
    constraints.minVitality != null &&
    (attrs["Vitality"]?.final ?? 0) < constraints.minVitality
  )
    return false;
  return true;
}
