...
self.onmessage = ({ data }) => {
  try {
    ...
    sim.activeTraitNames = new Set(
      (initAttrs.activeTraits || []).map((t) => t.name),
    );
    ...
    self.postMessage({
      ...
      booted: true,
      gearComboCount: gearCombos.length,
      nonGearCombos: combos.length,
    });
    ...
    const localTop = [];
    let topDirty = false;
    const useSchedulerCache = _canCacheScheduler(
      permaBoons,
      prefixes,
      activeSlots,
    );
    ...
    for (const combo of combos) {
      ...
      for (let gi = 0; gi < gearCombos.length; gi++) {
        ...
        if (
          dps > 0 &&
          (localTop.length < TOP_N ||
            dps > localTop[localTop.length - 1].rawDps)
        ) {
          localTop.push({
            rawDps: dps,
            dps,
...
function _evalCapture(
  ...
  const testBuild = _buildTestBuild(baseBuild, combo, gear);
  const attrs = calcAttributes(testBuild, selectedSkills);
  if (!_meetsConstraints(attrs.attributes, constraints))
    return { dps: -1, stream: null };
  sim.attributes = attrs;
