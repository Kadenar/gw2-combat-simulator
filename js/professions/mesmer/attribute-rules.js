...
function activeInstrumentCount(context) {
  ...
  for (const event of instrumentEvents(context)) {
    if (event.at <= context.time + EPSILON && event.expiresAt > context.time) {
      count += 1;
    }
  }
  return count;
...
export const mesmerModifierRules = Object.freeze([
  ...
  },
  {
    ...
    amount: -0.25,
    when: (context) =>
      illusionSource(context) && context.timeline?.furyActiveAt(context.time),
  },
  {
  ...
  },
  {
    ...
    target: MODIFIER_TARGET.STRIKE_DAMAGE,
    operation: "multiply",
    factor: (context) => (context.config.target?.activatingSkills ? 1.25 : 1.5),
    order: 100,
    when: (context) =>
      context.event?.shatter && hasTrait(context, TRAIT.MENTAL_ANGUISH),
  },
  {
    ...
    order: 100,
    when: (context) =>
      context.event?.blade && hasTrait(context, TRAIT.INFINITE_FORGE),
  },
  {
