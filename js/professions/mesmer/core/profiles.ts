...
export function mesmerShatterProfile(
  ...
  return variant(id, parentId, `${name} - Shatter`, {
    ...(shatter.rechargeReductionPerSource == null
      ? {}
      : { rechargeReduction: shatter.rechargeReductionPerSource }),
    effects: [
...
export function mesmerTraitDamageProfile(
  ...
  return trait(id, name, {
    ...
    ...(damage.damageIncrease == null
      ? {}
      : { damageIncrease: damage.damageIncrease }),
    effects: [
      {
        type: "strike",
...
function attackStatusEffect(
  status: MesmerAttackStatus,
  source: "Player" | "Clone",
): SkillEffect {
  return {
    type: "condition",
...
export function mesmerAmbushProfile(
  id: string,
  ...
  return variant(id, attack.id, `${attack.name} - Ambush`, {
    effects: [
      ...
      ...(attack.player.conditions || []).flatMap((status) =>
        Array.from({ length: Number(status.applications || 1) }, () =>
          attackStatusEffect(status, "Player"),
        ),
      ),
      ...(attack.playerBoons || []).map((status) =>
        boonStatusEffect(status, "Player"),
      ),
      {
        type: "strike",
        name: "Clone attack",
      ...
      ...(attack.clone.conditions || []).flatMap((status) =>
        Array.from({ length: Number(status.applications || 1) }, () =>
          attackStatusEffect(status, "Clone"),
        ),
      ),
      ...(attack.cloneBoons || []).map((status) =>
        boonStatusEffect(status, "Clone"),
      ),
...
const SHATTER_PROFILE_BY_SKILL_ID: Readonly<Record<number, string>> =
  Object.freeze({
...
export const MESMER_CORE_BALANCE_PROFILES: readonly BalanceProfile[] =
  Object.freeze([
    {
    ...
    variant(
      MESMER_CORE_BALANCE_PROFILE_IDS.signetOfIllusions,
      ID.SIGNET_OF_ILLUSIONS,
      "Signet of Illusions - Passive",
      { pulseInterval: 10, resourceGain: 1 },
    ),
    variant(
    ...
    variant(
      MESMER_CORE_BALANCE_PROFILE_IDS.signetOfMidnight,
    ...
    variant(
      MESMER_CORE_BALANCE_PROFILE_IDS.clarity,
      ID.MIND_THE_GAP,
      "Clarity",
      { durationMultiplier: 15 },
    ),
    trait(
      MESMER_CORE_BALANCE_PROFILE_IDS.chaoticPersistence,
      "Chaotic Persistence",
      { expertiseBonus: 100, concentrationBonus: 250 },
    ),
    trait(
      MESMER_CORE_BALANCE_PROFILE_IDS.compoundingPower,
      "Compounding Power",
      { maximumStacks: 5, durationMultiplier: 8 },
    ),
    ...
    trait(
      MESMER_CORE_BALANCE_PROFILE_IDS.illusionaryMembrane,
    ...
    trait(
      MESMER_CORE_BALANCE_PROFILE_IDS.maimTheDisillusioned,
    ...
    trait(
      MESMER_CORE_BALANCE_PROFILE_IDS.maliciousSorcery,
      "Malicious Sorcery",
    ...
    trait(
      MESMER_CORE_BALANCE_PROFILE_IDS.masterOfMisdirection,
...
export function mesmerBalanceProfile(
  context: unknown,
  id: SkillId,
...
export function mesmerBalanceValue(
  context: unknown,
  id: SkillId,
  field: string,
  fallback: number,
    ...
    Object.entries(shatters).map(([skillId, shatter]) => {
      ...
      return [
        Number(skillId),
        {
          ...
          balanceProfileId,
          coefficients:
            coefficients.length === shatter.coefficients.length &&
...
function profileStatuses(
  ...
  return (profile?.effects || [])
    ...
    .map((effect) => ({
      name: String(
        type === "condition" ? effect.condition || "" : effect.boon || "",
      ),
      duration: Number(effect.duration || 0),
      stacks: Number(effect.stacks || 1),
      ...(effect.applications == null
        ? {}
...
export function mesmerProfiledAmbush(
  ...
  const vulnerability = (profile?.effects || []).find(
    (effect) =>
      effect.type === "buff" && effect.kind === "target-vulnerability",
  );
  return {
    ...attack,
    balanceProfileId,
    player: {
      ...attack.player,
      coefficient: Number(
        playerStrike?.coefficient ?? attack.player.coefficient,
      ),
      hits: Number(playerStrike?.hits ?? attack.player.hits),
      conditions: profile
        ? profileStatuses(profile, "condition", "Player")
        : attack.player.conditions,
    },
    clone: {
      ...attack.clone,
      ...
      conditions: profile
        ? profileStatuses(profile, "condition", "Clone")
        : attack.clone.conditions,
    },
    playerBoons: profile
      ? profileStatuses(profile, "boon", "Player")
      : attack.playerBoons,
    cloneBoons: profile
      ? profileStatuses(profile, "boon", "Clone")
      : attack.cloneBoons,
...
export function mesmerProfiledTraitDamage(
  ...
  return {
    ...
    damageIncrease: mesmerBalanceValue(
      context,
      balanceProfileId,
      "damageIncrease",
