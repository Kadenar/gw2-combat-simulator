...
export function createSkillEffectController({
  ...
  const handleExceptionalProfile = (
    ...
    for (const group of strikeEffects) {
      ...
      const phantasmTicks =
        isPhantasmStrike &&
        Array.isArray(phantasmTiming?.damageTicks?.[groupName])
          ? phantasmTiming?.damageTicks?.[groupName]
          : null;
      const fixedTicks = group.ticks?.length ? group.ticks : null;
      const packets = phantasmTicks?.length
        ? phantasmTicks
        : fixedTicks;
      ...
      if (packets?.length) {
        ...
        for (let index = 0; index < hits; index += 1) {
          ...
          const packetAt = phantasmTicks?.length
