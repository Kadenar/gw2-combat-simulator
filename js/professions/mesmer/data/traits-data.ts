...
export const ELITE_SPECS = new Set(CATALOG_SPECIALIZATIONS.filter((spec) => spec.elite).map((spec) => spec.name));
export const CORE_SPECS = CATALOG_SPECIALIZATIONS.filter((spec) => !spec.elite).map((spec) => spec.name);
...
export const TRAITS: MesmerTraitRecord[] = CATALOG_SPECIALIZATIONS.flatMap((spec) => {
...
export function getActiveTraits(specializations?: readonly SpecializationSelection[] | null): MesmerTraitRecord[] {
  ...
  for (const spec of specializations || []) {
    ...
    majorTiers.forEach((tier, index) => {
      ...
      if (!pick) return;
      const trait = specTraits.find((candidate) => candidate.tier === tier && candidate.position === pick);
