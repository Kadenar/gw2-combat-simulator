...
function illusionSource(context: Gw2ModifierContext): boolean {
  return (
    context.event?.source === "Clone" || context.event?.source === "Phantasm"
  );
}

function timedStacks(
...
function timedActive(context: Gw2ModifierContext, kind: string): boolean {
