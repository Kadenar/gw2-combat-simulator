...
const petSkillIds = new Set<SkillId>(
  RANGER_PET_SKILLS.map((skill) => skill.id),
);
...
const DRUID_PROFESSION_SKILLS = Object.freeze([
  ID.CELESTIAL_AVATAR,
...
const SOULBEAST_PROFESSION_SKILLS = Object.freeze(
  apiSkills
    .filter(
      (skill) =>
        skill.type === "Profession" && !reservedProfessionIds.has(skill.id),
    )
    .map((skill) => skill.id),
);
...
function normalize(skill: RangerSkill): RangerSkill {
  ...
  return {
    ...
    unleashedHammerSkill:
      ...
      skill.weapon === "Hammer" &&
      skill.name.startsWith("Unleashed "),
  };
}
