...
export const RANGER_HAMMER_VARIANT_PAIRS: readonly (readonly [
  number,
  number,
])[] = Object.freeze([
  Object.freeze([ID.WILD_SWING, ID.UNLEASHED_WILD_SWING]) as readonly [
    number,
    number,
  ],
  Object.freeze([
...
export function normalizeRangerHammerSkillIds(value: unknown): number[] {
  ...
  return RANGER_HAMMER_VARIANT_PAIRS.map((pair, index) =>
    pair.includes(source[index])
      ? source[index]
      : DEFAULT_RANGER_HAMMER_SKILL_IDS[index],
  );
}
