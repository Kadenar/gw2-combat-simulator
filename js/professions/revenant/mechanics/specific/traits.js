...
const TWIN_MOON_SKILL_IDS = new Set([
  ID.TWIN_MOON_SWEEP,
...
function materializeThrillOfCombat(context, event) {
...
function consumeBattleScar(context, event) {
  const profile = MECHANICS.battleScars;
  const state = context.state.profession;
...
function isLegendaryStanceSkill(skill) {
...
function applyRenegadeCriticalTraits(context, event) {
  ...
  if (
    ...
    if (
      ...
      context.emitDerived(event, {
        ...
        name: "Enchanted Daggers â€” Siphon Damage",
        coefficient: 0,
    ...
    if (
      ...
      context.emitDerived(event, {
        ...
        name: "Razorclaw's Rage â€” Bleeding",
        condition: "Bleeding",
        stacks: 1,
