...
function selectedDodgeSkill(
  context: RevenantSchedulerContext,
): RevenantSkill | undefined {
  return skillById(
    context,
    vindicatorState.from(context).selectedDodge === "Imperial Impact"
      ? ID.IMPERIAL_IMPACT
      : ID.DEATH_DROP,
  );
}
...
export function performEnergyMeld(
  ...
  if (hasRevenantTrait(context.config, TRAIT.REAVERS_CURSE)) {
    const reaversCurse = balanceProfileById(
      context,
      VINDICATOR_BALANCE_PROFILE_IDS.reaversCurse,
    );
    const effect = reaversCurse?.effects?.find(
      (candidate) => candidate.type === "buff",
    );
  ...
  if (
    ...
    const angsiyansTrust = balanceProfileById(
      context,
      VINDICATOR_BALANCE_PROFILE_IDS.angsiyansTrust,
    );
  ...
  if (songOfArboreum) {
    const vigor = enduranceProfile?.effects?.find(
...
export function completeVindicatorDodge(
  ...
  const effect = profile?.effects?.find(
    (candidate) => candidate.type === "strike",
  );
  ...
  const reaversCurseProfile = balanceProfileById(
    context,
    VINDICATOR_BALANCE_PROFILE_IDS.reaversCurse,
  );
  ...
  context.emit({
    ...
    coefficient:
      ...
      (reaversCurse
        ? Math.max(0, Number(reaversCurseProfile?.damageMultiplier || 1))
        : 1),
  ...
  if (
    ...
    const forerunner = balanceProfileById(
      context,
      VINDICATOR_BALANCE_PROFILE_IDS.forerunnerOfDeath,
    );
    const forerunnerEffect = forerunner?.effects?.find(
      (candidate) => candidate.type === "buff",
    );
