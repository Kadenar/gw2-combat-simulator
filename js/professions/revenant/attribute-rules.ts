...
function player(context: RevenantModifierContext): boolean {
...
function affinity(context: RevenantModifierContext): number {
...
function kallasFervorStacks(context: RevenantModifierContext): number {
...
function targetHealthFraction(context: RevenantModifierContext): number {
...
function targetVulnerability(context: RevenantModifierContext): number {
  return Number(
...
const DAMAGING_CONDITIONS = new Set<string>([
  "Bleeding",
  "Burning",
  "Confusion",
  "Poisoned",
...
function activeBoonCount(context: RevenantModifierContext): number {
...
function playerHealthFraction(context: RevenantModifierContext): number {
  return Number(context.config?.playerHealthFraction ?? 1);
}
function vindicatorEnduranceIsNotFull(
  context: RevenantModifierContext,
): boolean {
...
function forerunnerOfDeathIsActive(
  context: RevenantModifierContext,
): boolean {
...
const rules: readonly Gw2ModifierRule[] = [
  {
...
function modifyConditionDuration(
  context: RevenantModifierContext,
  duration: number,
): number {
