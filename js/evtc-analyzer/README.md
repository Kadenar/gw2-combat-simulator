...
The `evtc-analyzer` parses ArcDPS EVTC combat logs and reconstructs recorded player actions as rotations that can be
loaded into the Guild Wars 2 Combat Simulator.

Unlike the `dps-report-analyzer`, which works from summarized Elite Insights JSON, this analyzer operates directly on
the raw EVTC event stream. That gives it access to animation events, state changes, buff transitions, effect packets,
weapon swaps, combat state, and other information that may no longer be present in a processed report.
