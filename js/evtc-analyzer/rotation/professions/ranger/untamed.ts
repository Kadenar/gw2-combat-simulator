...
const signalsByRawId = new Map(
  UNLEASHED_PET_SIGNALS.map((signal) => [signal.rawSkillId, signal]),
);
...
function nearestUnleashRanger(
  ...
  const closest = [...transitions].sort(
    (left, right) => Math.abs(left.start - time) - Math.abs(right.start - time),
  )[0];
  return closest && Math.abs(closest.start - time) <= PET_TRANSITION_WINDOW_MS
    ? closest
    : null;
}
...
function unleashedPetActions(
  ...
  const transitions = actions.filter((action) =>
  ...
  return context.log.events.flatMap((event, eventIndex) => {
    ...
    const start = transition
      ? transition.start - (UNLEASHED_PET_SIGNALS.length - signal.order)
      : event.time;
    return [
      directAction(
        eventIndex,
...
function initialUtilityPrecasts(
  ...
  const initialBuffs = context.log.events.filter(
    (event) =>
      event.stateChange === EVTC_STATE_CHANGE.BUFF_INITIAL && event.buff !== 0,
  );
  const hasPlayerBoon = initialBuffs.some(
    (event) =>
      event.source === context.playerAddress &&
      (event.target === context.playerAddress ||
        event.sourceMasterInstance === playerInstance(context)),
  );
  const identities = [
    ...(selectedSkill(context, STRENGTH_OF_THE_PACK) && hasPlayerBoon
      ? [STRENGTH_OF_THE_PACK]
      : []),
    ...(selectedSkill(context, WE_HEAL_AS_ONE) && hasPlayerBoon
      ? [WE_HEAL_AS_ONE]
      : []),
  ];
...
export function reconstructUntamedActions(
  ...
  actions = [
    ...actions,
    ...unleashedPetActions(context, actions),
    ...initialExplodingSpores(context, actions),
  ];
  return [...actions, ...initialUtilityPrecasts(context, actions)].map(
    (action) => {
      ...
      const skill = rangerSkill(
        context,
        UNLEASHED_OVERBEARING_SMASH.skillId,
        UNLEASHED_OVERBEARING_SMASH.name,
      );
      return skill
        ? {
            ...action,
