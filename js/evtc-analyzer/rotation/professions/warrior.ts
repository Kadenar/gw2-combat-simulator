...
const specializationAnalyzers: ReadonlyMap<string, WarriorActionTransform> =
  new Map([
...
export function reconstructWarriorProfessionActions(
  ...
  const actions = normalizeWarriorCommonActions(
    context,
  ...
  return removePostEncounterWarriorActions(
    context,
    analyzer?.(context, actions) || actions,
  );
}
