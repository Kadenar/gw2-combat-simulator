...
export function detectedWarriorCorePrecastIdentity(
  ...
  return detectedWarriorCorePrecast(context, id)
    ? WARRIOR_CORE_ACTIONS[id]
    : null;
}
...
export function normalizeWarriorCommonActions(
  ...
  for (const action of actions) {
    ...
    const commitMs = skill
      ? firstStrikePacketOffsetMs(skill, runtimeCastDurationMs(skill))
      : null;
    const duration = Math.max(0, action.end - action.start);
    const committedStrike =
      skill != null && commitMs != null && validatePackets(action).anyObserved;
    if (
      autoattack &&
      !committedStrike &&
      commitMs != null &&
    ...
    if (
      ...
      commitMs != null &&
      (duration >= commitMs || committedStrike)
    ) {
      const replayDuration = Math.max(
        duration,
        skill ? runtimeCastDurationMs(skill) : 0,
      );
      normalized.push({
        ...action,
