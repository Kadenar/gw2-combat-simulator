...
function bladeswornPrecasts(
  ...
  const firstTrigger = actions
    .filter(
      (action) =>
        action.rawSkillId === DRAGON_TRIGGER.skillId ||
        action.canonicalSkillId === DRAGON_TRIGGER.skillId,
    )
    .sort(
      (left, right) =>
...
function removeAutomaticDragonTriggerUnsheathes(
  ...
  return actions.filter(
    (action) =>
      !(
        (action.rawSkillId === UNSHEATHE_GUNSABER.skillId ||
          action.canonicalSkillId === UNSHEATHE_GUNSABER.skillId) &&
        triggers.some(
          (trigger) =>
            Math.abs(trigger.start - action.start) <= SIGNAL_WINDOW_MS,
        )
      ),
  );
}
...
function flowStabilizerActions(
  ...
  for (const signal of signals) {
    ...
    if (
      !current ||
      signal.event.time - current[0].event.time > SIGNAL_WINDOW_MS
    ) {
      groups.push([signal]);
    } else {
      current.push(signal);
  ...
  return groups.flatMap((group) => {
    ...
    if (
      group.length < 2 ||
      hasActionNear(actions, FLOW_STABILIZER, signal.event.time)
    ) {
      return [];
    }
    return [
...
function triggerguardActions(
  ...
  return context.log.events.flatMap((event, eventIndex) => {
    if (
      ...
      event.buffRemove !== 0 ||
      !isBuffApplication(event.stateChange) ||
      Math.max(event.value, event.buffDamage) !==
        TRIGGERGUARD_AEGIS_DURATION_MS ||
...
function flickerStepActions(
  ...
  return context.log.events.flatMap((event, eventIndex) => {
    ...
    const trigger = [...triggers]
      ...
      .find(
        (candidate) =>
          candidate.start <= event.time &&
          event.time <= candidate.end + PEITHA_TRIGGER_GRACE_MS,
      );
