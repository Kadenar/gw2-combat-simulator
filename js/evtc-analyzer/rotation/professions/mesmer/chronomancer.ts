...
function shatterSignals(
  ...
  return effects.length
    ? { signals: effects, maximumClusterSize: MAXIMUM_SHATTER_SOURCES }
    : { signals: direct };
}
...
function missingMirrorImagesActions(
  actions: readonly EvtcRecordedRotationAction[],
): EvtcRecordedRotationAction[] {
  const mirrors = actions
    .filter(
      (action) =>
        action.rawSkillId === MIRROR_IMAGES.skillId ||
        action.canonicalSkillId === MIRROR_IMAGES.skillId,
    )
  ...
  const shatters = actions
    .filter((action) =>
      RESOURCE_SHATTER_IDS.has(
        Number(action.canonicalSkillId ?? action.rawSkillId),
      ),
    )
    .sort((left, right) => left.start - right.start);
      ...
      const candidates = shatters
        ...
        .filter(
          ({ earlier, later }) =>
            ...
            ![...mirrors, ...inferred].some(
              (mirror) =>
                mirror.start > earlier.start && mirror.start < later.start,
            ),
        )
        .sort(
          (left, right) =>
