...
function shatterActions(
  ...
  const crySignals = primaryShatterSignals(
    context,
    CRY_OF_FRUSTRATION,
    MESMER_EFFECT_GUIDS.cryOfFrustration,
    750,
  );
  const mindSignals = primaryShatterSignals(
    context,
    MIND_WRACK,
    MESMER_EFFECT_GUIDS.distortionOrMindWrack,
    1250,
  );
  const distortionTimes = clusterSignals(
    buffGainSignals(context, DISTORTION_BUFF),
    500,
  ...
  const diversionSignals = clusterSignals(
    effectSignals(context, MESMER_EFFECT_GUIDS.diversion),
    750,
  ).filter(
    (signal) =>
      !otherShatterTimes.some(
        (time) => signal.event.time >= time && signal.event.time - time <= 1000,
      ),
  );
  return [
...
function mirageCloakActions(
  ...
  const mirrorSignals = effectSignals(
    context,
    MESMER_EFFECT_GUIDS.mirageMirror,
  );
  ...
  return buffGainSignals(context, MIRAGE_CLOAK_BUFF, true)
    ...
    .flatMap((signal) => {
      const mirror = mirrorSignals.some(
        (candidate) => Math.abs(candidate.event.time - signal.event.time) <= 50,
      );
      ...
      return [
        canonicalAction(
          ...
          signal.event.stateChange === EVTC_STATE_CHANGE.BUFF_INITIAL
            ...
            : "buff-transition",
          signal.event.stateChange === EVTC_STATE_CHANGE.BUFF_INITIAL
            ? { initialState: true, precast: true }
            : {},
        ),
      ];
    });
