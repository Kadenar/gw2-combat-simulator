...
function mechCommandActions(
  ...
  context.log.events.forEach((event, eventIndex) => {
    ...
    if (
      !identity ||
      event.sourceMasterInstance !== instance ||
      event.source === context.playerAddress
    ) {
      return;
    }
    ...
    if (
      event.value > 0 &&
      combatStart != null &&
      inferredStart < combatStart &&
      event.time <= combatStart + 1200
    ) {
      actions.push(
        canonicalAction(
          eventIndex,
...
function openingActions(
  ...
  const openingCore = commands.find(
    (action) =>
      action.rawName === "Core Reactor Shot" && action.start === opening.start,
  );
  if (openingCore) {
