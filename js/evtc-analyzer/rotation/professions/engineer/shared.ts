...
export function selectedIdentity(
  ...
  const selectedIds = new Set(
    (context.selectedSkillIds || []).map(Number).filter(Number.isFinite),
  );
  const selected = context.catalog?.skills.find(
    (skill) =>
      typeof skill.id === "number" &&
      selectedIds.has(Number(skill.id)) &&
      normalized(skill.name) === normalized(name),
  );
...
export function selectedSkill(
  ...
  return (
    ...
    context.selectedSkillNames.some(
      (selected) => normalized(selected) === normalized(name),
    )
  );
}
...
export function finalizeEngineerActions(
  ...
  return inEncounter.map((action) => ({
    ...
    suppressFollowingWait:
      action.suppressFollowingWait ??
      !(action.precast === true && action.rawName === "Throw Mine"),
  }));
}
