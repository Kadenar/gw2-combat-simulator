...
export function kitIdentity(
  ...
  const skill = context.catalog?.skills.find((candidate) =>
    stow
      ? normalized(candidate.kit) === normalized(kit) && candidate.handlerId === 'engineer.kit-stow'
      : normalized(candidate.kitName) === normalized(kit) && candidate.handlerId === 'engineer.kit-equip'
  );
  return skill && typeof skill.id === 'number' ? { name: skill.name, skillId: Number(skill.id) } : null;
}

export function inferDetonateActions(context: EvtcProfessionReconstructionContext): EvtcRecordedRotationAction[] {
...
export function normalizeKitTransitions(
  ...
  for (let index = 0; index < sorted.length; index += 1) {
    ...
    const nextKit = nextKitAction ? String(skillForAction(context, nextKitAction)?.kit || '') : '';
