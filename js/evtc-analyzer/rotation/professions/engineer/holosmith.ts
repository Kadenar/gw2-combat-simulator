...
function preserveOverheatBoundaries(
  ...
  for (const time of overheatTimes) {
    ...
    const preceding = actions
      ...
      .sort(
        (left, right) =>
          right.end - left.end || right.eventIndex - left.eventIndex,
      )[0];
    if (preceding) preserve.add(preceding);
  }
  return actions.map((action) => {
    if (complete.has(action)) return { ...action, forceCompleteReplay: true };
    if (preserve.has(action))
      return { ...action, suppressFollowingWait: false };
    return action;
  });
}
...
function normalizeHolosmithTransitions(
  ...
  for (let index = 0; index < sorted.length; index += 1) {
    ...
    const forgeSwap = swaps.find(
      (swap) => Number(swap.weaponSet) === FORGE_WEAPON_SET,
    );
    if (forgeSwap) {
      result.push(
        canonicalAction(
          forgeSwap.eventIndex,
          forgeSwap.start,
          ENGAGE_FORGE,
          0,
          "state-change",
    ...
    const normalSwap = swaps.find((swap) =>
      NORMAL_WEAPON_SETS.has(Number(swap.weaponSet)),
    );
    if (forgeActive && normalSwap) {
      if (!isOverheatTransition(context, normalSwap.start)) {
        result.push(
          canonicalAction(
            normalSwap.eventIndex,
            normalSwap.start,
            DEACTIVATE_FORGE,
            0,
            "state-change",
...
function inferDirectInputs(
  ...
  return context.log.events.flatMap((event, eventIndex) => {
    ...
    const skill = findRotationSkill(
      event.skillId,
      context.log.skills.find((candidate) => candidate.id === event.skillId)
        ?.name || "",
      context.catalog,
      context.profile,
    );
    ...
    if (
      previous != null &&
      event.time - previous < Math.max(1, effectWindowMs(skill))
    ) {
      return [];
    }
...
export function reconstructHolosmithActions(
  ...
  const actions = normalizeHolosmithTransitions(
    context,
