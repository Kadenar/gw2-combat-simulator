...
function parserForProfile(
  profile: EvtcRotationProfessionProfile,
): EvtcProfessionRotationParser {
...
export const EVTC_PROFESSION_ROTATION_PARSERS: readonly EvtcProfessionRotationParser[] =
  Object.freeze(EVTC_ROTATION_PROFILES.map(parserForProfile));

const parsersById = new Map(
  EVTC_PROFESSION_ROTATION_PARSERS.map((parser) => [parser.id, parser]),
);
...
export function reconstructEvtcRotation(
  ...
  if (options.playerAddress != null) {
    ...
    try {
      address =
        typeof options.playerAddress === "bigint"
          ? options.playerAddress
          : BigInt(options.playerAddress);
    } catch {
      address = null;
    }
    player = players.find(
      (candidate) => address != null && BigInt(candidate.address) === address,
    );
  } else if (
    players.length > 1 &&
  ...
  if (!player) {
    if (!players.length) {
      throw new EvtcError(
        "NO_PLAYER",
        "The EVTC log contains no known player.",
      );
    }
    throw new EvtcError(
  ...
  const parser = getEvtcProfessionRotationParser(
    player.professionId,
