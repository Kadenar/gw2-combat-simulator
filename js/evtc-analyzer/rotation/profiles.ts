...
export interface ProfessionProfileSource {
  ...
  >;
  readonly initialSummons?: readonly EvtcRotationInitialSummon[];
  readonly initialSummonsBySpecialization?: Readonly<
    Record<string, readonly EvtcRotationInitialSummon[]>
  >;
  readonly inferCombatStartFromFirstCast?: boolean;
}
      ...
      Object.entries(source.specializations).map(
        ([specializationId, specializationName]) =>
          Object.freeze({
            ...
            inferCombatStartFromFirstCast:
              source.inferCombatStartFromFirstCast === true,
          }),
      ),
    ),
  );
