...
export function parseEvtc(input: ArrayBuffer | Uint8Array): ParsedEvtc {
  ...
  if (magic !== "EVTC") {
    throw new EvtcError(
      "INVALID_MAGIC",
      "The file does not contain EVTC data.",
    );
  }
  ...
  if (!/^\d{8}$/.test(arcdpsBuild)) {
    throw new EvtcError(
      "INVALID_MAGIC",
      "The EVTC header has an invalid ArcDPS build identifier.",
    );
  }
  ...
  if (rawRevision !== 0 && rawRevision !== 1) {
    throw new EvtcError(
      ...
      `EVTC combat-event revision ${rawRevision} is not supported.`,
  ...
  if (agentCount > EVTC_PARSE_LIMITS.maximumAgents) {
    throw new EvtcError(
      "LIMIT_EXCEEDED",
      `The EVTC agent count (${agentCount}) exceeds the safety limit.`,
      { agentCount },
    );
  }
  ensureRange(
  ...
  for (let index = 0; index < agentCount; index += 1) {
    ...
    const names = decoder
      .decode(bytes.subarray(record + 28, record + 92))
      .split("\0");
    agents.push({
  ...
  if (skillCount > EVTC_PARSE_LIMITS.maximumSkills) {
    throw new EvtcError(
      "LIMIT_EXCEEDED",
      `The EVTC skill count (${skillCount}) exceeds the safety limit.`,
      { skillCount },
    );
  }
  ensureRange(
  ...
  if (eventCount > EVTC_PARSE_LIMITS.maximumEvents) {
    throw new EvtcError(
      "LIMIT_EXCEEDED",
      `The EVTC combat-event count (${eventCount}) exceeds the safety limit.`,
      { eventCount },
    );
  }
