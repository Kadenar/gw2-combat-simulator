  ...
  while (true) {
    ...
    if (total > maximumBytes) {
      ...
      throw new EvtcError(
        ...
        "The expanded EVTC file exceeds the 512 MiB safety limit.",
      );
    }
    chunks.push(value);
  }
...
async function expandZip(bytes: Uint8Array): Promise<Uint8Array> {
  ...
  if (entryCount !== 1) {
    throw new EvtcError(
      "INVALID_ZIP",
      "An EVTC ZIP must contain exactly one file entry.",
      { entryCount },
    );
  }
  if (
    centralOffset + centralSize > end ||
    signature(view, centralOffset) !== ZIP_CENTRAL_HEADER
  ) {
  ...
  if ([compressedSize, expandedSize, localOffset].includes(0xffffffff)) {
    throw new EvtcError(
      "INVALID_ZIP",
      "ZIP64 EVTC archives are not supported.",
    );
  }
  if ((flags & 1) !== 0) {
    throw new EvtcError(
      "INVALID_ZIP",
      "Encrypted EVTC ZIP entries are not supported.",
    );
  }
  ...
  if (
    compressedSize > 0 &&
    expandedSize / compressedSize > EVTC_FILE_LIMITS.maximumExpansionRatio
  ) {
    throw new EvtcError(
      "ZIP_BOMB",
      "The ZIP expansion ratio exceeds the EVTC safety limit.",
    );
  }
  if (signature(view, localOffset) !== ZIP_LOCAL_HEADER) {
    throw new EvtcError(
      "INVALID_ZIP",
      "The ZIP local entry header is invalid.",
    );
  }
  ...
  if (
    ...
    payloadOffset + compressedSize > bytes.byteLength
  ) {
    throw new EvtcError(
      "INVALID_ZIP",
  ...
  if (compressionMethod === 0) {
    ...
    expanded = await inflateRaw(
      compressed,
      EVTC_FILE_LIMITS.maximumExpandedBytes,
    );
  } else {
    throw new EvtcError(
      "UNSUPPORTED_COMPRESSION",
  ...
  if (crc32(expanded) !== expectedCrc) {
    throw new EvtcError(
      "INVALID_ZIP",
      "The EVTC ZIP entry failed its CRC check.",
    );
  }
  return expanded;
}

export async function decompressEvtcInput(
  ...
  if (isZip && bytes.byteLength > EVTC_FILE_LIMITS.maximumCompressedBytes) {
    throw new EvtcError(
      "FILE_TOO_LARGE",
      "The selected file exceeds the 64 MiB compressed-file safety limit.",
    );
  }
  if (!isZip && bytes.byteLength > EVTC_FILE_LIMITS.maximumExpandedBytes) {
    throw new EvtcError(
      "EXPANDED_SIZE_EXCEEDED",
      "The EVTC file exceeds the 512 MiB expanded-file safety limit.",
    );
  }
