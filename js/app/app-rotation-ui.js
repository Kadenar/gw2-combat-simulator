...
const RELIC_ICONS = {
  "Relic of Akeem":
    "https://render.guildwars2.com/file/594C437E9606A167F4F372BCEB0C2B7C7828037B/3122330.png",
  "Relic of Aristocracy":
    "https://render.guildwars2.com/file/BCC01F0B6616FE26ED4BE159532A6A6FBD0EA2D8/3122332.png",
  "Relic of the Eagle":
    "https://render.guildwars2.com/file/DFF4EB43AD0803F60D105658052321A0BE1AF02C/3592832.png",
  "Relic of Fireworks":
    "https://render.guildwars2.com/file/2999CCF7C94267B2EE3DDA7459050864622927C9/3122349.png",
  "Relic of the Fractal":
    "https://render.guildwars2.com/file/B2D409644147BF18935A95A52505ABCB9EECE142/3122351.png",
  "Relic of Peitha":
    "https://render.guildwars2.com/file/949A6A4179F514FCDEF3AC3D9C292B38D5E0047D/3122365.png",
  "Relic of the Thief":
    "https://render.guildwars2.com/file/3523AC08EB04347CF371E9A91F4B985D12FB4ED3/3122371.png",
  "Relic of Thorns":
    "https://wiki.guildwars2.com/images/8/8a/Relic_of_Thorns.png",
  "Relic of the Claw":
    "https://wiki.guildwars2.com/images/7/7f/Relic_of_the_Claw.png",
};
const EFFECT_COLORS = {
  Bleeding: "#d84b4b",
  Burning: "#f28b3c",
...
function applyTimelineDrop(app, insertAt) {
  ...
  if (drag.source === "timeline") {
    if (!moveRotationEntry(app.build.rotation, drag.index, insertAt))
      return false;
    app.changed(false);
    return true;
  }
  ...
  if (drag.source === "palette") {
    ...
    const boundedTarget = Math.max(
      0,
      Math.min(insertAt, app.build.rotation.length),
    );
  ...
  return `<div class="pal-group active-resource-group">
        ...
        <div class="active-resource" data-resource-count="${value}" title="${esc(title)}"
            ...
            <div class="active-resource-pips">${Array.from(
              ...
              (_, index) =>
                `<span class="active-resource-pip${index < value ? " active" : ""}"></span>`,
            ).join("")}</div>
...
export function renderStartResource(app) {
  ...
  const definition =
    app.results?.endState?.resourceDefinition ||
  ...
  element.querySelectorAll(".resource-pip").forEach((button) => {
    button.addEventListener("click", () => {
      ...
      app.build.initialResource =
        count === app.build.initialResource ? count - 1 : count;
      app.changed();
    });
  });
...
export function renderPalette(app) {
  ...
  const mechanicIds =
    professionGroups.find((group) => group.id === "profession")?.skillIds || [];
  ...
  const actions = app.skills.filter(
    (skill) =>
      skill.type === "Action" &&
  ...
  element.querySelectorAll(".pal-skill").forEach((icon) => {
    ...
    icon.addEventListener("click", (event) => {
      ...
      const instant =
        name === "__combat_start" || Number(skill?.activation || 0) === 0;
...
export function renderTimeline(app) {
  ...
  let timelineHtml = rows
    .map((row, rowNumber) => {
      ...
      row.skills.forEach(({ entry, index }) => {
        ...
        const time =
          step && !invalid ? `${(step.start / 1000).toFixed(2)}s` : "";
  ...
  if (procSteps.length) {
    ...
    const procs = procSteps
      .filter((proc) => procVisibility.has(procFilterKey(proc)))
      .map((proc) => {
        const traitIcon =
          proc.type === "trait_proc"
            ...
            : "";
        const relicIcon =
          proc.type === "relic_proc" ? RELIC_ICONS[proc.skill] : "";
        const sourceIcon = app.skillByName.get(proc.sourceSkill)?.icon;
        const icon =
          proc.icon || traitIcon || relicIcon || sourceIcon || PLACEHOLDER_ICON;
        const type =
          proc.type === "relic_proc"
            ? "Relic"
            : proc.type === "skill_proc"
  ...
  element
    .querySelectorAll(".rot-row:not(.rot-procs-row) > .rot-row-skills")
    .forEach((row) => {
...
export function simulationEventLogRows(result, build = null) {
  ...
  const maximumResource = Number(
    result?.endState?.resourceDefinition?.maximum || 0,
  );
  const push = (
    at,
    type,
...
export function renderEventLog(app) {
  ...
  const wasPhantasmOnly =
    element.querySelector(".log-filter-phantasm")?.checked ?? false;
  ...
  element
    .querySelector(".log-filter-phantasm")
    ?.addEventListener("change", (e) => {
      element
        .querySelector(".res-log")
        ?.classList.toggle("log-phantasm-only", e.target.checked);
    });
  element.querySelector(".btn-csv-export")?.addEventListener("click", () => {
    const blob = new Blob([simulationEventLogCsv(eventLog)], {
      type: "text/csv",
    });
...
export function buildChartSeries(result, sampleStepMs = 250) {
  ...
  const strikeEvents = resolved.filter(
    (event) => event.type === "damage" && Number(event.damage || 0) > 0,
  );
  const conditionEvents = resolved.filter(
    (event) => event.type === "condition" && Number(event.damage || 0) > 0,
  );
  const dpsStartMs =
...
function resultIcon(app, row) {
  ...
  const relicName = Object.keys(RELIC_ICONS).find((name) =>
...
function drawLineChart(canvas, lines, durationMs, height = 260) {
  ...
  if (!lines.length) {
    ...
    context.textAlign = "center";
    context.fillText(
      "No timed effects in this rotation",
...
function bindCharts(element, series) {
  ...
  const redraw = () => {
    const selected = new Set(
      [...element.querySelectorAll(".chart-toggles input:checked")].map(
    ...
    const effectLines = Object.entries(series.effects)
      ...
      .map(([name, points], index) => ({
        ...
        points,
        color:
          EFFECT_COLORS[name] || `hsl(${(index * 61 + 210) % 360} 62% 62%)`,
      }));
  ...
  const bindHover = (canvasId, tooltipId, kind) => {
    ...
    canvas.addEventListener("mousemove", (event) => {
      ...
      if (kind === "dps") {
        ...
        body = entries.length
          ? entries
              .map((entry) => `<div>${esc(entry.name)}: ${entry.value}</div>`)
              .join("")
  ...
  element
    .querySelectorAll(".chart-toggles input")
...
function applySkillBreakdownSort(element, app, number) {
  ...
  const rowsEl = element.querySelector(
    ".mesmer-skill-breakdown .res-skill-rows",
  );
  if (rowsEl)
    rowsEl.innerHTML = sorted
      .map((row) => renderSkillRow(app, row, number))
      .join("");

  const hdr = element.querySelector(
    ".mesmer-skill-breakdown .res-hdr-sortable",
  );
  if (hdr) {
