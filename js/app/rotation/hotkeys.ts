...
export type RotationHotkeyAction =
  (typeof ROTATION_HOTKEY_ACTIONS)[number]["id"];
...
const actionIds = new Set<string>(
  ROTATION_HOTKEY_ACTIONS.map((action) => action.id),
);
...
export function defaultRotationHotkeyBindings(): RotationHotkeyBindings {
  return Object.fromEntries(
    ROTATION_HOTKEY_ACTIONS.map(({ id, code }) => [id, code]),
  ) as RotationHotkeyBindings;
}
...
export function normalizeRotationHotkeyBindings(
  value: unknown,
  ...
  for (const action of ROTATION_HOTKEY_ACTIONS) {
    ...
    if (
      Object.prototype.hasOwnProperty.call(input, action.id) &&
      validKeyboardCode(candidate)
    ) {
...
export function loadRotationHotkeyBindings(
  storage: HotkeyStorage | null = browserStorage(),
): RotationHotkeyBindings {
  if (!storage) return defaultRotationHotkeyBindings();
  try {
    return normalizeRotationHotkeyBindings(
      JSON.parse(storage.getItem(ROTATION_HOTKEY_STORAGE_KEY) || "null"),
    );
  } catch {
...
export function saveRotationHotkeyBindings(
  ...
  try {
    storage.setItem(
      ...
      JSON.stringify(normalizeRotationHotkeyBindings(bindings)),
    );
  } catch {
...
export function rotationUtilityHotkeyAction(
  index: number,
): RotationHotkeyAction | "" {
  const action = `slot-${index + 6}`;
  return index >= 0 && index < 5 && actionIds.has(action)
    ? (action as RotationHotkeyAction)
    : "";
}
...
export function rotationHotkeyActionForCode(
  ...
  return (
    ROTATION_HOTKEY_ACTIONS.find(
      (action) => bindings[action.id] && bindings[action.id] === code,
    )?.id || null
  );
}
...
function currentHotkeyTarget(
  root: HTMLElement,
  action: RotationHotkeyAction,
): HTMLElement | null {
  const candidates = [
    ...root.querySelectorAll<HTMLElement>(".pal-skill[data-hotkey-action]"),
  ].filter((candidate) => candidate.dataset.hotkeyAction === action);
  return (
    candidates.find(
      (candidate) =>
        !candidate.classList.contains("pal-context-disabled") &&
        !candidate.classList.contains("pal-concealed"),
    ) || null
  );
}

function activateRotationHotkey(
...
function refreshHotkeyBadges(controller: RotationHotkeyController): void {
  for (const skill of controller.root.querySelectorAll<HTMLElement>(
    ".pal-skill[data-hotkey-action]",
  )) {
...
function populateDialog(
  ...
  const error = controller.dialog.querySelector<HTMLElement>(
    ".rotation-hotkey-error",
  );
  if (error) {
    error.hidden = true;
    error.textContent = "";
...
function ensureDialog(controller: RotationHotkeyController): void {
  ...
  for (const input of dialog.querySelectorAll<HTMLInputElement>(
    ...
    input.addEventListener("keydown", (event) => {
      ...
      const code = ["Backspace", "Delete"].includes(event.code)
        ? ""
        : event.code;
  ...
  dialog
    .querySelector("[data-hotkey-cancel]")
