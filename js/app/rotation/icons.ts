...
export function resolveProcIcon(
  ...
  return String(
    proc.icon || traitIcon || relicIcon || procSkillIcon || sourceIcon || "",
  );
}
...
export function resultSkillIcon(
  app: ProfessionAppState,
  row: ResultIconRow,
): string {
  ...
  const relicIcon =
    resolveRelicIcon(row.name) ||
    resolveRelicIcon(row.sourceSkill) ||
    resolveRelicIcon(row.sourceId);
