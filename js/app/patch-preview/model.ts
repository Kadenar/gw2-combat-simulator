...
export function numericEditForValue(
  liveValue: number,
  previewValue: number,
): NumEdit | undefined {
  return Object.is(liveValue, previewValue)
    ? undefined
    : { from: liveValue, to: previewValue };
}
...
function fieldLabel(field: string): string {
  return (
    FIELD_LABELS[field] ||
    field.replaceAll(/([a-z])([A-Z])/g, "$1 $2").toLocaleLowerCase()
  );
}
...
function effectTarget(patch: EffectPatch): string {
  ...
  return patch.tickIndex == null
    ? selector
    : `${selector} tick ${String(patch.tickIndex)}`;
}
...
function skillPatchSummary(edit: SkillPatchEdit): string {
  ...
  for (const effect of edit.effects || []) {
    for (const field of PATCHABLE_EFFECT_NUMERIC_FIELDS) {
      ...
      if (numericEdit != null) {
        changes.add(
          `${effectTarget(effect)} ${describeNumEdit(field, numericEdit)}`,
        );
      }
    }
  }
  ...
  return summary
    ? `${summary[0].toLocaleUpperCase()}${summary.slice(1)}.`
...
export function generatePatchOverview(
  ...
  const professions = generated.professions as
    Record<string, MutableRecord> | undefined;
  delete (generated as unknown as MutableRecord).notes;
  for (const [professionId, patch] of Object.entries(professions || {})) {
    delete patch.notes;
    const professionMetadata = metadata.find(
      (entry) => entry.professionId === professionId,
    );
    const skills = (patch.skills || {}) as Readonly<
...
export function compactPatchPreview(preview: PatchPreview): PatchPreview {
  const compacted = compactValue(structuredClone(preview)) as
