...
function castStatus(cast: DpsReportCast): EvtcRotationActionStatus {
...
function buildRotation(actions: readonly DpsReportResolvedAction[], combatStart: number): EvtcReconstructedCommand[] {
  const entries: Array<
    | { readonly type: 'action'; readonly action: DpsReportResolvedAction }
    | { readonly type: 'combat-start'; readonly at: number }
  > = actions.map((action) => ({ type: 'action', action }));
  entries.push({ type: 'combat-start', at: combatStart });
  entries.sort((left, right) => {
    ...
    return left.type === 'combat-start'
      ? -1
      : right.type === 'combat-start'
        ? 1
        : left.action.eventIndex - right.action.eventIndex;
  });

  const rotation: EvtcReconstructedCommand[] = [];
  let activeCastEnd = actions[0]?.start ?? combatStart;
  ...
  for (const entry of entries) {
    ...
    if (entry.type === 'combat-start') {
      rotation.push(
        previousCastStart != null && overlapping
          ? { name: '__combat_start', offset: Math.max(0, at - previousCastStart) }
          : { name: '__combat_start' }
      );
      continue;
    }

    // Profession recovery may need a simulator control marker when the report
    // omits a golem precast reset; controls are emitted without catalog skills.
    if (entry.action.control === 'cooldown-reset') {
...
export function reconstructDpsReportWithProfile(
  ...
  const actions: DpsReportRotationAction[] = resolved.map((action) => ({
    ...
    supportedByCatalog: action.skill != null || action.control != null,
  ...
  return {
    ...
    actions,
    rotation: buildRotation(resolved, phase.start),
