...
Native shared-engine profession. Entry point `engineer.html` boots the shared application shell. `definition.ts` is the
stable export and composes the Core-first tuple from `modules.ts`. A runtime contains Core plus at most one of Scrapper,
