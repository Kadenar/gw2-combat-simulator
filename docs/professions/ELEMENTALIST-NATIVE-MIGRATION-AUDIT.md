...
| ELM-504 | Medium   | Air, Ice, and Earth Elemental AI are not implemented because no representative EVTC traces were supplied. Native simulation warns and emits no borrowed Fire behavior for these variants. |

ELM-503 is an encounter-model migration rather than a native Elementalist combat-state blocker. ELM-504 requires additional combat evidence.
