...
export function isTerrestrialSkill(
  ...
  return typeof filterSkill === "function"
    ? filterSkill(skill, { weapon }) !== false
    : true;
}
...
export function createProfessionSnapshot({
  profession,
  specializationData,
  traitData,
  skillData,
  ...
  return {
    specializations: buildSpecializationSnapshots(
      specializationData,
      traitData,
    ),
...
export async function fetchProfessionSnapshot({
  ...
  const profession = await fetchGw2Api(
    `/professions/${professionName}`,
    options,
  );
  const specializationData = await fetchManyGw2(
    "specializations",
  ...
  const associations = professionSkillAssociations(
    profession,
    specializationData,
    config,
  );
...
export function serializeProfessionSnapshot({
  professionName,
  snapshotDate,
  specializations,
  skills,
  ...
  return [
    ...
    `// Snapshot: ${snapshotDate}. Run scripts/update-profession-api-data.mjs --profession ${professionName} to refresh.`,
