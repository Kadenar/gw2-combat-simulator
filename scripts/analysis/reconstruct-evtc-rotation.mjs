...
} from "../../dist/js/evtc-analyzer/rotation/index.js";
...
if (requestedAddress) {
  ...
  selected = players.find(
    (player) => address != null && BigInt(player.address) === address,
  );
} else if (
  players.length === 1 ||
...
const output = {
  metadata: {
    ...
    timingSource:
      "EVTC animation activations, state changes, and direct instant-skill effects",
    combatStartTimestampMs: result.combatStartTimestampMs,
    warnings: result.warnings,
    ...(process.argv.includes("--timeline")
      ? { reconstructedActions: result.actions }
      : {}),
  },
