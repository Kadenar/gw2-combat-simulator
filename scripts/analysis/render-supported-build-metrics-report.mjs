...
if (!baselinePath) {
  throw new TypeError(
    "Usage: node render-supported-build-metrics-report.mjs <baseline-json> [baseline-commit]",
  );
}
...
if (baseline.length !== rows.length) {
  throw new TypeError(
    `Metric count changed from ${baseline.length} to ${rows.length}.`,
  );
}

const changed = rows.filter(
  ({ before, current: after }) =>
    Math.abs(after.dps - before.dps) > 1e-6 ||
    Math.abs(after.totalDamage - before.totalDamage) > 1e-6,
);
const negativeNonReaper = changed.filter(
  ({ before, current: after }) =>
    after.profession !== "necromancer" &&
    (after.dps < before.dps - 1e-6 ||
      after.totalDamage < before.totalDamage - 1e-6),
);
const reaperExceptions = changed.filter(
  ({ current: after }) =>
    after.profession === "necromancer" && after.section === "Reaper",
);

const baselineWarnings = new Set(
  baseline.flatMap((entry) => entry.warnings.map(warningKey)),
);
const currentWarningRows = current.filter((entry) => entry.warnings.length);
const addedWarnings = currentWarningRows.flatMap((entry) =>
  entry.warnings
    .filter((warning) => !baselineWarnings.has(warningKey(warning)))
    .map((warning) => ({ entry, warning })),
);
...
const lines = [
  ...
  "",
  ...addedWarnings.map(
    ({ entry, warning }) => `- ${entry.section} — ${entry.label}: ${warning}`,
  ),
  "",
...
for (const profession of [
  ...new Set(current.map((entry) => entry.profession)),
]) {
  const professionRows = rows.filter(
    ({ current: entry }) => entry.profession === profession,
  );
  lines.push(
