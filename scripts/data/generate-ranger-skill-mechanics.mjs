...
function effectsFor(skill, petSkill) {
  ...
  for (const fact of skill.facts || []) {
    if (fact.type === "Damage" && Number(fact.dmg_multiplier) > 0) {
      ...
      fact.type === "Buff" &&
      CONDITIONS.has(fact.status) &&
      Number(fact.duration || 0) > 0
    ) {
      effects.push({
        type: "condition",
...
const rawSkills = await fetchMany("skills", [
  ...new Set([...SKILLS.map((skill) => skill.id), ...petSkillIds]),
]);
...
const activationById = new Map(
  identities.map((skill, index) => [skill.id, activations[index]]),
);
...
const supplementalMechanics = new Map([
  ...
  [
    77174,
    {
      castTimeMs: 1000,
      effects: [
        { type: "strike", coefficient: 4, hits: 1 },
        { type: "control" },
      ],
      arrowCost: 3,
      handlerId: "ranger.cyclone-bow-skill",
...
const overrides = new Map([
  [
    31869,
    { castTimeMs: 0, effects: [], handlerId: "ranger.celestial-avatar-enter" },
  ],
  [
    31411,
    { castTimeMs: 0, effects: [], handlerId: "ranger.celestial-avatar-exit" },
  ],
  ...
  [
    76787,
    { castTimeMs: 0, effects: [], handlerId: "ranger.cyclone-bow-enter" },
  ],
...
for (const identity of identities) {
  ...
  const castTimeMs =
    ...
    (petSkill ? 500 : identity.type === "Profession" ? 0 : 500);
...
`;
  const target = fileURLToPath(
    new URL(
      `../../js/professions/ranger/${directories[owner]}/skills.ts`,
      import.meta.url,
    ),
  );
