  ...
  for (const current of raw.facts || []) {
    ...
    if (current.type === 'Damage' && Number(current.dmg_multiplier) > 0) {
        ...
        baseStrike.coefficientModifiers = [
          {
            ...
            multiplier: (Number(current.dmg_multiplier) * hits) / Number(baseStrike.coefficient)
          }
        ];
        continue;
      }
      ...
      effects.push({
        ...
        hits,
        ...(current.text !== 'Damage' ? { name: `${raw.name} — ${current.text}` } : {})
      });
    } else if (current.type === 'Buff' && current.status === 'Blindness') {
      effects.push({ type: 'blind' });
    } else if (current.type === 'Buff' && CONDITIONS.has(current.status) && Number(current.duration) > 0) {
      effects.push({
        type: 'condition',
    ...
    } else if (current.type === 'Buff' && BOONS.has(current.status) && Number(current.duration) > 0) {
      effects.push({
        type: 'boon',
      ...
      effects.push({
        ...
        metadata: {
          ...
          ...(Number(current.duration) > 0 ? { duration: Number(current.duration) } : {})
        }
      });
    }
  }
