...
for (const [professionId, patch] of Object.entries(
  ...
  const modifierTargets = new Map(
    (profession.previewModifierRuleTargets || []).map((target) => [
      target.id,
      target,
    ]),
  );
  for (const [id, edit] of Object.entries(patch.modifierRules || {})) {
    ...
    for (const field of ["amount", "factor"]) {
      ...
      console.log(
        `  [ ] modifier ${id} (${target.moduleId}) ${field}: ${describeNumericEdit(edit[field])}`,
      );
    }
    for (const [name, numericEdit] of Object.entries(edit.parameters || {})) {
      console.log(
        `  [ ] modifier ${id} (${target.moduleId}) parameters.${name}: ${describeNumericEdit(numericEdit)}`,
      );
    }
  }
